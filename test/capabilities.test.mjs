...
test("version --json emits the packaged version document on stdout only", async () => {
  ...
  assert.deepEqual(payload.tool, { name: "azeforge", version: TOOL_VERSION });
  ...
  assert.ok(ids.includes("azeforge.capabilities/v2"));
  ...
  assert.ok(result.stdout.toString("utf8").includes(`"version":"${TOOL_VERSION}"`));
...
test("capabilities --json enumerates the P0 contract in canonical order", async () => {
  ...
  assert.deepEqual(Object.keys(payload), schema.required);
  assert.equal(payload.schema, "azeforge.capabilities/v2");
  ...
  assert.deepEqual(payload.profiles.tex, {
  ...
  assert.deepEqual(payload.engines.tex, {
    profiles: ["circuitikz", "tikz", "pgfplots", "chemfig", "tikz-cd"],
  ...
  assert.deepEqual(payload.limits.blocks.tex, {
    maxBodyChars: 50000,
  });
...
test("human version, capabilities, and help use stderr with empty stdout", () => {
  ...
  assert.ok(runCli(["version"]).stderr.toString("utf8").includes(`azeforge ${TOOL_VERSION}`));

  const versionLine = runCli(["--version"]);
  assert.equal(versionLine.status, 0);
  assert.equal(versionLine.stdout.toString("utf8"), `azeforge ${TOOL_VERSION}\n`);
...
test("human reports derive names and versions from the canonical model", () => {
  ...
  assert.ok(version.stderr.toString("utf8").includes(`azeforge ${TOOL_VERSION}\nazemark versions: 2\ndocument schema versions: 3`));
});
