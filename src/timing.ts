...
import {
  TIMING_BODY_SYNTAX_ID,
  TIMING_BODY_SYNTAX_VERSION,
  TIMING_PLUGIN_TYPE,
...
const STATE_CHARACTERS: Readonly<Record<string, TimingIntervalState>> = Object.freeze({
  "0": "low",
  "1": "high",
  x: "unknown",
  z: "impedance",
...
function limitExceeded(
  ...
  return diag(
    "limit-exceeded",
    `Timing ${subject} ${count} exceeds ${limit}.`,
    range,
    sourceName,
    { subject, count, limit },
  );
...
function inlineText(
  ...
  sourceName: string | undefined,
  diagnostics: Diagnostic[],
  limit = MAX_TIMING_TEXT_CODE_POINTS,
  ...
  if (runs.some((run) => run.kind !== "quantity" && codePoints(run.value) > MAX_TIMING_RUN_CODE_POINTS)) {
    diagnostics.push(
      limitExceeded("text run code points", MAX_TIMING_RUN_CODE_POINTS + 1, MAX_TIMING_RUN_CODE_POINTS, range, sourceName),
    );
    return undefined;
  }
  return runs;
...
interface BodyItem {
  ...
  /** Ordered nested records for `intervals:`, each with its own fields. */
  readonly records: readonly { readonly range: SourceRange; readonly fields: readonly Field[] }[];
}
...
interface MutableBodyItem {
  ...
  readonly records: { readonly range: SourceRange; readonly fields: Field[] }[];
}

const BODY_SHAPE = "Timing body entries begin with `- kind:` and use flat `key: value` fields.";

function parseBody(
  lines: readonly TimingInputLine[],
  diagnostics: Diagnostic[],
  sourceName: string | undefined,
): BodyItem[] {
...
function parseWave(
  ...
  while (index < raw.length) {
    let digits = "";
    while (/[0-9]/.test(raw[index] ?? "")) digits += raw[index++];
...
function busValue(
  ...
  sourceName: string | undefined,
  diagnostics: Diagnostic[],
): { readonly value: CircuitText; readonly next: number } | undefined {
...
function parseIntervals(
  ...
  for (const record of item.records) {
    ...
    let text: CircuitText | undefined;
    if (value !== undefined) {
      if (resolved !== "bus") {
...
function intervalSpan(interval: TimingInterval): number {
...
export function validateTimingBlock(options: TimingValidationOptions): {
  ...
  const signalRefs = new Map<string, TimingSignal>();
  const groups: TimingGroup[] = [];
  const markers: TimingMarker[] = [];
  const arrows: { from: TimingAnchor; to: TimingAnchor; label?: CircuitText; range: SourceRange }[] = [];
  ...
  const depth = runs.map((run, index) =>
    runs.filter((other, otherIndex) =>
      otherIndex !== index &&
      other.length > run.length &&
      Math.min(...run) >= Math.min(...other) && Math.max(...run) <= Math.max(...other),
    ).length + 1,
  );
  ...
  if (diagnostics.length > 0 || title === undefined) return { diagnostics };
  return {
    diagnostics,
    block: {
      kind: "timing",
...
export const timingPlugin: AzeBlockPlugin = Object.freeze({ descriptor });
