...
export function parseTexHeader(entries: readonly { readonly key: string; readonly value: string; readonly range: SourceRange }[], sourceName: string | undefined): TexHeader {
  ...
  for (const entry of entries) {
    ...
    if (entry.key === "title" || entry.key === "description") { const limit = entry.key === "title" ? 200 : 1000; if (entry.value.length === 0 || entry.value.length > limit) diagnostics.push(diagnostic("azeforge.tex#invalid-attribute", `TeX attribute ${entry.key} must be 1..${limit} characters.`, entry.range, sourceName, { attribute: entry.key })); else if (entry.key === "title") title = entry.value; else description = entry.value; continue; }
...
export function validateTexBody(options: { readonly header: TexHeader; readonly body: string; readonly blockRange: SourceRange; readonly sourceName?: string }): { readonly block?: TexBlock; readonly diagnostics: readonly Diagnostic[] } {
  ...
  if (body.length === 0) diagnostics.push(diagnostic("azeforge.tex#empty", "TeX Block body must not be empty.", options.blockRange, options.sourceName));
  ...
  if (/\\(?:documentclass|usepackage|input|include|openin|openout|write18)\b/.test(body)) diagnostics.push(diagnostic("azeforge.tex#forbidden-command", "TeX Block body may not declare a document, packages, files, or shell escape.", options.blockRange, options.sourceName));
