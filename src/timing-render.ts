...
export const TIMING_HTML_BLOCK_RENDERER_ID = "azeforge.timing.html/v1" as const;
export const TIMING_HTML_BLOCK_RENDERER_VERSION = "1.0.0" as const;
export const TIMING_EMITTER_VERSION = "1.0.0" as const;
...
function textRuns(
  ...
  const runs = text
    .map((run) => {
      ...
      return `<tspan dy="${dy}" font-size="${quantize(scripted ? size * 0.72 : size)}">${escapeXml(
        run.kind === "quantity" ? `${run.coefficient} ${run.prefix}${run.unit}` : run.value,
      )}</tspan>`;
    })
    .join("");
...
function intervalLength(interval: TimingInterval): number {
  return Number(interval.count ?? interval.duration ?? "0");
}

/** Level of an interval on its row: `-1` high, `+1` low, `0` mid-line. */
function levelOf(interval: TimingInterval, previous: number): number {
...
function hatch(x0: number, x1: number, y: number): string {
  const parts: string[] = [];
...
function intervalMarkup(
  interval: TimingInterval,
  x: number,
  width: number,
  y: number,
  previousState: TimingInterval["state"] | undefined,
  ...
  const line = (path: string, attributes = ""): string =>
    `<path class="${stateClass}" d="${path}" ${STROKE} stroke-width="2"${attributes}/>`;
  const band = (fillLabel: CircuitText | undefined): string => {
    const outline =
      `M${quantize(x)} ${quantize(y)}L${quantize(x + 6)} ${quantize(y - 10)}H${quantize(x + width - 6)}` +
  ...
  switch (shape) {
    ...
    case "rise":
      return line(
        `M${quantize(x)} ${quantize(y + AMPLITUDE)}L${quantize(x + width)} ${quantize(y - AMPLITUDE)}`,
      );
    case "fall":
      return line(
        `M${quantize(x)} ${quantize(y - AMPLITUDE)}L${quantize(x + width)} ${quantize(y + AMPLITUDE)}`,
    ...
    case "unknown":
      return (
        `<path class="${stateClass}" d="M${quantize(x)} ${quantize(y)}H${quantize(x + width)}" ${STROKE} stroke-width="1"/>` +
        hatch(x, x + width, y)
      );
    case "impedance":
      return line(
        `M${quantize(x)} ${quantize(y)}H${quantize(x + width)}`,
...
function arrowHead(
  ...
  return (
    `<path d="M${quantize(tipX)} ${quantize(tipY)}` +
 ...
 */
export function renderTimingFragment(block: TimingBlock, _context: BlockRendererContext): string {
  const spanUnits = block.signals.reduce(
    (span, signal) =>
      Math.max(
        span,
        Number(signal.phase) +
          signal.intervals.reduce((total, interval) => total + intervalLength(interval), 0),
      ),
    0,
  );
  ...
  const rowCenter = (index: number): number => ROW_TOP + index * ROW_HEIGHT + ROW_HEIGHT / 2;
  ...
  const xFor = (units: number): number => gridLeft + units * pxPerUnit;

  const grid: string[] = [];
  for (let cycle = 0; cycle <= spanUnits; cycle += 1) {
    grid.push(
      `<path class="aze-timing-gridline" d="M${quantize(xFor(cycle))} ${quantize(ROW_TOP - 18)}V${quantize(height - PAD)}" ${STROKE} stroke-width="0.5" stroke-opacity="0.35"/>`,
    );
  }
  ...
  const rows = block.signals
    .map((signal, index) => {
      ...
      let previousState: TimingInterval["state"] | undefined;
      let previousLevel = 0;
      const shapes = signal.intervals
        .map((interval) => {
          const length = intervalLength(interval);
          const markup = intervalMarkup(
            interval,
            xFor(cursor),
            length * pxPerUnit,
            y,
      ...
      if (signal.clock) {
        ...
        for (const interval of signal.intervals) {
          ...
          if (level === 1 && next === -1) {
            edges.push(
              `<path class="aze-timing-edge" d="M${quantize(edgeX - 5)} ${quantize(y + AMPLITUDE)}L${quantize(edgeX + 5)} ${quantize(y + AMPLITUDE)}L${quantize(edgeX)} ${quantize(y + AMPLITUDE - 9)}Z" fill="currentColor" stroke="none"/>`,
            );
          }
          level = next;
  ...
  const runs = block.groups.map((group) =>
    group.signals.map((ref) => block.signals.findIndex((signal) => signal.ref === ref)),
  );
  const groups = block.groups
    .map((group, index) => {
      ...
      const depth =
        runs.filter(
          (other, otherIndex) =>
            ...
            Math.max(...positions) <= Math.max(...other),
        ).length + 1;
      ...
      const label =
        group.label.length === 0 ? "" : textRuns(group.label, x + 4, top - 5, 11, "start");
  ...
  const markers = block.markers
    .map((marker) => {
      ...
      const label =
        marker.label === undefined ? "" : textRuns(marker.label, x, ROW_TOP - 34, 11, "middle");
      ...
      const label =
        arrow.label === undefined
          ...
          : textRuns(
              arrow.label,
              (from.x + to.x) / 2,
              Math.min(from.y, to.y) - 6,
              11,
              "middle",
            );
      return (
        `<g class="aze-timing-arrow">${label}` +
  ...
  return (
    `<figure class="aze-timing" id="${svgId}" data-timing-scale="${block.scale}"${label}${number}>` +
...
export function timingDependencyClosure(): JsonValue {
  return Object.freeze({ emitter: TIMING_EMITTER_VERSION });
}

export const timingHtmlBlockRenderer: AzeBlockRenderer<TimingBlock> = Object.freeze({
  ...
  render(block: TimingBlock, context: BlockRendererContext): string {
