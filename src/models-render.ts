...
import type {
  ModelsLayout,
  ModelsLayoutFragment,
  ModelsMarkerKind,
  ModelsRole,
  ModelsLayoutGeometry,
  ModelsLayoutKind,
  ModelsLayoutNestedBox,
  ModelsLayoutPath,
} from "./models-layout.js";
...
export const MODELS_EMITTER_VERSION = "1.0.0" as const;
export const MODELS_HTML_BLOCK_RENDERER_VERSION = "1.0.0" as const;
export const SEQUENCE_HTML_BLOCK_RENDERER_ID = "azeforge.sequence.html/v1" as const;
export const STATE_HTML_BLOCK_RENDERER_ID = "azeforge.state.html/v1" as const;
export const ENTITY_HTML_BLOCK_RENDERER_ID = "azeforge.entity.html/v1" as const;
export const CLASS_HTML_BLOCK_RENDERER_ID = "azeforge.class.html/v1" as const;
...
const ID_PREFIX: Readonly<Record<ModelsLayoutKind, string>> = Object.freeze({
  sequence: "seq",
  state: "state",
  entity: "entity",
  class: "class",
...
const KIND_TITLE: Readonly<Record<ModelsLayoutKind, string>> = Object.freeze({
  sequence: "Sequence",
  state: "State machine",
...
export class ModelsRenderError extends Error {
  ...
  readonly remedy =
    "Re-check the model declaration list; a renderer failure publishes no Artifact.";
...
function assertExtent(layout: ModelsLayout): void {
  const width = layout.widthPx;
  const height = layout.heightPx;
  if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
    throw new ModelsRenderError(
      `The ${layout.kind} figure has no positive finite extent (${quantize(width)} × ${quantize(height)} px).`,
    );
  }
  if (width > MODELS_MAX_WIDTH_PX || height > MODELS_MAX_HEIGHT_PX) {
    throw new ModelsRenderError(
      `The ${layout.kind} figure is ${quantize(width)} × ${quantize(height)} px, over the ` +
...
const ROLE_CLASS: Readonly<Record<ModelsRole, string>> = Object.freeze({
  ...
  "participant-label":
    "aze-models-label aze-models-caption aze-sequence-participant-label aze-participant-label",
  ...
  "fragment-label":
    "aze-models-label aze-models-caption aze-sequence-fragment-label aze-fragment-label",
  ...
  "transition-label":
    "aze-models-label aze-models-member aze-state-transition-label aze-transition-label",
  ...
  "entity-optional":
    "aze-models-label aze-models-member aze-entity-attribute aze-entity-optional aze-attribute-label",
  ...
  "entity-cardinality":
    "aze-models-label aze-models-marker aze-entity-cardinality aze-cardinality-label",
  "entity-role": "aze-models-label aze-models-marker aze-entity-role aze-role-label",
  "entity-relationship-label":
    "aze-models-label aze-models-caption aze-entity-relationship-label aze-relationship-label",
  ...
  "class-static-member":
    "aze-models-label aze-models-member aze-class-member aze-member-label aze-class-marker aze-class-static",
  "class-visibility":
    "aze-models-label aze-models-member aze-class-visibility aze-class-marker aze-member-label",
  "class-multiplicity":
    "aze-models-label aze-models-marker aze-class-multiplicity aze-multiplicity-label",
  "class-relationship-label":
    "aze-models-label aze-models-caption aze-class-relationship-label aze-relationship-label",
...
const ARROW_ROLE_CLASS: Readonly<Partial<Record<ModelsRole, string>>> = Object.freeze({
...
const MARKER_CLASS: Readonly<Record<Exclude<ModelsMarkerKind, "arrow">, string>> = Object.freeze({
  none: "",
...
function markerClass(path: ModelsLayoutPath): string {
...
function emitBox(box: ModelsLayoutGeometry["boxes"][number]): string {
  const radius =
    box.cornerRadiusPx > 0
      ? ` rx="${quantize(box.cornerRadiusPx)}" ry="${quantize(box.cornerRadiusPx)}"`
      : "";
  return (
    `<rect class="${ROLE_CLASS[box.role]}" x="${quantize(box.x)}" y="${quantize(box.y)}"` +
    ` width="${quantize(box.width)}" height="${quantize(box.height)}"${radius}` +
    ` aria-hidden="true"/>`
  );
}

function emitNestedBox(box: ModelsLayoutNestedBox): string {
  const radius =
    box.cornerRadiusPx > 0
      ? ` rx="${quantize(box.cornerRadiusPx)}" ry="${quantize(box.cornerRadiusPx)}"`
      : "";
  const dividers = box.dividerYs
    .map(
      (offset) =>
        `<line class="${ROLE_CLASS[box.dividerRole]}" x1="${quantize(box.x)}" y1="${quantize(box.y + offset)}"` +
...
function emitCircle(circle: ModelsLayoutGeometry["circles"][number]): string {
  const outer =
    `<circle class="${ROLE_CLASS[circle.role]}" cx="${quantize(circle.cx)}" cy="${quantize(circle.cy)}"` +
    ` r="${quantize(circle.radiusPx)}" aria-hidden="true"/>`;
  if (circle.innerRadiusPx <= 0 || circle.innerRole === undefined) return outer;
  return (
    outer +
...
function emitBar(bar: ModelsLayoutGeometry["bars"][number]): string {
  return (
    `<rect class="${ROLE_CLASS[bar.role]}" x="${quantize(bar.x)}" y="${quantize(bar.y)}"` +
    ` width="${quantize(bar.width)}" height="${quantize(bar.height)}" aria-hidden="true"/>`
  );
}

function emitNote(note: ModelsLayoutGeometry["notes"][number]): string {
...
function emitFragment(fragment: ModelsLayoutFragment): string {
  const divisions = fragment.divisionYs
    .map(
      (y) =>
        `<line class="${ROLE_CLASS[fragment.divisionRole]}" x1="${quantize(fragment.x)}" y1="${quantize(y)}"` +
        ` x2="${quantize(fragment.x + fragment.width)}" y2="${quantize(y)}" aria-hidden="true"/>`,
    )
    .join("");
  return (
    `<rect class="${ROLE_CLASS[fragment.role]}" x="${quantize(fragment.x)}" y="${quantize(fragment.y)}"` +
 ...
 */
function emitMarker(path: ModelsLayoutPath): string {
  if (path.marker === "none" || path.points.length < 2) return "";
  ...
  if (path.marker === "arrow") {
    ...
    return (
      `<path class="${markerClass(path)}" d="M${quantize(tip.x)} ${quantize(tip.y)}` +
...
function emitPath(path: ModelsLayoutPath): string {
  const first = path.points[0];
  if (first === undefined) return "";
  const data = path.points
    .map((entry, index) =>
      `${index === 0 ? "M" : "L"}${quantize(entry.x)} ${quantize(entry.y)}`,
    )
    .join("");
  return (
    `<path class="${ROLE_CLASS[path.role]}" d="${data}${path.closed ? "Z" : ""}" aria-hidden="true"/>` +
    emitMarker(path)
  );
}

function emitText(text: ModelsLayoutGeometry["texts"][number]): string {
  return (
    `<text class="${ROLE_CLASS[text.role]}" x="${quantize(text.x)}" y="${quantize(text.y)}"` +
    ` text-anchor="${text.anchor}"${text.ariaHidden ? ' aria-hidden="true"' : ""}>` +
    `${escapeXml(text.text)}</text>`
  );
}

function emitGroup(className: string, inner: string): string {
...
function describeSequence(block: SequenceBlock): string {
  const timeline: string[] = [];
  let messages = 0;
  let notes = 0;
  let altDivisions = 0;
  let loops = 0;
  const walk = (items: readonly SequenceTimelineItem[]): void => {
    for (const item of items) {
      if (item.kind === "message") {
        messages += 1;
        timeline.push(
          `${timeline.length + 1}. ${item.from} → ${item.to}${
      ...
      if (item.kind === "loop") {
        loops += 1;
        timeline.push(
          `${timeline.length + 1}. loop${item.condition === undefined ? "" : ` [${item.condition}]`}`,
        );
        walk(item.body);
        continue;
      }
      for (const division of item.divisions) {
        altDivisions += 1;
        timeline.push(
          `${timeline.length + 1}. alt${
            division.condition === undefined ? "" : ` [${division.condition}]`
          }`,
        );
        walk(division.body);
      }
  ...
  if (block.participants.length > 0) {
    parts.push(
      `Lanes in order: ${block.participants
        .map((participant) =>
  ...
  if (timeline.length > 0) parts.push(`Timeline in order: ${timeline.join("; ")}.`);
  return describeWith(block.description, parts);
}

function describeState(block: StateBlock): string {
  const states: string[] = [];
  const transitions: string[] = [];
  let composites = 0;
  let initial = 0;
  let final = 0;
  const walk = (items: readonly (StateScopedItem | StateTransition)[]): void => {
    for (const item of items) {
      ...
      if (item.kind === "initial") initial += 1;
      else final += 1;
    }
  };
  walk(block.items);
  ...
  if (states.length > 0) parts.push(`States in authored order: ${states.join("; ")}.`);
...
function describeEntity(block: EntityBlock): string {
  const entities = block.items.filter((item): item is EntityEntity => item.kind === "entity");
  const relationships = block.items.filter(
    (item): item is EntityRelationship => item.kind === "relationship",
  );
  const attributeCount = entities.reduce(
    (total, entity) => total + (entity.attributes?.length ?? 0),
    0,
  );
  const parts: string[] = [
  ...
  if (entities.length > 0) {
    parts.push(
      `Entities in authored order: ${entities
        .map((entity) =>
          entity.label === undefined ? entity.name : `${entity.name} (${entity.label})`,
        )
        .join("; ")}.`,
    );
  }
  if (relationships.length > 0) {
    parts.push(
      `Relationships in authored order: ${relationships
        .map(
          (relationship) =>
            `${relationship.first.entity} ${cardinalityDisplay(relationship.first.cardinality)}` +
...
function describeClass(block: ClassBlock): string {
  const classifiers = block.items.filter(
    (item): item is ClassClassifier => item.kind === "class",
  );
  const relationships = block.items.filter(
    (item): item is ClassRelationship => item.kind === "relationship",
  );
  const interfaces = classifiers.filter((classifier) => classifier.kind === "interface").length;
  const attributes = classifiers.reduce(
    (total, classifier) => total + (classifier.attributes?.length ?? 0),
    0,
  );
  const operations = classifiers.reduce(
    (total, classifier) => total + classifier.operations.length,
    0,
  );
  const parts: string[] = [
  ...
  if (classifiers.length > 0) {
    parts.push(
      `Classifiers in authored order: ${classifiers
        .map((classifier) => {
  ...
  if (relationships.length > 0) {
    parts.push(
      `Relationships in authored order: ${relationships
        .map(
          (relationship) =>
            `${relationship.from} ${relationship.fromMultiplicity ?? ""} → ${relationship.to} ` +
...
function describeWith(authored: string | undefined, parts: readonly string[]): string {
  const pieces: string[] = [];
  if (authored !== undefined && authored !== "") pieces.push(`${authored}.`);
  pieces.push(...parts);
...
interface FigureMeta {
  readonly kind: ModelsLayoutKind;
 ...
 */
function emitFigure(layout: ModelsLayout, meta: FigureMeta): string {
  assertExtent(layout);
  ...
  const shapes = [
    ...
    emitGroup(
      `aze-${kind}-boxes`,
      layout.nestedBoxes.map(emitNestedBox).join("") + layout.boxes.map(emitBox).join(""),
    ),
  ...
  return (
    `<figure class="aze-${kind}" id="${base}"${idAttribute}${numberAttribute}>` +
...
export function renderSequenceFragment(
  block: SequenceBlock,
  context: ModelsRenderContext,
): string {
...
export function renderStateFragment(block: StateBlock, context: ModelsRenderContext): string {
...
export function renderEntityFragment(block: EntityBlock, context: ModelsRenderContext): string {
...
export function renderClassFragment(block: ClassBlock, context: ModelsRenderContext): string {
 ...
 */
export function modelsDependencyClosure(): JsonValue {
...
function descriptor(id: string, blockType: string): {
  readonly id: string;
...
export const sequenceHtmlBlockRenderer: AzeBlockRenderer<SequenceBlock> = Object.freeze({
  descriptor: descriptor(SEQUENCE_HTML_BLOCK_RENDERER_ID, "sequence"),
  render(block: SequenceBlock, context: BlockRendererContext): string {
    return renderSequenceFragment(block, context);
  },
});

export const stateHtmlBlockRenderer: AzeBlockRenderer<StateBlock> = Object.freeze({
  descriptor: descriptor(STATE_HTML_BLOCK_RENDERER_ID, "state"),
  render(block: StateBlock, context: BlockRendererContext): string {
    return renderStateFragment(block, context);
  },
});

export const entityHtmlBlockRenderer: AzeBlockRenderer<EntityBlock> = Object.freeze({
  descriptor: descriptor(ENTITY_HTML_BLOCK_RENDERER_ID, "entity"),
  render(block: EntityBlock, context: BlockRendererContext): string {
    return renderEntityFragment(block, context);
  },
});

export const classHtmlBlockRenderer: AzeBlockRenderer<ClassBlock> = Object.freeze({
  descriptor: descriptor(CLASS_HTML_BLOCK_RENDERER_ID, "class"),
  render(block: ClassBlock, context: BlockRendererContext): string {
