...
export function blockInlineRuns(
  block: AzeBlock | ParsedBlock,
): readonly (readonly Inline[])[] {
  if (block.kind === "invalid") return [];
  switch (block.kind) {
    case "heading":
    case "paragraph":
    ...
    case "derivation":
      return block.steps.flatMap((step) =>
        step.annotation === undefined ? [] : [step.annotation],
      );
    case "figure":
    case "statement":
    case "example":
...
function algorithmInlineRuns(
  statements: readonly AlgorithmStatement[],
): readonly (readonly Inline[])[] {
  const out: (readonly Inline[])[] = [];
...
function tableProseCells(block: TableBlock): readonly (readonly Inline[])[] {
  const out: (readonly Inline[])[] = [];
  for (const row of block.data.rows) {
    for (const column of block.data.columns) {
      const cell = row[column.key];
      if (cell !== undefined && cell.kind === "prose") out.push(cell.value);
    }
  }
  return out;
}
