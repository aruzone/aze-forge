...
export const CONTROL_LAYOUT_ERROR_CODE = "azeforge.renderer#control-layout" as const;
/** Version of the option set below; it rides in the rendering fingerprint. */
export const CONTROL_LAYOUT_OPTIONS_VERSION = "control-options/v1" as const;
...
const ELK_DIRECTION: Readonly<Record<ControlFlow, string>> = Object.freeze({
  "top-to-bottom": "DOWN",
  "bottom-to-top": "UP",
  "left-to-right": "RIGHT",
  "right-to-left": "LEFT",
...
const LAYOUT_REMEDY =
  "Shorten the block's transfer-function and signal labels, or reduce its number of items and edges, then render the document again.";

/* ------------------------------------------------------------------ *
 * Returned geometry (contract §5 — frozen)
 * ------------------------------------------------------------------ */

export interface ControlLayoutPoint { readonly x: number; readonly y: number }
...
function refusal(detail: string, remedy: string = LAYOUT_REMEDY): ControlLayoutError {
...
export function controlLabelTypography(theme: Theme): ControlLabelTypography {
  const tokens = theme.control;
  const size = (value: number): number => Math.max(value, tokens.minimumLabelFontSizePx);
 ...
 */
function blockBox(item: ControlBlockItem, typography: ControlLabelTypography): Box {
 ...
 */
function sumBox(item: ControlSumItem, typography: ControlLabelTypography): Box {
  let widestSign = 0;
  for (const sign of item.signs) {
    widestSign = Math.max(
      widestSign,
      Math.max(
        advanceWidth([{ kind: "text", value: sign }], typography.sumSignFontSizePx),
        0,
      ),
    );
  }
  const seated =
    (item.signs.length * (widestSign + SUM_SIGN_GAP)) / Math.PI;
  const diameter = Math.max(
    seated,
    typography.sumSignFontSizePx + 2 * SUM_SIGN_PADDING,
    MIN_SUM_DIAMETER,
  );
...
function stubBox(item: ControlStubItem, typography: ControlLabelTypography): Box {
...
function nodeBox(
  item: ControlBlockItem | ControlSumItem | ControlStubItem,
  typography: ControlLabelTypography,
): Box {
  switch (item.kind) {
    case "block":
...
interface PlannedNode {
  readonly id: string;
  readonly declaration: ControlBlockItem | ControlSumItem | ControlStubItem;
  /** Index of the declaration in the Block's authored declaration list. */
  readonly declarationIndex: number;
}

interface PlannedEdge {
  readonly id: string;
  readonly declaration: ControlEdgeItem;
  /** Index among the Block's `edge` declarations, in authored order. */
  readonly declarationIndex: number;
  readonly from: PlannedNode;
  readonly to: PlannedNode;
}
...
function nodeShell(planned: PlannedNode, typography: ControlLabelTypography): ElkNode {
 ...
 */
function planGraph(block: ControlBlock, theme: Theme): PlannedGraph {
  const typography = controlLabelTypography(theme);
  const nodes: PlannedNode[] = [];
  const edges: PlannedEdge[] = [];
  const nodeByName = new Map<string, PlannedNode>();
  const shells: ElkNode[] = [];
  const edgeDeclarations: ControlEdgeItem[] = [];
  ...
  for (const declaration of block.declarations) {
    ...
    const planned: PlannedNode = { id: `n${nodes.length}`, declaration, declarationIndex };
...
function projectNode(planned: PlannedNode, index: EngineIndex): ControlLayoutNode {
  ...
  if (element === undefined || origin === undefined) {
    throw refusal(
      `the layout engine returned no box for the item "${planned.declaration.name}".`,
    );
  }
...
function projectEdge(planned: PlannedEdge, index: EngineIndex): ControlLayoutEdge {
  const edge = index.edges.get(planned.id);
  if (edge === undefined) {
    throw refusal(
      `the layout engine returned no route for the edge from "${planned.declaration.from}".`,
    );
  }
  const sections = (edge.sections ?? []).map((section, sectionIndex) => {
    ...
    return {
      ...
      bendPoints: (section.bendPoints ?? []).map((point, pointIndex) =>
        projectPoint(point, `${what} bend ${pointIndex}`, { x: 0, y: 0 }),
      ),
    };
  });
  return {
 ...
 */
export async function layoutControl(
  block: ControlBlock,
  theme: Theme,
  ...
  try {
    ...
    if (error instanceof ControlLayoutError) throw error;
