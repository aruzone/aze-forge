...
function labelText(label: DiagramLabel | undefined): string {
...
function endpointText(endpoint: DiagramEndpoint): string {
...
function labelMarkup(
  ...
  const top =
    anchor === "center" ? y - ((lines.length - 1) * lineHeight) / 2 : y;
  const spans = lines
    .map(
      (line, index) =>
        `<tspan x="${quantize(x)}" dy="${quantize(index === 0 ? 0 : lineHeight)}">${escapeXml(line)}</tspan>`,
    )
    .join("");
...
function shapeMarkup(shape: DiagramShape, box: DiagramLayoutBox, id: string): string {
  ...
  const polygon = (points: readonly string[]): string =>
    `<polygon id="${id}" class="${base}" points="${points.join(" ")}"/>`;
  switch (shape) {
    case "rectangle":
      return `<rect id="${id}" class="${base}" x="${quantize(x)}" y="${quantize(y)}" width="${quantize(width)}" height="${quantize(height)}"/>`;
    case "rounded":
      return (
        `<rect id="${id}" class="${base}" x="${quantize(x)}" y="${quantize(y)}"` +
    ...
    case "circle":
      return (
        `<ellipse id="${id}" class="${base}" cx="${quantize(centerX)}" cy="${quantize(centerY)}"` +
    ...
    case "cylinder": {
      const cap = Math.max(
        CYLINDER_MIN_CAP_PX,
        Math.min(height * CYLINDER_CAP_RATIO, height / 3),
      );
...
function polyline(edge: DiagramLayoutEdge): readonly DiagramLayoutPoint[] {
...
function polylineMidpoint(points: readonly DiagramLayoutPoint[]): DiagramLayoutPoint | undefined {
...
function pathData(edge: DiagramLayoutEdge): string {
  const parts: string[] = [];
  for (const section of edge.sections) {
    parts.push(`M${point(section.startPoint.x, section.startPoint.y)}`);
    for (const bend of section.bendPoints) parts.push(`L${point(bend.x, bend.y)}`);
...
function arrowMarkup(edge: DiagramLayoutEdge): string {
  const section: DiagramLayoutSection | undefined =
    edge.sections[edge.sections.length - 1];
...
function edgeLabelMarkup(
  label: DiagramLabel,
  at: DiagramLayoutPoint,
  theme: Theme,
  ...
  if (text === "") return "";
  return (
    `<rect class="aze-diagram-edge-label-background" x="${quantize(centerX - width / 2)}" y="${quantize(centerY - height / 2)}"` +
    ` width="${quantize(width)}" height="${quantize(height)}"` +
...
interface DiagramEntities {
  readonly nodes: readonly DiagramNode[];
  readonly groups: readonly DiagramGroup[];
  readonly edges: readonly DiagramEdge[];
}

function entitiesOf(block: DiagramBlock): DiagramEntities {
...
function assertProjection(condition: boolean, detail: string): void {
  if (!condition) {
    throw new DiagramRenderError(
      `The diagram layout does not match the authored Block: ${detail}.`,
    );
  }
}
...
function describeDiagram(
  ...
  parts.push(
    `${entities.nodes.length} nodes, ${entities.edges.length} edges, ` +
    ...
    entities.edges.length === 0
      ...
      : `Edges in authored order: ${entities.edges
          .map((edge) => {
            ...
            return (
              `${endpointText(edge.from)} -> ${endpointText(edge.to)}` +
              (label === "" ? "" : ` (${label})`)
            );
          })
          .join("; ")}.`,
...
function diagramTitle(block: DiagramBlock, ordinal: number): string {
...
function emitDiagramFragment(
  ...
  assertProjection(
    layout.groups.length === entities.groups.length,
    `${layout.groups.length} layout groups for ${entities.groups.length} authored groups`,
  );
  assertProjection(
    layout.edges.length === entities.edges.length,
    `${layout.edges.length} layout edges for ${entities.edges.length} authored edges`,
  );
  ...
  const groups = layout.groups
    ...
    .map(({ group, authored, index }) => {
      assertProjection(
        authored !== undefined && authored.name === group.name,
        `group ${index} is "${group.name}", not the authored "${
          authored?.name ?? "?"
        }"`,
      );
      const depthClass =
        group.depth >= 1 && group.depth <= 3
          ? ` aze-diagram-group-depth-${group.depth}`
          : "";
      const rect =
        `<rect id="${svgId}-g-${index}" class="aze-diagram-group${depthClass}"` +
  ...
  const nodes = layout.nodes
    .map((node, index) => {
      ...
      assertProjection(
        authored !== undefined && authored.name === node.name,
        `node ${index} is "${node.name}", not the authored "${
          authored?.name ?? "?"
        }"`,
      );
      assertProjection(
        node.ports.length === (authored?.ports.length ?? -1),
        `node "${node.name}" has ${node.ports.length} layout ports for ${
          authored?.ports.length ?? 0
        } authored ports`,
      );
      ...
      const label: DiagramLabel =
        authoredLabel === undefined
          ...
          : authoredLabel;
      const labelTextMarkup = labelMarkup(
        label,
        node.x + node.width / 2,
      ...
      const ports = node.ports
        .map((port) => {
          const markup =
            `<rect id="${svgId}-p-${portIndex}" class="aze-diagram-port"` +
  ...
  const edges = layout.edges
    .map((edge, index) => {
      ...
      const label =
        authored.label === undefined || at === undefined
          ? ""
  ...
  const idAttribute =
    block.id === undefined ? "" : ` data-diagram-id="${escapeXml(block.id)}"`;
  const fragment =
    `<figure class="aze-diagram" data-ordinal="${quantize(ordinal)}"${idAttribute}` +
...
export const diagramHtmlBlockRenderer: DiagramBlockRenderer = Object.freeze({
