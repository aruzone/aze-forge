...
import {
  CONTROL_BODY_SYNTAX_ID,
  CONTROL_BODY_SYNTAX_VERSION,
  CONTROL_PLUGIN_TYPE,
...
export const DEFAULT_CONTROL_FLOW: ControlFlow = "left-to-right";
 ...
 */
const PORTS: Readonly<Record<ReferenceableKind, readonly string[]>> = Object.freeze({
  block: ["in", "out"],
  sum: ["in", "out"],
  input: ["out"],
  output: ["in"],
...
function limitExceeded(
  ...
  return diag(
    "limit-exceeded",
    `Control ${subject} ${count} exceeds ${limit}.`,
    range,
    sourceName,
    { subject, count, limit },
  );
 ...
 */
function parseBody(
  lines: readonly ControlInputLine[],
  diagnostics: Diagnostic[],
  sourceName: string | undefined,
): BodyItem[] {
  const items: BodyItem[] = [];
  let current: { readonly kind: string; readonly range: SourceRange; readonly fields: Field[] } | undefined;
  for (const line of lines) {
    ...
    if (item !== null) {
      ...
      if (entry === null || (entry[1] ?? "").toLowerCase() !== "kind") {
        diagnostics.push(
          diag("unknown-declaration", BODY_SHAPE, line.range, sourceName, {
...
function fieldOf(fields: readonly Field[], key: string): Field | undefined {
  ...
  for (const field of item.fields) {
    if (!registered.includes(field.key)) {
      diagnostics.push(
        diag(
          "unknown-field",
          `Control ${owner} field "${field.key}" is not supported.`,
          field.range,
          sourceName,
          { field: field.key },
    ...
    if (seen.has(field.key)) {
      diagnostics.push(
        diag("duplicate-field", `Control field "${field.key}" is declared twice.`, field.range, sourceName, {
          field: field.key,
        }),
      );
      continue;
...
function readName(
  ...
  if (field === undefined || field.value === "") {
    diagnostics.push(
      diag("missing-field", `${owner} requires \`name:\`.`, field?.range ?? fallbackRange, sourceName, {
        field: "name",
      }),
    );
    return undefined;
...
function readInlineText(
  ...
  if (field === undefined) {
    if (required) {
      diagnostics.push(
        diag("missing-field", `${owner} requires \`${key}:\`.`, fallbackRange, sourceName, { field: key }),
      );
    }
    return undefined;
  }
  return parseCircuitText({
    ...
    invalid: (message) => {
      diagnostics.push(
        diag("invalid-label", `Control \`${field.key}:\` is invalid inline text: ${message}`, field.range, sourceName, {
          field: field.key,
        }),
      );
    },
    exceeded: (count) => {
      diagnostics.push(
        limitExceeded("label code points", count, MAX_CONTROL_LABEL_CODE_POINTS, field.range, sourceName),
      );
    },
  });
}
...
function readSigns(
  ...
  sourceName: string | undefined,
  diagnostics: Diagnostic[],
): readonly ControlSign[] | undefined {
  if (field === undefined) {
    diagnostics.push(
      diag("missing-field", "A control sum requires `signs:`.", fallbackRange, sourceName, { field: "signs" }),
    );
    return undefined;
  }
  const raw = field.value;
  const invalid = (message: string, data: Record<string, JsonValue>): undefined => {
  ...
  for (const token of inner.split(",").map((entry) => entry.trim())) {
    ...
    if (/\s/.test(token)) {
      return invalid(
        "Control `signs:` entries are single `+` or `-` tokens; a nested `- -` collection is not readable.",
        { field: "signs", value: token },
      );
    }
    if (!oneOf(SIGNS, token)) {
      diagnostics.push(
        diag(
          "unknown-sign",
          `Control sign "${token}" is not registered.`,
          field.range,
          sourceName,
          { field: "signs", value: token },
          vocabularySuggestion(token, SIGNS),
        ),
      );
      return undefined;
    }
  ...
  if (signs.length > MAX_CONTROL_SIGNS_PER_SUM) {
    diagnostics.push(
      limitExceeded("signs per sum", signs.length, MAX_CONTROL_SIGNS_PER_SUM, field.range, sourceName),
    );
    return undefined;
  }
  return signs;
...
interface HeaderFields {
  ...
  readonly title: CircuitText | undefined;
  readonly description: CircuitText | undefined;
}
 ...
 */
function readHeader(
  headerLines: readonly ControlInputLine[],
  sourceName: string | undefined,
  diagnostics: Diagnostic[],
): HeaderFields {
  const seen = new Set<string>();
  let flow: ControlFlow = DEFAULT_CONTROL_FLOW;
  ...
  for (const line of headerLines) {
    ...
    if (match === null) {
      diagnostics.push(
        diag("unknown-field", "A control header entry is one `key: value` per line.", line.range, sourceName, {
          field: declarationKey(line.text),
        }),
      );
      continue;
    ...
    if (!HEADER_FIELDS.includes(key)) {
      diagnostics.push(
        diag(
          "unknown-field",
          `Control header field "${key}" is not supported.`,
          range,
          sourceName,
          { field: key },
    ...
    if (seen.has(key)) {
      diagnostics.push(
        diag("duplicate-field", `Control header field "${key}" is declared twice.`, range, sourceName, {
          field: key,
        }),
      );
      continue;
    ...
    seen.add(key);
    if (key === "flow") {
      if (oneOf(FLOWS, value)) flow = value;
      else {
        diagnostics.push(
          diag(
            "unknown-field",
            `Control flow "${value}" is not registered.`,
            range,
            sourceName,
            { field: key, value },
            vocabularySuggestion(value, FLOWS),
          ),
        );
      }
      continue;
    ...
    if (key === "id") {
      if (value === "") {
        diagnostics.push(
          diag("missing-field", "Control `id:` requires a non-empty value.", range, sourceName, { field: key }),
        );
        continue;
      }
      id = value;
    ...
    if (key === "number") {
      if (value !== "true" && value !== "false") {
        diagnostics.push(
          diag(
            "unknown-field",
            `Control \`number:\` must be true or false, not "${value}".`,
            range,
            sourceName,
            { field: key, value },
            vocabularySuggestion(value, ["true", "false"]),
          ),
        );
        continue;
      }
    ...
    const field: Field = { key, value, range };
...
type RawDeclaration = RawBlock | RawSum | RawStub | RawEdge;

type Referenceable = RawBlock | RawSum | RawStub;

/** One edge's settled endpoints: the authored name an endpoint resolved to. */
interface Endpoints {
 ...
 * partially published.
 */
export function validateControlBlock(options: ControlValidationOptions): {
  ...
  const text = (
    ...
    if (parsed !== undefined) labelCodePoints += [...circuitTextValue(parsed)].length;
    return parsed;
  };
  ...
  for (const item of items) {
    if (!oneOf(DECLARATION_KINDS, item.kind)) {
      diagnostics.push(
        diag(
          "unknown-declaration",
          `Unknown control declaration "${item.kind}".`,
          item.range,
          sourceName,
          { kind: item.kind },
          vocabularySuggestion(item.kind, DECLARATION_KINDS),
        ),
      );
      continue;
    }
    if (item.kind === "block") {
      ...
      if (name === undefined || tf === undefined) continue;
    ...
    if (item.kind === "sum") {
      ...
      if (name === undefined || signs === undefined) continue;
    ...
    if (item.kind === "edge") {
      ...
      if (absentFrom) {
        diagnostics.push(
          diag("missing-field", "A control edge requires `from:`.", fromField?.range ?? item.range, sourceName, {
            field: "from",
          }),
        );
      }
      if (absentTo) {
        diagnostics.push(
          diag("missing-field", "A control edge requires `to:`.", toField?.range ?? item.range, sourceName, {
            field: "to",
          }),
        );
      }
  ...
  const names = new Map<string, Referenceable>();
  for (const declaration of declared) {
    ...
    if (names.has(declaration.name)) {
      diagnostics.push(
        diag(
          "duplicate-name",
          `Control name "${declaration.name}" is already declared in this Block.`,
          declaration.range,
          sourceName,
  ...
  const endpoints = new Map<RawEdge, Endpoints>();
  const incoming = new Map<string, RawEdge[]>();
  const fanIn = new Map<string, RawEdge[]>();
  const fanInIndex = new Map<RawEdge, number>();
  for (const declaration of declared) {
    if (declaration.kind !== "edge") continue;
    ...
    if (arriving === undefined) incoming.set(to, [declaration]);
    else arriving.push(declaration);
    if (from === to) continue;
  ...
  for (const declaration of declared) {
    if (declaration.kind === "edge") {
      ...
      if (from === undefined) {
        diagnostics.push(
          diag(
            ...
            `Control endpoint "${declaration.fromField.value}" does not name a declared item.`,
            declaration.fromField.range,
            sourceName,
            { name: declaration.fromField.value },
          ),
        );
      }
      if (to === undefined) {
        diagnostics.push(
          diag(
            "unresolved-reference",
            `Control endpoint "${declaration.toField.value}" does not name a declared item.`,
            declaration.toField.range,
            sourceName,
            { name: declaration.toField.value },
          ),
        );
      }
      if (from === undefined || to === undefined) continue;
      ...
      if (index !== undefined && index > 0 && target !== undefined && (target.kind === "block" || target.kind === "output")) {
        diagnostics.push(
          diag(
            "port-fan-in",
            `Control ${target.kind === "block" ? "block" : "output stub"} "${to}" receives more than one in-edge.`,
            declaration.range,
            sourceName,
            { item: to },
          ),
      ...
      if (target !== undefined && target.kind === "input") {
        diagnostics.push(
          diag(
            ...
            `Control edge "to:" cannot feed the input stub "${to}".`,
            declaration.toField.range,
            sourceName,
            { item: to },
          ),
      ...
      if (source !== undefined && source.kind === "output") {
        diagnostics.push(
          diag(
            "invalid-edge-endpoint",
            `Control edge "from:" cannot leave the output stub "${from}".`,
            declaration.fromField.range,
            sourceName,
            { item: from },
          ),
    ...
    if (inputs.length === 0) {
      diagnostics.push(
        diag("sum-no-inputs", `Control sum "${declaration.name}" has no in-edge.`, declaration.range, sourceName, {
    ...
    if (inputs.length !== declaration.signs.length) {
      diagnostics.push(
        diag(
          "sign-count-mismatch",
          `Control sum "${declaration.name}" declares ${declaration.signs.length} signs for ${inputs.length} in-edges.`,
          declaration.range,
          sourceName,
          { signs: declaration.signs.length, inputs: inputs.length },
        ),
      );
    }
  }
  ...
  for (const item of items) {
    ...
    else if (item.kind === "sum") sumCount += 1;
    else if (item.kind === "input" || item.kind === "output") stubCount += 1;
    else if (item.kind === "edge") edgeCount += 1;
  }
  if (items.length > MAX_CONTROL_DECLARATIONS) {
    diagnostics.push(
      limitExceeded("declaration count", items.length, MAX_CONTROL_DECLARATIONS, blockRange, sourceName),
    );
  }
  ...
  if (!failed) {
    ...
    for (const declaration of declared) {
      ...
      for (const port of PORTS[declaration.kind]) {
        ...
        if (declaration.kind === "sum" && port === "in") continue;
        diagnostics.push(
          warn(
            "unconnected-port",
            `Control ${declaration.kind} "${declaration.name}" has no edge on its \`${port}\` port.`,
            declaration.range,
            sourceName,
            { item: declaration.name, port },
          ),
        );
      }
    }
  ...
  const declarations: ControlDeclaration[] = [];
  for (const declaration of declared) {
    if (declaration.kind === "block") {
      const block: ControlBlockItem = {
        kind: "block",
        name: declaration.name,
...
const descriptor = Object.freeze({
  ...
  summary:
    "Control signal-flow diagrams: SISO function blocks, summing junctions, boundary stubs, and labelled signal edges over one Block-local namespace.",
...
export const controlPlugin: AzeBlockPlugin = Object.freeze({ descriptor });
