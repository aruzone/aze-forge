...
export const COMPOSITION_SCHEMA_ID = "azeforge.composition/v1" as const;
 ...
 */
export const NUMBERING_CLASSES: Readonly<
  Record<string, Readonly<{ readonly word: string }>>
> = Object.freeze({
...
export const NUMBERING_CLASS_ORDER: readonly string[] = Object.freeze(
  Object.keys(NUMBERING_CLASSES),
);
 ...
 */
export function referenceGroupDelimiters(
  style: CitationStyle,
): ResolvedReferenceGroup {
 ...
 */
const NUMBERING_WORDS: Readonly<Record<string, string>> = Object.freeze({
  ...Object.fromEntries(
    Object.entries(NUMBERING_CLASSES).map(([kind, entry]) => [kind, entry.word]),
  ),
  bibliography: "Bibliography",
});

/** The closed locator-word shortcut set used by the numeric style. */
const LOCATOR_ABBREVIATIONS: Readonly<Record<string, string>> = Object.freeze({
  page: "p.",
  pages: "pp.",
  chapter: "chap.",
  section: "sec.",
...
const EMPTY_RANGE: SourceRange = Object.freeze({
...
/** The optional header fields the resolver reads off any Block kind. */
interface SharedBlockFields {
  readonly id?: string;
...
function sharedFields(block: AzeBlock): AzeBlock & SharedBlockFields {
  return block as AzeBlock & SharedBlockFields;
}

/** The numbering class of one Block: its statement kind, else its Block kind. */
function numberingClass(block: AzeBlock): string {
  return block.kind === "statement" ? block.statementKind : block.kind;
}

function mapBlockList(
  blocks: readonly AzeBlock[],
  map: (block: AzeBlock) => AzeBlock,
): readonly AzeBlock[] {
  let changed = false;
 ...
 */
function mapContainedBlocks(
  block: AzeBlock,
  map: (child: AzeBlock) => AzeBlock,
): AzeBlock {
  switch (block.kind) {
    case "figure":
    case "callout":
    case "blockquote": {
      const children = block.children as readonly AzeBlock[];
    ...
    case "list": {
      ...
      const items = block.items.map((item) => {
        const mapped = mapBlockList(item.blocks as readonly AzeBlock[], map);
        if (mapped === item.blocks) return item;
        changed = true;
    ...
    case "statement": {
      const text = mapBlockList(block.text as readonly AzeBlock[], map);
      const proof =
        block.proof === undefined
          ? undefined
          : mapBlockList(block.proof as readonly AzeBlock[], map);
    ...
    case "example": {
      const problem = mapBlockList(block.problem as readonly AzeBlock[], map);
      let changed = problem !== block.problem;
      const steps = block.steps.map((step) => {
        const text = mapBlockList(step.text as readonly AzeBlock[], map);
      ...
      const result =
        block.result === undefined
          ? undefined
          : mapBlockList(block.result as readonly AzeBlock[], map);
      if (result !== undefined && result !== block.result) changed = true;
 ...
 * they take part in.
 */
function inlinePlainText(nodes: readonly Inline[]): string {
  let text = "";
...
function circuitRunText(run: unknown): string {
 ...
 */
function captionText(block: AzeBlock): string | undefined {
  ...
  if (typeof title === "string") return title.trim() === "" ? undefined : title;
...
/* ------------------------------------------------------------------ *
 * Declarations: identifiers, numbering and the bibliography
 * ------------------------------------------------------------------ */
...
interface DuplicateDefinition {
  readonly label: string;
  readonly range: SourceRange;
  readonly first: SourceRange;
}

interface DeclarationState {
  readonly counters: Map<string, number>;
  readonly declarations: Map<string, Declaration>;
  readonly occurrences: Map<string, SourceRange[]>;
  readonly invalidIds: { readonly id: string; readonly range: SourceRange }[];
  readonly bibliographies: { readonly block: BibliographyBlock; readonly range: SourceRange }[];
  readonly entries: EntryRecord[];
  readonly definitions: Map<string, DefinitionRecord>;
  readonly definitionsSeen: DefinitionRecord[];
  readonly duplicates: DuplicateDefinition[];
}

function createDeclarationState(): DeclarationState {
  return {
    counters: new Map(),
...
function addOccurrence(
  ...
  if (sites === undefined) state.occurrences.set(identifier, [range]);
  else sites.push(range);
}

function assignNumber(block: AzeBlock, state: DeclarationState): Numbered | undefined {
  if (sharedFields(block).number !== true) return undefined;
  const className = numberingClass(block);
  if (!Object.hasOwn(NUMBERING_WORDS, className)) return undefined;
...
function objectLabel(block: AzeBlock, numbered: Numbered | undefined): string {
  if (numbered !== undefined) return `${numbered.word} ${numbered.number}`;
  return captionText(block) ?? kindWord(numberingClass(block));
}

function withNumberLabel(block: AzeBlock, label: string): AzeBlock {
  switch (block.kind) {
    case "table":
    case "figure":
    case "algorithm":
...
function declareBlock(block: AzeBlock, state: DeclarationState): AzeBlock {
...
function declareBibliography(block: BibliographyBlock, state: DeclarationState): void {
...
function declareFootnoteDefinition(
  block: FootnoteDefinitionBlock,
  state: DeclarationState,
): void {
  const record: DefinitionRecord = { label: block.label, range: block.range };
...
function declarationDiagnostics(state: DeclarationState): Diagnostic[] {
  ...
  for (const { id, range } of state.invalidIds) {
    diagnostics.push(
      createDiagnostic(
        ...
        "error",
        `Block ID "${id}" is not a valid AzeMark ID.`,
        {
          location: { range },
          data: { id },
          suggestion:
  ...
  for (const [identifier, sites] of state.occurrences) {
    ...
    const relatedLocations: RelatedLocation[] = sites.map((range, index) => ({
      range,
      message:
        index === 0
    ...
    diagnostics.push(
      createDiagnostic(
        ...
        "error",
        `Identifier "${identifier}" is used more than once.`,
        {
  ...
  for (const bibliography of state.bibliographies.slice(1)) {
    diagnostics.push(
      createDiagnostic(
        "azeforge.citation#duplicate-bibliography",
        "error",
        "A document may declare at most one bibliography directive.",
        {
          location: { range: bibliography.range },
          data: { directives: state.bibliographies.length },
          relatedLocations:
            firstBibliography === undefined
              ? []
              : [
                  {
  ...
  if (state.entries.length > MAX_BIBLIOGRAPHY_ENTRIES) {
    ...
    diagnostics.push(
      createDiagnostic(
        "azeforge.citation#limit-exceeded",
        "error",
        `A bibliography may hold at most ${MAX_BIBLIOGRAPHY_ENTRIES} entries.`,
        {
  ...
  if (state.definitionsSeen.length > MAX_FOOTNOTE_DEFINITIONS) {
    ...
    diagnostics.push(
      createDiagnostic(
        "azeforge.footnote#limit-exceeded",
        "error",
        `A document may hold at most ${MAX_FOOTNOTE_DEFINITIONS} footnote definitions.`,
        {
  ...
  for (const duplicate of state.duplicates) {
    diagnostics.push(
      createDiagnostic(
        ...
        "error",
        `Footnote "${duplicate.label}" is defined more than once.`,
        {
...
interface MapContext {
  ...
  /** A caption may carry reference and citation tokens but never a marker. */
  readonly inCaption: boolean;
}

type TokenVisitor = (node: Inline, context: MapContext) => Inline;

function isSourceRange(value: unknown): value is SourceRange {
  if (typeof value !== "object" || value === null) return false;
  const record = value as Record<string, unknown>;
  return (
    typeof record["start"] === "object" &&
    record["start"] !== null &&
...
function mapValue(
  ...
  if (kind === "referenceGroup") {
    const group = value as ReferenceGroupInline;
    const mapped = mapValue(record["targets"], visit, local);
    const node: ReferenceGroupInline =
      mapped === record["targets"]
        ? group
        : { ...group, targets: mapped as readonly ReferenceInline[] };
    return visit(node, local);
  }
  if (kind === "reference" || kind === "footnote") {
    return visit(value as Inline, local);
  }

  let changed = false;
  const changedKeys: string[] = [];
  const mappedValues = new Map<string, unknown>();
  ...
  const rebuilt: Record<string, unknown> = { ...record };
  for (const key of changedKeys) rebuilt[key] = mappedValues.get(key);
  return rebuilt;
}

/* ------------------------------------------------------------------ *
 * Pass 2: the scan (forward references, citation and marker order)
 * ------------------------------------------------------------------ */

interface ScanState {
  tokens: number;
  tokenLimitReported: boolean;
  readonly citationOrder: string[];
  readonly cited: Set<string>;
  readonly markerCounts: Map<string, number>;
  readonly markerOrder: string[];
...
function scanVisitor(
  ...
  const report = (
    ...
    state.diagnostics.push(
      createDiagnostic(code, severity, message, { location: { range }, data }),
    );
  };
  ...
  return (node, context) => {
    if (node.kind === "reference") {
      ...
      if (state.tokens > MAX_REFERENCE_TOKENS && !state.tokenLimitReported) {
        ...
        report(
          "azeforge.reference#limit-exceeded",
          "error",
          `A document may hold at most ${MAX_REFERENCE_TOKENS} reference and citation tokens.`,
          node.range ?? context.fallback,
          {
      ...
      const locatorLength =
        node.locator === undefined ? 0 : [...node.locator.value].length;
      if (locatorLength > MAX_LOCATOR_VALUE_LENGTH) {
        report(
          "azeforge.citation#limit-exceeded",
          "error",
          `A locator value may be at most ${MAX_LOCATOR_VALUE_LENGTH} characters.`,
          range,
          {
      ...
      if (declaration === undefined) {
        if (!hasBibliography && node.locator !== undefined) {
          report(
            ...
            "error",
            `Citation "@${node.target}" needs a :::: bibliography directive.`,
            range,
            { target: node.target },
          );
        } else {
          report(
            ...
            "error",
            `Reference "@${node.target}" does not match any identifier in this document.`,
            range,
            { target: node.target },
          );
        }
      ...
      if (node.locator !== undefined && !declaration.citation) {
        report(
          "azeforge.citation#locator-on-reference",
          "error",
          `A locator may only target a bibliography entry, not object "@${node.target}".`,
          range,
          { target: node.target },
        );
      }
    ...
    if (node.kind === "referenceGroup") {
      if (node.targets.length > MAX_REFERENCE_GROUP_TARGETS) {
        report(
          "azeforge.reference#limit-exceeded",
          "error",
          `A reference group may hold at most ${MAX_REFERENCE_GROUP_TARGETS} targets.`,
          node.range ?? context.fallback,
          {
    ...
    if (node.kind !== "footnote") return node;
    if (context.inFootnote) {
      report(
        "azeforge.footnote#nested-marker",
        "error",
        "A footnote marker may not appear inside footnote content.",
        node.range ?? context.fallback,
        { label: node.label },
      );
      return node;
    ...
    if (context.inCaption) {
      report(
        "azeforge.footnote#marker-in-caption",
        "error",
        "A footnote marker may not appear in a caption.",
        node.range ?? context.fallback,
        { label: node.label },
      );
      return node;
    ...
    if (!definitions.has(node.label)) {
      report(
        "azeforge.footnote#missing-definition",
        "error",
        `Footnote "${node.label}" is referenced but never defined.`,
        node.range ?? context.fallback,
        { label: node.label },
      );
      return node;
    ...
    if (count === 1) state.markerOrder.push(node.label);
    return node;
  };
}
...
function authorKey(entry: BibliographyEntry): string {
  const author = entry.authors[0];
  if (author === undefined) return "";
  return author.family ?? author.name;
}

function yearText(entry: BibliographyEntry): string {
  const year = entry.year;
  return year === undefined || year === "unspecified" ? "n.d." : year;
}

function yearValue(entry: BibliographyEntry): number | undefined {
  const year = entry.year;
  if (year === undefined || year === "unspecified") return undefined;
  const parsed = Number.parseInt(year, 10);
  return Number.isFinite(parsed) ? parsed : undefined;
}

function compareYears(left: number | undefined, right: number | undefined): number {
...
function citationGroupKey(entry: BibliographyEntry): string {
  return `${authorKey(entry)}\u0000${yearText(entry)}`;
}

function citationBaseLabel(entry: BibliographyEntry): string {
...
): { readonly labels: ReadonlyMap<string, string>; readonly order: readonly string[] } {
  const records = new Map<string, EntryRecord>();
  for (const record of state.entries) {
    if (!records.has(record.entry.key)) records.set(record.entry.key, record);
  }
  const cited: EntryRecord[] = [];
  for (const key of citationOrder) {
    const record = records.get(key);
    if (record !== undefined) cited.push(record);
  }
...
function locatorSuffix(style: CitationStyle, locator: Locator | undefined): string {
...
interface FillState {
  ...
  readonly citationLabels: ReadonlyMap<string, string>;
  readonly footnoteNumbers: ReadonlyMap<string, number>;
  readonly markerSequence: Map<string, number>;
}

function fillVisitor(state: FillState): TokenVisitor {
  return (node, context) => {
    if (node.kind === "reference") {
      const declaration = state.declarations.get(node.target);
      if (declaration === undefined) return node;
    ...
    if (node.kind !== "footnote") return node;
    if (context.inFootnote) return node;
...
function patchBibliographies(
  blocks: readonly AzeBlock[],
  lists: readonly (readonly BibliographyEntry[])[],
): readonly AzeBlock[] {
  let index = 0;
  const visit = (block: AzeBlock): AzeBlock => {
...
function worksCitedLists(
  state: DeclarationState,
  order: readonly string[],
): readonly (readonly BibliographyEntry[])[] {
  const position = new Map<string, number>();
  order.forEach((key, index) => {
    if (!position.has(key)) position.set(key, index);
  });
  return state.bibliographies.map(({ block }) =>
    block.entries
 ...
 */
export function resolveDocumentComposition(
  ...
  mapValue(
    declared,
    scanVisitor(scan, state.declarations, state.definitions, hasBibliography),
    rootContext,
  );
  ...
  const resolved = mapValue(
    declared,
    fillVisitor(fill),
    rootContext,
  ) as readonly AzeBlock[];

  const definitions = new Map<string, FootnoteDefinitionBlock>();
  ...
  diagnostics.push(
    ...citationDiagnostics(style, state, scan),
    ...footnoteWarnings(state, scan),
  );

  const composition: DocumentComposition = { citationStyle: style, endnotes };
  return {
    document: {
      ...document,
...
function citationDiagnostics(
  ...
  for (const record of state.entries) {
    ...
    if (
      ...
      diagnostics.push(
        createDiagnostic(
          "azeforge.citation#missing-family-name",
          "error",
          `Citation record "${entry.key}" is missing an author family name.`,
          {
    ...
    if (!scan.cited.has(entry.key)) {
      diagnostics.push(
        createDiagnostic(
          "azeforge.citation#uncited-entry",
          "warning",
          `Citation record "${entry.key}" is never cited.`,
          {
            location: { range: entry.range },
            data: { key: entry.key },
...
function footnoteWarnings(state: DeclarationState, scan: ScanState): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  for (const [label, record] of state.definitions) {
    if (scan.markerCounts.has(label)) continue;
    diagnostics.push(
      createDiagnostic(
...
export function renderEndnotesSection(document: AzeDocument): string {
  ...
  const items = endnotes.map((entry) => {
    ...
    for (let marker = 1; marker <= entry.markers; marker += 1) {
      backlinks.push(
        `<a class="aze-endnote-backlink" href="#fnref-${label}-${marker}" role="doc-backlink" aria-label="Back to reference ${marker} of note ${entry.number}">&#8617;</a>`,
      );
    }
    const links =
      backlinks.length === 0
