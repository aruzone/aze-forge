...
import {
  FREE_BODY_BODY_SYNTAX_ID,
  FREE_BODY_BODY_SYNTAX_VERSION,
...
const NAMESPACE = "azeforge.free-body" as const;
const BLANK = /^[ \t]*$/;
const COMMENT = /^[ \t]*\/\/[^\n]*$/;
...
function limitExceeded(
  ...
  return diag(
    "limit-exceeded",
    `Free-body ${subject} ${count} exceeds the limit of ${limit}.`,
    range,
    sourceName,
...
function unregisteredField(
  ...
  return diag(
    "unknown-field",
    `Free-body ${owner} "${spelling}" is not registered.`,
    field.range,
    sourceName,
    {
...
/* ------------------------------------------------------------------ *
 * Exact decimals and angles (the §7 frame rules, borrowed as conventions)
 * ------------------------------------------------------------------ */
 ...
 */
function canonicalNumber(spelling: string): string | undefined {
...
function readDecimal(
  ...
  if (text === undefined || !Number.isFinite(value)) {
    diagnostics.push(
      diag(
        "invalid-coordinate",
        `Free-body value "${field.value}" for "${field.key}" is not a finite exact decimal.`,
        field.range,
        sourceName,
        { data: { field: field.key, value: field.value } },
      ),
...
function readAngle(
  ...
  if (canonical === undefined || !Number.isFinite(Number(canonical))) {
    diagnostics.push(
      diag(
        "unknown-direction",
        `Free-body angle "${field.value}" is not an exact decimal in degrees.`,
        field.range,
        sourceName,
        { data: { field: field.key, value: field.value } },
      ),
...
/* ------------------------------------------------------------------ *
 * Header: registered keys, the canvas group and the scale switch
 * ------------------------------------------------------------------ */

interface FreeBodyField {
  readonly key: string;
  readonly value: string;
  readonly range: SourceRange;
}

interface FreeBodyEntry {
  readonly value: string;
  readonly range: SourceRange;
}
  ...
  return parseCircuitText({
    ...
    invalid: (message) => {
      diagnostics.push(
        diag(
          "invalid-label",
          `Free-body ${owner} is not plain single-line text. ${message}`,
          field.range,
          sourceName,
    ...
    exceeded: (count) => {
      diagnostics.push(
        limitExceeded(field.key, count, MAX_FREE_BODY_LABEL_CHARS, field.range, sourceName),
      );
    },
  });
}
...
function finishBounds(
  ...
  for (const field of fields) {
    if (authored.has(field.key)) {
      diagnostics.push(
        diag(
          "duplicate-field",
          `Free-body bounds field "${field.key}" is declared twice.`,
          field.range,
          sourceName,
          { data: { field: field.key } },
        ),
    ...
    if (canonical === undefined || !Number.isFinite(Number(canonical))) {
      diagnostics.push(
        diag(
          "invalid-coordinate",
          `Free-body bounds field "${field.key}" value "${field.value}" is not a finite exact decimal.`,
          field.range,
          sourceName,
          { data: { field: field.key, value: field.value } },
        ),
  ...
  if (missing.length > 0) {
    diagnostics.push(
      diag(
        "invalid-coordinate",
        `Free-body bounds require min-x, min-y, max-x, max-y (missing: ${missing.join(", ")}).`,
        opener,
        sourceName,
        { data: { field: "bounds", missing: [...missing] } },
      ),
    );
    return { diagnostics };
  }
  ...
  if (!(Number(minX) < Number(maxX) && Number(minY) < Number(maxY))) {
    diagnostics.push(
      diag(
        "invalid-bounds",
        "Free-body bounds must be finite and non-empty (min < max on both axes).",
        opener,
        sourceName,
        { data: { field: "bounds" } },
      ),
...
function parseHeader(
  headerLines: readonly FreeBodyInputLine[],
  sourceName: string | undefined,
): { header: ParsedHeader; diagnostics: Diagnostic[] } {
  const diagnostics: Diagnostic[] = [];
  let id: string | undefined;
  let number: boolean | undefined;
  let title: CircuitText | undefined;
  let description: CircuitText | undefined;
  let scale: string | undefined;
  let width = DEFAULT_FREE_BODY_WIDTH;
  let height = DEFAULT_FREE_BODY_HEIGHT;
  let bounds: GeometryBounds | undefined;
  const seen = new Set<string>();
  let boundsOpener: SourceRange | undefined;
  let boundsFields: FreeBodyField[] | undefined;

  const closeBounds = (): void => {
    if (boundsOpener === undefined || boundsFields === undefined) return;
  ...
  for (const line of headerLines) {
    ...
    if (match === null) {
      diagnostics.push(
        diag(
          "unknown-field",
          "A free-body header is one `key: value` field per line.",
          line.range,
          sourceName,
        ),
      );
    ...
    if (key === "bounds") {
      ...
      if (seen.has("bounds")) {
        diagnostics.push(
          diag("duplicate-field", "Free-body header field \"bounds\" is declared twice.", line.range, sourceName, {
            data: { field: "bounds" },
          }),
        );
        continue;
    ...
    if (!HEADER_FIELDS.includes(key)) {
      diagnostics.push(
        diag(
          "unknown-field",
          `Free-body header field "${key}" is not registered.`,
          line.range,
          sourceName,
          {
    ...
    if (seen.has(key)) {
      diagnostics.push(
        diag(
          "duplicate-field",
          `Free-body header field "${key}" is declared twice.`,
          line.range,
          sourceName,
          { data: { field: key } },
        ),
    ...
    seen.add(key);
    const field: FreeBodyField = { key, value, range: line.range };
    if (key === "scale") {
      ...
      if (canonical === undefined || !(Number(canonical) > 0)) {
        diagnostics.push(
          diag(
            "invalid-scale",
            `Free-body scale "${value}" must be a finite exact decimal greater than 0.`,
            line.range,
            sourceName,
            { data: { field: "scale", value } },
          ),
        );
        continue;
      }
    ...
    if (key === "id") {
      if (value === "") {
        diagnostics.push(
          diag("missing-field", "Free-body `id:` requires a value.", line.range, sourceName, {
            data: { field: "id" },
          }),
        );
        continue;
    ...
    if (key === "number") {
      if (value !== "true" && value !== "false") {
        diagnostics.push(
          diag(
            "unknown-field",
            'Free-body "number:" must be true or false.',
            line.range,
            sourceName,
    ...
    if (key === "width" || key === "height") {
      if (!POSITIVE_INTEGER.test(value)) {
        diagnostics.push(
          diag(
            "unknown-field",
            `Free-body "${key}:" must be a positive integer.`,
            line.range,
            sourceName,
            { data: { field: key, value } },
          ),
      ...
      if (parsed <= 0 || parsed > MAX_FREE_BODY_DIMENSION_PX) {
        diagnostics.push(
          limitExceeded(key, parsed, MAX_FREE_BODY_DIMENSION_PX, line.range, sourceName),
        );
        continue;
      }
...
/* ------------------------------------------------------------------ *
 * Body parsing: `- kind:` items with flat fields and `vertices:` entries
 * ------------------------------------------------------------------ */

interface RawItem {
  readonly kind: string;
  readonly range: SourceRange;
  readonly fields: readonly FreeBodyField[];
  readonly collections: ReadonlyMap<string, readonly FreeBodyEntry[]>;
}

const BODY_SHAPE = "Free-body body entries begin with `- kind:` and use flat `key: value` fields.";

function parseBody(
  bodyLines: readonly FreeBodyInputLine[],
  diagnostics: Diagnostic[],
  sourceName: string | undefined,
): RawItem[] {
  const items: RawItem[] = [];
  let kind: string | undefined;
  let kindRange: SourceRange | undefined;
  let fields: FreeBodyField[] = [];
  let collections = new Map<string, FreeBodyEntry[]>();
  let openCollection: string | undefined;

  const flush = (): void => {
    if (kind === undefined || kindRange === undefined) return;
  ...
  for (const line of bodyLines) {
    ...
    if (field === null || kind === undefined) {
      diagnostics.push(
        diag(
          kind === undefined ? "unknown-declaration" : "unknown-field",
          kind === undefined
            ? BODY_SHAPE
            : `Free-body line "${line.text.trim()}" is not a "key: value" field.`,
          line.range,
          sourceName,
        ),
      );
...
/** One attachment value before name resolution: a point name or a coordinate pair. */
type ParsedAttachment =
  | { readonly kind: "point"; readonly name: string; readonly range: SourceRange }
  | { readonly kind: "coordinates"; readonly x: string; readonly y: string; readonly range: SourceRange };

/**
 * One direction form before name resolution: an angle is already complete, a
 * relative form still carries the field that names its line.
 */
type ParsedDirection =
  | { readonly kind: "angle"; readonly degrees: string }
  | { readonly kind: "parallel-to"; readonly field: FreeBodyField }
  | { readonly kind: "perpendicular-to"; readonly field: FreeBodyField };

/** Which namespace a referenceable declaration lives in. */
type ReferenceKind = "body" | "point" | "line";
...
interface RawBase {
  readonly range: SourceRange;
  /** Authored ordinal within this kind, for positional labels. */
  readonly index: number;
}

interface RawBlock extends RawBase {
  readonly kind: "block";
  readonly name: string;
  readonly x: string;
  readonly y: string;
...
interface RawCircle extends RawBase {
...
interface RawPolygon extends RawBase {
  readonly kind: "polygon";
  readonly name: string;
  readonly visible: boolean;
  readonly vertices: readonly FreeBodyEntry[];
  resolvedVertices: readonly string[];
}

interface RawParticle extends RawBase {
...
interface RawPoint extends RawBase {
  readonly kind: "point";
  readonly name: string;
  readonly x: string;
  readonly y: string;
  readonly label: CircuitText | undefined;
  readonly visible: boolean;
}

interface RawLine extends RawBase {
  readonly kind: "line";
  readonly name: string;
  readonly visible: boolean;
  readonly style: "solid" | "dashed";
  readonly from: FreeBodyField;
  readonly to: FreeBodyField;
  resolvedFrom: string | undefined;
  resolvedTo: string | undefined;
}

interface RawForce extends RawBase {
  readonly kind: "force";
  readonly at: ParsedAttachment;
  readonly direction: ParsedDirection | undefined;
  readonly magnitude: string | undefined;
  readonly length: string | undefined;
  readonly label: CircuitText | undefined;
  resolvedAt: FreeBodyAttachment | undefined;
  resolvedDirection: FreeBodyDirection | undefined;
}

interface RawMoment extends RawBase {
...
interface RawAxes extends RawBase {
  readonly kind: "axes";
  readonly at: ParsedAttachment;
  readonly angle: string;
...
interface RawAngleMark extends RawBase {
  readonly kind: "angle-mark";
  readonly first: FreeBodyField;
  readonly vertex: FreeBodyField;
  readonly third: FreeBodyField;
  readonly label: CircuitText | undefined;
  resolvedFirst: string | undefined;
  resolvedVertex: string | undefined;
  resolvedThird: string | undefined;
}

interface RawDimension extends RawBase {
  readonly kind: "dimension";
  readonly from: ParsedAttachment;
  readonly to: ParsedAttachment;
  readonly label: CircuitText;
  resolvedFrom: FreeBodyAttachment | undefined;
  resolvedTo: FreeBodyAttachment | undefined;
}
...
const REFERENCEABLE: Readonly<Record<string, ReferenceKind>> = Object.freeze({
  block: "body",
  circle: "body",
  polygon: "body",
  particle: "body",
...
function fieldMap(fields: readonly FreeBodyField[]): Map<string, FreeBodyField> {
  const map = new Map<string, FreeBodyField>();
  for (const field of fields) {
...
function requireField(
  kind: string,
  fields: Map<string, FreeBodyField>,
  range: SourceRange,
  key: string,
  sourceName: string | undefined,
  diagnostics: Diagnostic[],
): FreeBodyField | undefined {
  const field = fields.get(key);
  if (field === undefined || field.value === "") {
    diagnostics.push(
      diag("missing-field", `Free-body ${kind} requires a "${key}:" field.`, range, sourceName, {
        data: { field: key },
      }),
    );
    return undefined;
...
function optionalField(fields: Map<string, FreeBodyField>, key: string): FreeBodyField | undefined {
  const field = fields.get(key);
  if (field === undefined || field.value === "") return undefined;
  return field;
}
...
function readVisible(
  ...
  if (field.value !== "true" && field.value !== "false") {
    diagnostics.push(
      diag(
        "unknown-field",
        `Free-body ${kind} "visible:" must be true or false.`,
        field.range,
        sourceName,
        { data: { field: "visible", value: field.value } },
      ),
    );
    return fallback;
  }
...
function readAttachment(
  ...
  if (match !== null) {
    ...
    diagnostics.push(
      diag(
        "invalid-coordinate",
        `Free-body "${field.key}:" value "${raw}" is not a bounded \`(x, y)\` coordinate pair.`,
        field.range,
        sourceName,
        { data: { field: field.key, value: raw } },
      ),
    );
    return undefined;
  }
...
function resolvePoint(
  name: string,
  range: SourceRange,
  names: ReadonlyMap<string, ReferenceTarget>,
  sourceName: string | undefined,
  diagnostics: Diagnostic[],
): string | undefined {
  ...
  if (target === undefined) {
    diagnostics.push(
      diag(
        ...
        `Free-body reference "${name}" does not name a declared point.`,
        range,
        sourceName,
        { data: { name } },
      ),
  ...
  if (target.reference !== "point") {
    diagnostics.push(
      diag(
        ...
        `Free-body reference "${name}" names a ${target.reference}; a point is required here.`,
        range,
        sourceName,
        { data: { name, expected: "point" } },
      ),
    );
    return undefined;
  }
...
function resolveLine(
  ...
  if (target === undefined) {
    diagnostics.push(
      diag(
        "unresolved-reference",
        `Free-body reference "${name}" does not name a declared line.`,
        range,
        sourceName,
        { data: { name } },
      ),
  ...
  if (target.reference !== "line") {
    diagnostics.push(
      diag(
        "invalid-reference-target",
        `Free-body direction target "${name}" names a ${target.reference}; a line is required.`,
        range,
        sourceName,
        { data: { name, expected: "line" } },
      ),
    );
    return undefined;
  }
...
/** What a per-kind reader needs beyond the declaration's own authored fields. */
interface DeclarationContext {
  readonly raw: RawItem;
  readonly fields: Map<string, FreeBodyField>;
...
function readScaleSwitch(
  context: DeclarationContext,
  subject: string,
): { magnitude: string | undefined; length: string | undefined } {
  ...
  if (length !== undefined && !(length.value > 0)) {
    diagnostics.push(
      diag(
        "invalid-coordinate",
        `Free-body schematic length "${lengthField?.value ?? ""}" must be an exact decimal greater than 0.`,
        lengthField?.range ?? context.raw.range,
        sourceName,
        { data: { field: "length", value: lengthField?.value ?? "" } },
      ),
    );
  }
  const conflict = (
      ...
      diag(
        ...
        reason === "both"
          ...
          : reason === "length-with-scale"
            ...
            : `Free-body ${subject} has no \`scale:\`, so it is schematic: author \`length:\`, not \`magnitude:\`.`,
        field?.range ?? context.raw.range,
        sourceName,
        { data: { subject, reason } },
      ),
    );
  ...
  if (magnitudeField !== undefined && lengthField !== undefined) {
    ...
    if (lengthField !== undefined) conflict(subject, "length-with-scale", lengthField);
    else if (magnitudeField === undefined) {
      diagnostics.push(
        diag(
          "missing-field",
          `Free-body ${subject} is under \`scale:\` and requires a "magnitude:" field.`,
          context.raw.range,
          sourceName,
    ...
    diagnostics.push(
      diag(
        "missing-field",
        `Free-body ${subject} is schematic and requires a positive "length:" field.`,
        context.raw.range,
        sourceName,
        { data: { field: "length" } },
      ),
...
function readDirection(
  context: DeclarationContext,
  subject: string,
): ParsedDirection | undefined {
  const { fields, diagnostics, sourceName } = context;
  const authored = DIRECTION_FIELDS.map((key) => fields.get(key)).filter(
    (field): field is FreeBodyField => field !== undefined,
  );
  if (authored.length === 0) {
    diagnostics.push(
      diag(
        "missing-field",
        `Free-body ${subject} requires exactly one direction form.`,
        context.raw.range,
        sourceName,
        { data: { fields: [...DIRECTION_FIELDS] } },
      ),
    );
    return undefined;
  }
  if (authored.length > 1) {
    diagnostics.push(
      diag(
        "conflicting-fields",
        `Free-body ${subject} authors more than one direction form; exactly one is permitted.`,
        authored[1]?.range ?? context.raw.range,
        sourceName,
        { data: { fields: authored.map((field) => field.key) } },
      ),
    );
    return undefined;
  }
  ...
  if (field.key === "angle") {
    if (field.value === "") {
      diagnostics.push(
        diag("missing-field", `Free-body ${subject} requires a value for "angle:".`, field.range, sourceName, {
          data: { field: "angle" },
        }),
      );
      return undefined;
  ...
  if (field.value === "") {
    diagnostics.push(
      diag(
        "missing-field",
        `Free-body ${subject} requires a value for "${field.key}:".`,
        field.range,
        sourceName,
        { data: { field: field.key } },
      ),
...
function validateDeclaration(
  raw: RawItem,
  context: Omit<DeclarationContext, "raw" | "fields">,
  index: number,
  ...
  const state: DeclarationContext = { ...context, raw, fields };
  ...
  for (const field of raw.fields) {
    if (seenKeys.has(field.key)) {
      diagnostics.push(
        diag(
          "duplicate-field",
          `Free-body ${raw.kind} field "${field.key}" is declared twice.`,
          field.range,
          sourceName,
          { data: { field: field.key } },
        ),
  ...
  const readLabel = (owner: string): CircuitText | undefined =>
    labelField === undefined ? undefined : readText(labelField, owner, sourceName, diagnostics);
  ...
  const readOffset = (key: string): DecimalValue | undefined => {
  ...
  const readAngleField = (key: string, fallback: string): string | undefined => {
  ...
  switch (raw.kind) {
    ...
    case "polygon": {
      ...
      if (entries === undefined || entries.length === 0) {
        diagnostics.push(
          diag(
            "missing-field",
            'Free-body polygon requires a "vertices:" collection of point references.',
            raw.range,
            sourceName,
            { data: { field: "vertices", count: 0 } },
          ),
        );
        return undefined;
      }
      ...
      if (entries.length < 3) {
        diagnostics.push(
          diag(
            "missing-field",
            `Free-body polygon requires at least 3 vertex references and authors ${entries.length}.`,
            raw.range,
            sourceName,
            { data: { field: "vertices", count: entries.length } },
          ),
        );
        return undefined;
      }
    ...
    case "line": {
      ...
      if (styleField !== undefined) {
        if (!STYLES.includes(styleField.value as "solid" | "dashed")) {
          diagnostics.push(
            unregisteredField("line style", styleField.value, styleField, STYLES, sourceName),
          );
        } else {
          style = styleField.value as "solid" | "dashed";
        }
      }
    ...
    case "force": {
      ...
      if (at === undefined || direction === undefined) return undefined;
      return {
        kind: "force",
        range: raw.range,
        index,
    ...
    case "moment": {
      const atField = requireField(raw.kind, fields, raw.range, "at", sourceName, diagnostics);
      const at = atField === undefined ? undefined : readAttachment(atField, sourceName, diagnostics);
      const directionField = requireField(
        raw.kind,
        fields,
        raw.range,
        "direction",
      ...
      if (directionField !== undefined) {
        if (directionField.value === "cw" || directionField.value === "ccw") {
          ...
          diagnostics.push(
            diag(
              "unknown-direction",
              `Free-body moment direction "${directionField.value}" is not registered.`,
              directionField.range,
              sourceName,
              {
    ...
    case "axes": {
      ...
      if (at === undefined || angle === undefined) return undefined;
    ...
    case "angle-mark": {
      ...
      if (first === undefined || vertex === undefined || third === undefined) return undefined;
      return {
        kind: "angle-mark",
        range: raw.range,
        index,
    ...
    case "dimension": {
      ...
      const labelFieldRequired = requireField(
        raw.kind,
        fields,
        raw.range,
        "label",
      ...
      const label =
        labelFieldRequired === undefined
          ? undefined
          : readText(labelFieldRequired, "dimension `label:`", sourceName, diagnostics);
      if (from === undefined || to === undefined || label === undefined) return undefined;
...
function readAxisLabel(
  ...
  const field = optionalField(fields, key);
  if (field === undefined) return [{ kind: "text", value: fallback }];
...
/* ------------------------------------------------------------------ *
 * Resolution, ceilings and warnings
 * ------------------------------------------------------------------ */
...
function attachmentPosition(
  attachment: FreeBodyAttachment | undefined,
  points: ReadonlyMap<string, Point2>,
): Point2 | undefined {
...
function extents(
  ...
  switch (declaration.kind) {
    ...
    case "dimension": {
      ...
      return [from, to].filter((point): point is Point2 => point !== undefined);
    }
  }
}
 ...
 */
function declarationLabel(declaration: RawDeclaration): string {
...
export function validateFreeBodyBlock(options: {
...
}): { readonly block?: FreeBodyBlock; readonly diagnostics: readonly Diagnostic[] } {
  ...
  if (items.length === 0) {
    diagnostics.push(
      diag("empty", "A free-body Block requires at least one declaration.", blockRange, sourceName),
    );
    return { diagnostics };
  }
  if (items.length > MAX_FREE_BODY_DECLARATIONS) {
    diagnostics.push(
      limitExceeded("declarations", items.length, MAX_FREE_BODY_DECLARATIONS, blockRange, sourceName),
    );
  }
  ...
  const names = new Map<string, ReferenceTarget>();
  ...
  for (const raw of items) {
    if (!DECLARATION_KINDS.includes(raw.kind)) {
      diagnostics.push(
        diag(
          "unknown-declaration",
          `Free-body declaration kind "${raw.kind}" is not registered.`,
          raw.range,
          sourceName,
          {
            data: { kind: raw.kind },
    ...
    const declaration = validateDeclaration(
      raw,
      { scalePresent: header.scale !== undefined, sourceName, diagnostics },
      index,
    );
    ...
    if (name !== undefined) {
      if (names.has(name)) {
        diagnostics.push(
          diag(
            "duplicate-name",
            `Free-body name "${name}" is already declared in this Block.`,
            declaration.range,
            sourceName,
            { data: { name } },
          ),
        ...
        if (reference !== undefined) names.set(name, { name, reference });
  ...
  for (const declaration of declarations) {
    switch (declaration.kind) {
      ...
      case "force": {
        ...
        let resolved: FreeBodyDirection | undefined;
        ...
        if (at.kind === "point") referenced.add(at.name);
        break;
      }
      case "moment":
      case "axes": {
      ...
      case "dimension": {
        ...
        for (const attachment of [from, to]) {
          if (attachment.kind === "point") referenced.add(attachment.name);
        }
        break;
      }
      default:
    ...
    for (const declaration of declarations) {
      ...
      if (name !== undefined && invisible && !referenced.has(name)) {
        diagnostics.push(
          diag(
            "unused-declaration",
            `Free-body ${declaration.kind} "${name}" is invisible and referenced by nothing.`,
            declaration.range,
            sourceName,
            { severity: "warning", data: { name } },
          ),
        );
      }
      ...
      const escaped = extents(declaration, points).some(
        (point) =>
      ...
      if (escaped) {
        ...
        diagnostics.push(
          diag(
            "out-of-bounds",
            `Free-body ${label} lies outside the authored bounds.`,
            declaration.range,
            sourceName,
            { severity: "warning", data: { names: [label] } },
          ),
        );
      }
    }
...
function publishDeclaration(declaration: RawDeclaration): FreeBodyDeclaration | undefined {
  switch (declaration.kind) {
    case "block": {
      const body: FreeBodyBody = {
        kind: "block",
    ...
    case "line": {
      if (declaration.resolvedFrom === undefined || declaration.resolvedTo === undefined) return undefined;
      const line: FreeBodyLine = {
        kind: "line",
        name: declaration.name,
    ...
    case "moment": {
      if (declaration.resolvedAt === undefined) return undefined;
      const moment: FreeBodyMoment = {
        kind: "moment",
        at: declaration.resolvedAt,
    ...
    case "dimension": {
      if (declaration.resolvedFrom === undefined || declaration.resolvedTo === undefined) return undefined;
      const dimension: FreeBodyDimension = {
        kind: "dimension",
        from: declaration.resolvedFrom,
        to: declaration.resolvedTo,
...
function publishBlock(
  ...
  const published: FreeBodyDeclaration[] = [];
  for (const declaration of declarations) {
    const item = publishDeclaration(declaration);
    if (item !== undefined) published.push(item);
  }
  return {
    kind: "free-body",
...
const descriptor = Object.freeze({
  ...
  summary:
    "Native two-dimensional free-body diagrams: authored bodies, points and lines carrying force vectors, moments, axes, angle marks and dimensions over an exact-decimal frame, with an explicit `scale:` switching forces from schematic length to magnitude.",
...
export const freeBodyPlugin: AzeBlockPlugin = Object.freeze({ descriptor });
