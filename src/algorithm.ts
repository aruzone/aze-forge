import {
  ...
  ALGORITHM_BODY_SYNTAX_ID,
  ALGORITHM_BODY_SYNTAX_VERSION,
...
import { rangeFromLineSlice, type SourceLine } from "./source-map.js";
import type {
  AzeBlockPlugin,
  AzeBlockRenderer,
  AlgorithmBlock,
...
export const ALGORITHM_HTML_BLOCK_RENDERER_ID =
  "azeforge.algorithm.html/v1" as const;
export const ALGORITHM_HTML_BLOCK_RENDERER_VERSION = "1.0.0" as const;
...
const EXPRESSION_HINT =
  "Pseudocode expressions use identifiers, integer or decimal literals, " +
...
export const algorithmPlugin: AzeBlockPlugin = Object.freeze({
...
export function parseAlgorithmHeader(
...
interface DeclarationField {
  ...
  /** Index in `line.text` of the value's first character. */
  readonly valueStart: number;
  readonly range: SourceRange;
  readonly collection?: readonly DeclarationEntry[];
  readonly record?: readonly DeclarationField[];
}

interface DeclarationEntry {
  readonly line: SourceLine;
  readonly range: SourceRange;
  /** The bare value of a `- value` item; the item line's text for a record. */
  readonly value: string;
  readonly valueStart: number;
  readonly fields: readonly DeclarationField[];
}
...
interface ParseContext {
  ...
  statementsReported: boolean;
  parametersReported: boolean;
  statementCount: number;
  inlineFailed: boolean;
}

function lineRange(line: SourceLine): SourceRange {
  return rangeFromLineSlice(line, 0, line.text.length);
}

/** A sub-range inside `line`, clamped to the line's own extents. */
function subRange(line: SourceLine, start: number, length: number): SourceRange {
...
function baselineIndent(lines: readonly SourceLine[]): number {
  for (const line of lines) {
    if (line.text.trim() === "" || STRUCTURAL_COMMENT.test(line.text)) continue;
    return leadingIndent(line.text) ?? 0;
  }
  return 0;
}

function indentationFault(ctx: ParseContext, line: SourceLine): void {
...
function expectedDeclaration(
  ctx: ParseContext,
  line: SourceLine,
  message: string,
): void {
  ctx.diagnostics.push(
    algorithmDiagnostic("unknown-field", message, lineRange(line), ctx.sourceName, {
      data: { field: declarationKey(line.text) },
    }),
  );
}
...
function readField(
  ...
  indent: number,
  depth: number,
): { readonly field: DeclarationField; readonly next: number } | undefined {
  ...
  let valueStart =
    lineIndent + match[0].length - (match[2] ?? "").length;
...
function readFields(
  ...
  indent: number,
  depth: number,
): { readonly fields: readonly DeclarationField[]; readonly next: number } {
  if (depth > MAX_DECLARATION_FRAMES) return { fields: [], next: start };
...
function readEntries(
  ...
  indent: number,
  depth: number,
): { readonly entries: readonly DeclarationEntry[]; readonly next: number } {
  if (depth > MAX_DECLARATION_FRAMES) return { entries: [], next: start };
...
type ExpressionMode = "expression" | "condition";

type TokenKind = "ident" | "number" | "operator" | "punct" | "equals";
...
interface ExpressionFault {
  readonly code: "unknown-expression-token" | "assignment-in-condition";
...
interface LexResult {
  ...
  readonly fault?: ExpressionFault;
}
 ...
 */
function lexExpression(text: string): LexResult {
  const tokens: Token[] = [];
  let index = 0;
  while (index < text.length) {
    ...
    if (isIdentifierStart(character)) {
      ...
      const keyword = (KEYWORD_OPERATORS as readonly string[]).includes(word);
      tokens.push({
    ...
    if (isDigit(character)) {
      let end = index + 1;
      while (end < text.length && isDigit(text.charAt(end))) end += 1;
    ...
    const symbol = (SYMBOL_OPERATORS as readonly string[]).find((operator) =>
...
/**
 * The parsed shape of one pseudocode expression. Nothing here is evaluated:
 * the tree exists to validate structure and to re-print a canonical,
 * reflow-invariant spelling.
 */
type ExpressionNode =
  | { readonly kind: "ident"; readonly name: string }
  | { readonly kind: "number"; readonly spelling: string }
  | { readonly kind: "unary"; readonly op: "not" | "-"; readonly operand: ExpressionNode }
  | {
  ...
  | { readonly kind: "index"; readonly base: ExpressionNode; readonly index: ExpressionNode }
  | { readonly kind: "apply"; readonly name: string; readonly args: readonly ExpressionNode[] }
  | { readonly kind: "group"; readonly node: ExpressionNode };
...
class ExpressionSyntaxError extends Error {
  readonly fault: ExpressionFault;
  constructor(fault: ExpressionFault) {
    super(fault.message);
...
function faultFor(token: Token, mode: ExpressionMode): ExpressionFault {
  if (token.kind === "equals" && mode === "condition") {
    return {
      code: "assignment-in-condition",
      message:
        "A condition must compare; a bare `=` assigns. Equality in conditions is `==`.",
...
class PseudocodeParser {
  ...
  constructor(
    ...
    private readonly mode: ExpressionMode,
  ...
  parse(): ExpressionNode {
    ...
    if (token !== undefined) this.fail(token);
    return node;
  }
  ...
  private atOperator(text: string): boolean {
    const token = this.peek();
    return (
      token !== undefined && token.kind === "operator" && token.text === text
    );
  }
  ...
  private atComparison(): boolean {
    ...
    return (
      token !== undefined &&
      token.kind === "operator" &&
      (COMPARISON_OPERATORS as readonly string[]).includes(token.text)
    );
  }

  private advance(): Token {
    const token = this.tokens[this.position];
    if (token === undefined) this.failEnd();
    this.position += 1;
    return token;
  }
  ...
  private expectPunct(text: string): void {
    const token = this.peek();
    if (token === undefined) this.failEnd();
    if (token.kind !== "punct" || token.text !== text) this.fail(token);
    this.advance();
  }
  ...
  private parsePrimary(): ExpressionNode {
    ...
    if (token.kind === "ident") {
      this.advance();
      if (!this.atPunct("(")) return { kind: "ident", name: token.text };
      this.advance();
...
function parseTokenExpression(
  ...
  try {
    ...
    if (error instanceof ExpressionSyntaxError) return { fault: error.fault };
    throw error;
  }
}

function parsePseudocodeExpression(
  text: string,
  mode: ExpressionMode,
): ParsedExpression {
  const lexed = lexExpression(text);
  if (lexed.fault !== undefined) return { fault: lexed.fault };
  return parseTokenExpression(lexed.tokens, mode, text.length);
}

function reportExpressionFault(
  ctx: ParseContext,
  ...
  ctx.diagnostics.push(
    algorithmDiagnostic(
      fault.code,
      fault.message,
      subRange(line, base + fault.start, fault.length),
      ctx.sourceName,
...
function expressionSlice(
  ...
  from: number,
  to: number,
): { readonly text: string; readonly base: number } {
 ...
 */
function withinExpressionCeiling(
  ctx: ParseContext,
  field: DeclarationField,
  slice: { readonly text: string; readonly base: number },
): boolean {
  if (slice.text.length <= MAX_PSEUDOCODE_EXPRESSION_LENGTH) return true;
  ctx.diagnostics.push(
    algorithmDiagnostic(
      "limit-exceeded",
      `A pseudocode expression may be at most ${MAX_PSEUDOCODE_EXPRESSION_LENGTH} characters.`,
      subRange(field.line, slice.base, slice.text.length),
      ctx.sourceName,
      {
        data: {
...
function expressionSpelling(
  ...
  if (!withinExpressionCeiling(ctx, field, { text, base })) return undefined;
...
function conditionSpelling(
  ...
  if (field.value === "") {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        `A \`${keyword}:\` statement requires a condition.`,
        field.range,
        ctx.sourceName,
        {
...
/* ------------------------------------------------------------------ *
 * Statements (contract: issue #66 §5)
 * ------------------------------------------------------------------ */
...
function assignmentTarget(
  ...
  const fail = (message: string): undefined => {
    ctx.diagnostics.push(
      algorithmDiagnostic("invalid-assign-target", message, range, ctx.sourceName, {
        suggestion:
          "An assignment target is a name or one indexing level, such as `A[i]`.",
      }),
    );
    return undefined;
  };
  const first = tokens[0];
  if (first === undefined) return fail("An assignment target must name a variable.");
  if (first.kind !== "ident") {
    return fail(`\`${first.text}\` is not an assignable name.`);
  }
  if (tokens.length === 1) return { name: first.text };
  let depth = 0;
  let close = -1;
  for (let index = 1; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (token === undefined || token.kind !== "punct") continue;
  ...
  if (!withinExpressionCeiling(ctx, field, indexSlice)) return undefined;
...
function rejectExtraFields(
  ctx: ParseContext,
  fields: readonly DeclarationField[],
  from: number,
): void {
  for (let index = from; index < fields.length; index += 1) {
    ...
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "unknown-field",
        `An algorithm statement has no \`${field.key}:\` field.`,
        field.range,
        ctx.sourceName,
        {
...
function branchStatements(
  ...
  field: DeclarationField,
  depth: number,
): readonly AlgorithmStatement[] {
  if (field.record !== undefined) {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "unknown-field",
        `\`${field.key}:\` holds an ordered collection of statements.`,
        field.range,
        ctx.sourceName,
        {
          data: { field: field.key },
  ...
  if (entries.length === 0) {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        `\`${field.key}:\` requires at least one statement.`,
        field.range,
        ctx.sourceName,
        { data: { field: field.key }, suggestion: STATEMENT_SUGGESTION },
      ),
    );
    return [];
  }
...
function parseStatements(
  ...
  if (depth > MAX_ALGORITHM_NESTING_DEPTH) {
    if (!ctx.depthReported) {
      ...
      ctx.diagnostics.push(
        algorithmDiagnostic(
          "limit-exceeded",
          `An algorithm nests at most ${MAX_ALGORITHM_NESTING_DEPTH} statement levels.`,
          first === undefined ? ctx.blockRange : first.range,
          ctx.sourceName,
          {
            data: {
  ...
  const statements: AlgorithmStatement[] = [];
    ...
    if (ctx.statementCount >= MAX_ALGORITHM_STATEMENTS) {
      if (!ctx.statementsReported) {
        ...
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "limit-exceeded",
            `An algorithm holds at most ${MAX_ALGORITHM_STATEMENTS} statements.`,
            entry.range,
            ctx.sourceName,
            {
              data: {
  ...
  if (head === undefined) {
    if (entry.value === "") {
      ctx.diagnostics.push(
        algorithmDiagnostic(
          "missing-body",
          "An algorithm statement requires one of the six statement forms.",
          entry.range,
          ctx.sourceName,
      ...
      ctx.diagnostics.push(
        algorithmDiagnostic(
          "unknown-statement",
          `\`${entry.value}\` is not an algorithm statement.`,
          entry.range,
          ctx.sourceName,
          {
  ...
  switch (head.key) {
    ...
    default:
      ctx.diagnostics.push(
        algorithmDiagnostic(
          "unknown-statement",
          `\`${head.key}\` is not an algorithm statement.`,
          entry.range,
          ctx.sourceName,
          {
...
function parseAssignStatement(
  ...
  const missing = (): undefined => {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "An `assign:` statement requires `target = expression`.",
        head.range,
        ctx.sourceName,
        { suggestion: "Write `assign: total = total + 1`." },
      ),
    );
    return undefined;
  };
  if (head.value === "") return missing();
  ...
  if (equalsToken === undefined) return missing();
  const target = assignmentTarget(
    ctx,
    head,
    lexed.tokens.slice(0, equals),
    equalsToken.start,
  );
  ...
  if (expressionText === "") {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "An `assign:` statement requires an expression after `=`.",
        subRange(head.line, head.valueStart + equalsToken.start, equalsToken.length),
        ctx.sourceName,
        { suggestion: "Write `assign: total = total + 1`." },
      ),
    );
  ...
  const expression = expressionSpelling(
    ctx,
    head,
    expressionText,
    expressionBase,
...
function parseIfStatement(
  ...
  if (thenField === undefined || thenField.key !== "then") {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "An `if:` statement requires `then:`.",
        head.range,
        ctx.sourceName,
        {
          data: { statement: "if", field: "then" },
          suggestion:
            "Add a `then:` collection holding the statements taken when the condition holds.",
        },
      ),
    );
    return undefined;
  ...
  index += 1;
  const elseIf: AlgorithmIfBranch[] = [];
  while (true) {
    const branchField = fields[index];
    if (branchField === undefined || branchField.key !== "else-if") break;
    index += 1;
    ...
    if (branchThen === undefined || branchThen.key !== "then") {
      ctx.diagnostics.push(
        algorithmDiagnostic(
          "missing-body",
          "An `else-if:` branch requires `then:`.",
          branchField.range,
          ctx.sourceName,
          {
            data: { statement: "if", field: "then" },
            suggestion:
              "Add a `then:` collection holding the statements taken when the branch condition holds.",
          },
        ),
      );
      break;
  ...
  let elseStatements: readonly AlgorithmStatement[] | undefined;
...
function parseWhileStatement(
  ...
  if (doField === undefined || doField.key !== "do") {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "A `while:` statement requires `do:`.",
        head.range,
        ctx.sourceName,
        {
          data: { statement: "while", field: "do" },
          suggestion:
            "Add a `do:` collection holding the statements the loop repeats.",
        },
      ),
    );
    return undefined;
...
function parseForStatement(
  ...
  const headerFault = (message: string): undefined => {
    ctx.diagnostics.push(
      algorithmDiagnostic("missing-for-header", message, head.range, ctx.sourceName, {
        suggestion:
          "Write `for: i = 0 to n`, `for: i = n downto 0` or add `by <expression>`.",
      }),
    );
    return undefined;
  };
  if (head.value === "") {
    return headerFault(
      "A `for:` statement requires `var = start to|downto end`.",
    );
  }
  const lexed = lexExpression(head.value);
  if (lexed.fault !== undefined) {
  ...
  const directionToken =
    directionIndex < 0 ? undefined : tokens[directionIndex];
  if (directionToken === undefined) {
    return headerFault(
      "A `for:` statement requires `to` or `downto` between its bounds.",
    );
  }
  let byIndex = -1;
  ...
  const toTokens = tokens.slice(
    directionIndex + 1,
    byIndex < 0 ? tokens.length : byIndex,
  );
  if (fromTokens.length === 0 || toTokens.length === 0) {
    return headerFault(
      "A `for:` statement requires an expression for each bound.",
    );
  }
  ...
  const toSlice = expressionSlice(
    head,
    tokens[directionIndex + 1]?.start ?? fromEnd,
    toEnd,
  );
  if (
    !withinExpressionCeiling(ctx, head, fromSlice) ||
  ...
  if (byIndex >= 0 && byToken !== undefined) {
    ...
    const bySlice = expressionSlice(
      head,
      byToken.start + byToken.length,
      head.value.length,
    );
    if (!withinExpressionCeiling(ctx, head, bySlice)) return undefined;
    const parsedBy = parseTokenExpression(
      byTokens,
      "expression",
      head.value.length,
    );
...
function parseReturnStatement(
  ...
  if ((head.collection ?? []).length > 0 || head.record !== undefined) {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "unknown-field",
        "`return:` takes an optional expression, not a nested structure.",
        head.range,
        ctx.sourceName,
        {
          data: { field: "return" },
  ...
  if (head.value === "") return { kind: "return", range: entry.range };
  const expression = expressionSpelling(
    ctx,
    head,
    head.value,
...
function parseTextStatement(
  ...
  if (head.value === "") {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "A `text:` statement requires an authored prose line.",
        head.range,
        ctx.sourceName,
        { suggestion: "Write `text: invariant: A is sorted ascending`." },
      ),
    );
    return undefined;
  }
...
export function parseAlgorithmBody(args: {
  ...
  let steps: readonly AlgorithmStatement[] | undefined;
  ...
  for (const field of fields) {
    if (seen.has(field.key)) {
      ctx.diagnostics.push(
        algorithmDiagnostic(
          "unknown-field",
          `Algorithm field "${field.key}" is declared twice.`,
          field.range,
          ctx.sourceName,
          {
            data: { field: field.key },
            suggestion:
              "An algorithm body declares `procedure:`, `parameters:` and `steps:` once each.",
          },
        ),
      );
      continue;
    ...
    if (field.key === "procedure") {
      if (field.record !== undefined || (field.collection ?? []).length > 0) {
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "unknown-field",
            "`procedure:` takes one plain name.",
            field.range,
            ctx.sourceName,
            {
      ...
      if (field.value === "") {
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "missing-body",
            "An algorithm requires a procedure name.",
            field.range,
            ctx.sourceName,
            { suggestion: "Write `procedure: BinarySearch`." },
          ),
      ...
      if (!IDENTIFIER.test(field.value)) {
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "unknown-field",
            `Procedure name "${field.value}" must start with a letter or underscore and continue with letters, digits or underscores.`,
            field.range,
            ctx.sourceName,
            { data: { field: "procedure", value: field.value } },
          ),
        );
        continue;
      }
    ...
    if (field.key === "parameters") {
      if (field.record !== undefined) {
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "unknown-field",
            "`parameters:` holds an ordered collection of names.",
            field.range,
            ctx.sourceName,
            {
      ...
      for (const entry of field.collection ?? []) {
        if (entry.fields.length > 0 || entry.value === "") {
          ctx.diagnostics.push(
            algorithmDiagnostic(
              "unknown-field",
              "A parameter is a plain name such as `A`.",
              entry.range,
              ctx.sourceName,
              { data: { field: "parameters" } },
            ),
        ...
        if (!IDENTIFIER.test(entry.value)) {
          ctx.diagnostics.push(
            algorithmDiagnostic(
              "unknown-field",
              `Parameter name "${entry.value}" must start with a letter or underscore and continue with letters, digits or underscores.`,
              entry.range,
              ctx.sourceName,
              { data: { field: "parameters", value: entry.value } },
            ),
          );
          continue;
        }
      ...
      if (
        names.length > MAX_ALGORITHM_PARAMETERS &&
        ...
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "limit-exceeded",
            `An algorithm takes at most ${MAX_ALGORITHM_PARAMETERS} parameters.`,
            field.range,
            ctx.sourceName,
            {
              data: {
    ...
    if (field.key === "steps") {
      if (field.record !== undefined) {
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "unknown-field",
            "`steps:` holds an ordered collection of statements.",
            field.range,
            ctx.sourceName,
            {
              data: { field: "steps" },
      ...
      if (entries.length === 0) {
        ctx.diagnostics.push(
          algorithmDiagnostic(
            "missing-body",
            "An algorithm requires at least one step.",
            field.range,
            ctx.sourceName,
            { data: { field: "steps" }, suggestion: STATEMENT_SUGGESTION },
          ),
        );
        continue;
      }
    ...
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "unknown-field",
        `Algorithm field "${field.key}" is not supported.`,
        field.range,
        ctx.sourceName,
        {
          data: { field: field.key },
          suggestion:
            "An algorithm body declares `procedure:`, `parameters:` and `steps:`.",
        },
      ),
    );
  }
  ...
  if (procedure === undefined && !seen.has("procedure")) {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "An algorithm requires `procedure:`.",
        args.blockRange,
        ctx.sourceName,
        { suggestion: "Write `procedure: BinarySearch`." },
      ),
  ...
  if (steps === undefined && !seen.has("steps")) {
    ctx.diagnostics.push(
      algorithmDiagnostic(
        "missing-body",
        "An algorithm requires `steps:`.",
        args.blockRange,
        ctx.sourceName,
        { suggestion: "Add a `steps:` collection holding the statements." },
      ),
    );
  }

  const diagnostics: readonly Diagnostic[] = Object.freeze(ctx.diagnostics);
  if (
  ...
  const body: AlgorithmBody = Object.freeze({
    procedure,
...
function keywordSpan(keyword: string): string {
  return `<span class="aze-algorithm-keyword">${keyword}</span>`;
}

function expressionSpan(spelling: string): string {
  return `<span class="aze-algorithm-expression">${escapeHtml(spelling)}</span>`;
}

function renderStatementList(statements: readonly AlgorithmStatement[]): string {
  return `<ol class="aze-algorithm-branch">${statements
    .map(renderStatement)
    .join("")}</ol>`;
}

function renderStatement(statement: AlgorithmStatement): string {
 ...
 */
export function renderAlgorithmFragment(
  block: AlgorithmBlock,
  _context: BlockRendererContext,
): string {
  const id =
    block.id === undefined ? "" : ` data-algorithm-id="${escapeAttribute(block.id)}"`;
  ...
  const parameters = block.parameters
    .map(
      (parameter) =>
        `<span class="aze-algorithm-parameter">${escapeHtml(parameter)}</span>`,
    )
    .join(", ");
...
export const algorithmHtmlBlockRenderer: AzeBlockRenderer<AlgorithmBlock> =
  Object.freeze({
