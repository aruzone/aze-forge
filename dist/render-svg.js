...
export const SVG_PROFILE = "azeforge.svg.foreign-object/v1";
export const SVG_SERIALIZER = "azeforge-svg/v1";
...
function svgBlockRenderer(renderer) {
...
export const svgBlockRenderers = Object.freeze([
    ...
    svgBlockRenderer(diagramHtmlBlockRenderer),
    svgBlockRenderer(sequenceHtmlBlockRenderer),
...
export const pinnedSvgBrowserCapability = Object.freeze({
    launch: launchPinnedBrowser,
});
export const svgRendererDescriptor = Object.freeze({
    id: SVG_RENDERER_ID,
    version: SVG_RENDERER_VERSION,
    formats: Object.freeze(["svg"]),
...
function canonicalXhtmlBody(body) {
...
export function sanitizeWholeDocumentSvg(svg) {
    const failures = [];
    const stack = [];
    ...
    parser.on("opentag", (tag) => {
        ...
        if (depth === 0) {
            rootElements += 1;
            if (name !== "svg")
                failures.push("The Artifact root must be SVG.");
        }
        ...
        for (const attribute of Object.values(tag.attributes)) {
            ...
            const readerLink = tag.uri === XHTML_NAMESPACE && name === "a" && attributeName === "href";
            const namespaceDeclaration = attribute.name === "xmlns" || attribute.prefix === "xmlns";
            if ((tag.uri === SVG_NAMESPACE && attributeName === "href") ||
                (!readerLink &&
                    !namespaceDeclaration &&
                    /^(?:https?:|file:|ftp:|javascript:|vbscript:|data:text\/html)/i.test(attribute.value))) {
...
async function measureSettledLayout(browser, layout, width) {
    ...
    try {
        ...
        await page.setContent(`<!doctype html><html lang="und"><head><meta charset="utf-8"><style>${layout.css}*{animation:none!important;transition:none!important;caret-color:transparent!important}</style></head><body>${layout.body}</body></html>`, { waitUntil: "domcontentloaded" });
...
export async function renderSvg(layout, contentHash, theme, assetManifest, browserCapability) {
