...
import { chemistryDependencyClosure, renderFormulaFragment, renderReactionFragment, renderStructureFragment, } from "./chemistry.js";
...
import { modelsDependencyClosure, } from "./models-render.js";
...
export const HTML_SERIALIZER = "azeforge-html/v2";
 ...
 */
function anchorAttribute(block) {
    if (!("id" in block))
        return "";
    const id = block.id;
...
function renderBlockBody(block, context) {
    ...
    switch (block.kind) {
        case "equation":
            return (context.equationFragments.get(block) ?? '<figure class="aze-equation"></figure>');
        case "derivation":
            return (context.derivationFragments.get(block) ??
                '<figure class="aze-derivation"></figure>');
        case "mermaid":
            return (context.mermaidFragments.get(block) ??
                '<figure class="aze-mermaid"></figure>');
        case "tex":
            return context.texFragments.get(block) ?? '<figure class="aze-tex"></figure>';
        case "diagram":
            return (context.diagramFragments.get(block) ??
        ...
        case "thematicBreak":
            return "<hr>";
        case "blockquote":
            return `<blockquote>${renderBlocks(block.children, context)}</blockquote>`;
        case "list": {
            const tag = block.ordered ? "ol" : "ul";
            const start = block.ordered && block.start !== undefined ? ` start="${block.start}"` : "";
            const items = block.items
                .map((item) => `<li>${renderBlocks(item.blocks, context)}</li>`)
                .join("");
            return `<${tag}${start}>${items}</${tag}>`;
        }
        case "code": {
            const language = block.language === undefined ? "" : ` class="language-${escapeHtml(block.language)}"`;
...
function embeddedFontCss(fontFaces) {
    return fontFaces
        .map((font) => {
...
export function createHtmlLayout(document, theme, fontFaces, equationFragments = new Map(), equationDependencyClosure = { katex: KATEX_VERSION }, derivationFragments = new Map(), mermaidFragments = new Map(), mermaidDependencyClosure = { mermaid: MERMAID_VERSION }, diagramFragments = new Map(), diagramDependencyClosure = {}, modelsFragments = new Map(), engineeringFragments = new Map(), controlDependencyClosureValue = {}, freeBodyDependencyClosureValue = {}, pluginRenderers = {}, texFragments = new Map(), texRendererIdentity) {
    const renderCallout = pluginRenderers.renderCallout ?? renderCalloutFragment;
    const renderTable = pluginRenderers.renderTable ??
        ((block, context) => renderTableFragment(block, context));
    const renderPlot = pluginRenderers.renderPlot ?? ((block, context) => renderPlotFragment(block, context));
    const renderChart = pluginRenderers.renderChart ?? ((block, context) => renderChartFragment(block, context));
    const renderGeometry = pluginRenderers.renderGeometry ?? ((block, context) => renderGeometryFragment(block, context));
    const renderFormula = pluginRenderers.renderFormula ?? ((block, context) => renderFormulaFragment(block, context));
    const renderReaction = pluginRenderers.renderReaction ?? ((block, context) => renderReactionFragment(block, context));
    const renderStructure = pluginRenderers.renderStructure ?? ((block, context) => renderStructureFragment(block, context));
    const renderCircuit = pluginRenderers.renderCircuit ?? ((block, context) => renderCircuitFragment(block, context));
    const renderTiming = pluginRenderers.renderTiming ?? ((block, context) => renderTimingFragment(block, context));
    const renderAlgorithm = pluginRenderers.renderAlgorithm ?? ((block, context) => renderAlgorithmFragment(block, context));
    const renderStatement = pluginRenderers.renderStatement ?? ((block, context) => renderStatementFragment(block, context));
    const renderExample = pluginRenderers.renderExample ?? ((block, context) => renderExampleFragment(block, context));
    ...
    const renderBibliography = pluginRenderers.renderBibliography ??
        ((block, context) => renderBibliographyFragment(block, context, citationStyle));
    const context = {
        equationFragments,
        derivationFragments,
        mermaidFragments,
    ...
    return {
        ...
        fingerprintDependencies: {
            ...
            ...(document.blocks.some((block) => block.kind === "timing")
                ...
                : {}),
            ...(document.blocks.some((block) => ["sequence", "state", "entity", "class"].includes(block.kind))
...
export async function renderHtml(document, contentHash, theme, fontFaces, equationFragments = new Map(), equationDependencyClosure = { katex: KATEX_VERSION }, derivationFragments = new Map(), mermaidFragments = new Map(), mermaidDependencyClosure = { mermaid: MERMAID_VERSION }, diagramFragments = new Map(), diagramDependencyClosure = {}, modelsFragments = new Map(), engineeringFragments = new Map(), controlDependencyClosureValue = {}, freeBodyDependencyClosureValue = {}, pluginRenderers = {}, texFragments = new Map(), texRendererIdentity, assetManifest = []) {
    const layout = createHtmlLayout(document, theme, fontFaces, equationFragments, equationDependencyClosure, derivationFragments, mermaidFragments, mermaidDependencyClosure, diagramFragments, diagramDependencyClosure, modelsFragments, engineeringFragments, controlDependencyClosureValue, freeBodyDependencyClosureValue, pluginRenderers, texFragments, texRendererIdentity);
