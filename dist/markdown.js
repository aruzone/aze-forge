...
export function isSafeLinkTarget(href) {
    ...
    const firstDelim = [slash, question, hash].filter((i) => i >= 0).reduce((min, i) => Math.min(min, i), Number.POSITIVE_INFINITY);
...
const LOCATOR_WORDS = Object.freeze({
    page: "page",
    pages: "pages",
    chapter: "chapter",
    section: "section",
 ...
 */
function opensReferenceToken(previous) {
    return previous === undefined || /[\s([{"'<]/.test(previous);
}
function tryParseFootnoteMarker(parser) {
...
function tryParseReferenceToken(parser, form) {
...
function tryReadLocator(parser) {
    ...
    if (word === undefined || value.length === 0)
        return undefined;
 ...
 */
function tryParseReferenceGroup(parser) {
    const start = parser.index;
...
function parseInlineRecursive(parser, stop) {
    const nodes = [];
    let literal = "";
    const flush = () => {
        if (literal !== "") {
    ...
    while (parser.index < text.length) {
        ...
        if (stop !== undefined && rest.startsWith(stop))
            break;
        ...
        if (char === "`") {
            let length = 0;
            while (text[parser.index + length] === "`")
                length += 1;
...
function parseLinkDestination(text, index) {
    let cursor = index;
    while (text[cursor] === " " || text[cursor] === "\n")
        cursor += 1;
    if (text[cursor] === "<") {
...
function parseLinkTitle(text, href, index) {
    let cursor = index;
    while (text[cursor] === " " || text[cursor] === "\n")
        cursor += 1;
    if (text[cursor] !== ")") {
...
function tryParseLink(parser) {
    ...
    const childParser = { text: label, index: 0, unsafeTargets: parser.unsafeTargets };
...
export function parseInlineFragment(text, rangeFor) {
    const parser = {
        text,
        index: 0,
        unsafeTargets: [],
...
export function inlineTextValue(nodes) {
    return nodes
        .map((node) => {
...
export function splitTableRow(line) {
    ...
    cells.push(current);
    if (cells.length > 0 && cells[0] !== undefined && cells[0].trim() === "")
        cells.shift();
    if (cells.length > 0 && cells[cells.length - 1] !== undefined && (cells[cells.length - 1] ?? "").trim() === "")
        cells.pop();
    if (cells.length === 0)
        return undefined;
...
export function tryParseGfmTable(lines) {
    if (lines.length < 2)
        return undefined;
    const headerCells = splitTableRow(lines[0] ?? "");
    const delimiterCells = splitTableRow(lines[1] ?? "");
    if (headerCells === undefined || delimiterCells === undefined)
        return undefined;
...
export function isGfmTableStart(headerLine, delimiterLine) {
