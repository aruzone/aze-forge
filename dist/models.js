...
import { CLASS_BODY_SYNTAX_ID, CLASS_BODY_SYNTAX_VERSION, CLASS_PLUGIN_TYPE, CLASS_PLUGIN_VERSION, ENTITY_BODY_SYNTAX_ID, ENTITY_BODY_SYNTAX_VERSION, ENTITY_PLUGIN_TYPE, ENTITY_PLUGIN_VERSION, SEQUENCE_BODY_SYNTAX_ID, SEQUENCE_BODY_SYNTAX_VERSION, SEQUENCE_PLUGIN_TYPE, SEQUENCE_PLUGIN_VERSION, STATE_BODY_SYNTAX_ID, STATE_BODY_SYNTAX_VERSION, STATE_PLUGIN_TYPE, STATE_PLUGIN_VERSION, classDataSchema, classSourceSchema, entityDataSchema, entitySourceSchema, sequenceDataSchema, sequenceSourceSchema, stateDataSchema, stateSourceSchema, } from "./models-schemas.js";
/* ------------------------------------------------------------------ *
...
export const PARTICIPANT_KINDS = ["participant", "actor"];
export const MESSAGE_FORMS = ["sync", "async", "return"];
export const TIMELINE_KINDS = ["message", "alt", "loop", "note"];
...
export const TOP_LEVEL_STATE_KINDS = ["state", "initial", "final", "transition"];
export const NESTED_STATE_KINDS = ["state", "initial", "final"];
...
export const ENTITY_KEYS = ["primary", "foreign", "unique"];
...
export const CLASS_ITEM_KINDS = ["class", "interface", "relationship"];
...
export const VISIBILITIES = ["public", "private", "protected", "package"];
export const CARDINALITIES = ["one", "zero-or-one", "many", "one-or-many"];
const CARDINALITY_DISPLAY = Object.freeze({
    one: "1",
    "zero-or-one": "0..1",
    many: "0..*",
    "one-or-many": "1..*",
...
export function cardinalityDisplay(value) {
 ...
 * Diagnostics
 * ------------------------------------------------------------------ */
function diag(namespace, code, message, range, sourceName, data, suggestion) {
...
function warn(namespace, code, message, range, sourceName, data = {}) {
    return createDiagnostic(`${namespace}#${code}`, "warning", message, {
        location: sourceName === undefined ? { range } : { source: sourceName, range },
        data,
    });
}
function related(range, message, sourceName) {
    return {
...
function limitExceeded(namespace, subject, count, limit, range, sourceName) {
    return diag(namespace, "limit-exceeded", `This Block's ${subject} count ${count} exceeds ${limit}.`, range, sourceName, { subject, count, limit });
}
function unknownKind(namespace, field, value, registered, range, sourceName) {
    return diag(namespace, "unknown-kind", `"${value}" is not a registered ${field} value.`, range, sourceName, { field, value }, vocabularySuggestion(value, registered));
}
...
function indentationFault(target, line) {
    target.diagnostics.push(diag(target.namespace, "unknown-field", "Declarations indent with spaces only, two per structural level.", line.range, target.sourceName, { field: declarationKey(line.text) }));
}
/**
 ...
 */
function readLiteral(lines, start, indent, target) {
    const raw = [];
    let index = start;
    let end = start;
...
function readField(lines, start, indent, target) {
    ...
    if (raw === "") {
        ...
        const opensCollection = first !== undefined && leadingIndent(first.text) === indent + 2 && ITEM.test(first.text.slice(indent + 2));
        if (opensCollection) {
...
function readFields(lines, start, indent, target) {
    const fields = [];
    let index = start;
    while (index < lines.length) {
        const line = lines[index];
        ...
        if (lineIndent > indent || ITEM.test(line.text.slice(indent))) {
            target.diagnostics.push(diag(target.namespace, "unknown-field", `Expected a \`key: value\` declaration at indentation ${indent}.`, line.range, target.sourceName, { field: declarationKey(line.text) }));
            index += 1;
            continue;
        }
        const read = readField(lines, index, indent, target);
        if (read === undefined) {
            target.diagnostics.push(diag(target.namespace, "unknown-field", "Expected a `key: value` declaration.", line.range, target.sourceName, {
        ...
        if (fields.some((field) => field.key === read.field.key)) {
            target.diagnostics.push(diag(target.namespace, "duplicate-field", `Field "${read.field.key}" is declared twice.`, line.range, target.sourceName, { field: read.field.key }));
            index = read.next;
            continue;
        }
...
function readEntries(lines, start, indent, target) {
    const entries = [];
    let index = start;
    while (index < lines.length) {
        const line = lines[index];
        ...
        if (lineIndent < indent)
            break;
        if (lineIndent > indent) {
            target.diagnostics.push(diag(target.namespace, "unknown-field", `Expected a \`- \` collection item at indentation ${indent}.`, line.range, target.sourceName, { field: declarationKey(line.text) }));
            index += 1;
            continue;
        }
        const item = ITEM.exec(line.text.slice(indent));
        if (item === null) {
            target.diagnostics.push(diag(target.namespace, "unknown-field", "Expected a `- ` collection item.", line.range, target.sourceName, {
                field: declarationKey(line.text),
            }));
            index += 1;
            continue;
        ...
        const fields = [{ key, value: raw, range: line.range }];
        for (const field of nested.fields) {
            if (fields.some((entry) => entry.key === field.key)) {
                target.diagnostics.push(diag(target.namespace, "duplicate-field", `Field "${field.key}" is declared twice.`, field.range, target.sourceName, { field: field.key }));
                continue;
            }
            fields.push(field);
        }
...
function allowedFields(namespace, fields, registered, owner, sourceName, diagnostics) {
    for (const field of fields) {
        if (!registered.includes(field.key)) {
            diagnostics.push(diag(namespace, "unknown-field", `${owner} field "${field.key}" is not supported.`, field.range, sourceName, { field: field.key }, vocabularySuggestion(field.key, registered)));
        }
    }
}
/* ------------------------------------------------------------------ *
...
function readTextField(namespace, field, owner, required, fallbackRange, sourceName, diagnostics, limit = MAX_MODELS_TEXT_CHARS) {
    if (field === undefined) {
        if (required) {
    ...
    if (field.collection !== undefined) {
        diagnostics.push(diag(namespace, required ? "missing-field" : "invalid-text", `${owner} requires a single-line value.`, field.range, sourceName, { field: field.key }));
        return undefined;
    }
    if (field.literal !== undefined) {
        const content = field.literal.join("\n");
        if (content === "" && required) {
            diagnostics.push(diag(namespace, "missing-field", `${owner} requires a value.`, field.range, sourceName, {
                field: field.key,
            }));
            return undefined;
        }
        if (field.literal.length > 1) {
            diagnostics.push(diag(namespace, "invalid-text", `${owner} must be one line of plain text.`, field.range, sourceName, {
                field: field.key,
            }));
            return undefined;
        }
    ...
    if (decoded.invalid) {
        diagnostics.push(diag(namespace, "invalid-text", `${owner} is not a well-formed quoted value.`, field.range, sourceName, {
            field: field.key,
        }));
        return undefined;
    }
    ...
    if (value === "") {
        diagnostics.push(diag(namespace, required ? "missing-field" : "invalid-text", required ? `${owner} requires a value.` : `${owner} is declared without a value.`, field.range, sourceName, { field: field.key }));
        return undefined;
    }
    if (codePoints(value) > limit) {
        diagnostics.push(diag(namespace, "invalid-text", `${owner} is ${codePoints(value)} characters; the ceiling is ${limit}.`, field.range, sourceName, { field: field.key, count: codePoints(value), limit }));
        return undefined;
    }
...
function readNameField(namespace, field, owner, fallbackRange, sourceName, diagnostics) {
    if (field === undefined || field.collection !== undefined || field.literal !== undefined) {
        diagnostics.push(diag(namespace, field === undefined ? "missing-field" : "invalid-name", `${owner} requires a name.`, field?.range ?? fallbackRange, sourceName));
        return undefined;
    }
    const decoded = decodeText(field.value.trim());
    const value = decoded.value;
    if (decoded.invalid || value === "" || /[\r\n\x00-\x1f\x7f]/.test(value)) {
        diagnostics.push(diag(namespace, "invalid-name", `${owner} must be a single-line name.`, field.range, sourceName, {
            field: field.key,
            value,
        }));
        return undefined;
    ...
    if (!NAME.test(value) || value.trim() !== value) {
        diagnostics.push(diag(namespace, "invalid-name", `${owner} "${value}" uses characters outside letters, digits, space, \`_\`, \`-\` and \`.\`.`, field.range, sourceName, { field: field.key, value }));
        return undefined;
    }
    if (codePoints(value) > MAX_MODELS_NAME_CHARS) {
        diagnostics.push(diag(namespace, "invalid-name", `${owner} "${value}" is ${codePoints(value)} characters; the ceiling is ${MAX_MODELS_NAME_CHARS}.`, field.range, sourceName, { field: field.key, value, count: codePoints(value), limit: MAX_MODELS_NAME_CHARS }));
        return undefined;
    }
    return value;
}
/** Read one `true`/`false` field; omission resolves to the registered default. */
function readBooleanField(namespace, field, fallback, sourceName, diagnostics) {
    if (field === undefined)
        return fallback;
...
function readEnumField(namespace, field, owner, registered, fallbackRange, sourceName, diagnostics) {
    if (field === undefined) {
        diagnostics.push(diag(namespace, "missing-field", `${owner} requires a registered value.`, fallbackRange, sourceName, {
            field: owner,
        }));
        return undefined;
    }
...
function readDefaultEnumField(namespace, field, registered, fallback, sourceName, diagnostics) {
    if (field === undefined)
        return fallback;
    if (!oneOf(registered, field.value)) {
        diagnostics.push(unknownKind(namespace, field.key, field.value, registered, field.range, sourceName));
...
function readHeader(namespace, headerLines, sourceName, diagnostics) {
    const header = {};
    const seen = new Set();
    for (const line of headerLines) {
        const text = line.text.trim();
        if (text === "" || COMMENT.test(line.text))
            continue;
        const match = FIELD.exec(text);
        if (match === null) {
            diagnostics.push(diag(namespace, "unknown-field", "A Block header is one `key: value` per line.", line.range, sourceName, {
        ...
        if (!HEADER_FIELDS.includes(key)) {
            diagnostics.push(diag(namespace, "unknown-field", `Block header field "${key}" is not supported.`, line.range, sourceName, { field: key }, vocabularySuggestion(key, HEADER_FIELDS)));
            continue;
        }
        if (seen.has(key)) {
            diagnostics.push(diag(namespace, "duplicate-field", `Header field "${key}" is declared twice.`, line.range, sourceName, {
                field: key,
            }));
            continue;
        }
        ...
        if (key === "id") {
            if (!LOWER_KEBAB.test(value) || codePoints(value) > MAX_MODELS_NAME_CHARS) {
                diagnostics.push(diag(namespace, "invalid-name", `Block id "${value}" must be lowercase-kebab, at most ${MAX_MODELS_NAME_CHARS} characters.`, line.range, sourceName, { field: "id", value }));
                continue;
            }
            header.id = value;
            continue;
        ...
        const text_ = readTextField(namespace, field, key === "title" ? "Block `title:`" : "Block `description:`", false, line.range, sourceName, diagnostics);
        if (text_ === undefined)
            continue;
        if (key === "title")
            header.title = text_;
        else
            header.description = text_;
    }
    return header;
}
function blockHeaderFields(header) {
    return {
...
function stacksDiffer(left, right) {
    const names = new Set([...left.keys(), ...right.keys()]);
    for (const name of names) {
        if ((left.get(name) ?? 0) !== (right.get(name) ?? 0))
            return true;
    }
    return false;
}
export function validateSequenceBlock(options) {
    const diagnostics = [];
    const sourceName = options.sourceName;
    const namespace = SEQUENCE_NAMESPACE;
    const target = { namespace, sourceName, diagnostics, bodyRange: options.blockRange };
    const header = readHeader(namespace, options.headerLines, sourceName, diagnostics);
    const text = (field, owner, required, range) => readTextField(namespace, field, owner, required, range, sourceName, diagnostics);
    const nameOf = (field, owner, range) => readNameField(namespace, field, owner, range, sourceName, diagnostics);
    const booleanOf = (field, fallback) => readBooleanField(namespace, field, fallback, sourceName, diagnostics);
    ...
    if (participantsField === undefined) {
        diagnostics.push(diag(namespace, "missing-field", "A sequence Block requires a `participants:` section.", options.blockRange, sourceName));
    }
    else if (participantsField.collection === undefined) {
        diagnostics.push(diag(namespace, "missing-field", "`participants:` opens an ordered collection of `- name:` records.", participantsField.range, sourceName));
    }
    if (timelineField === undefined) {
        diagnostics.push(diag(namespace, "missing-field", "A sequence Block requires a `timeline:` section.", options.blockRange, sourceName));
    }
    else if (timelineField.collection === undefined) {
        diagnostics.push(diag(namespace, "missing-field", "`timeline:` opens an ordered collection of `- kind:` items.", timelineField.range, sourceName));
    }
    ...
    if (participantEntries.length === 0) {
        diagnostics.push(diag(namespace, "empty", "A sequence Block declares no participants.", participantsField.range, sourceName));
    }
    if (participantEntries.length > MAX_SEQUENCE_PARTICIPANTS) {
        diagnostics.push(limitExceeded(namespace, "participants", participantEntries.length, MAX_SEQUENCE_PARTICIPANTS, participantsField.range, sourceName));
    }
    const participants = [];
    ...
    for (const entry of participantEntries) {
        ...
        if (name === undefined)
            continue;
        const kind = readDefaultEnumField(namespace, fieldOf(entry, "kind"), PARTICIPANT_KINDS, "participant", sourceName, diagnostics);
        ...
        if (first !== undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `Participant "${name}" is declared more than once.`, {
    ...
    if (timelineEntries.length === 0) {
        diagnostics.push(diag(namespace, "empty", "A sequence Block declares an empty timeline.", timelineField.range, sourceName));
    }
    const used = new Set();
    let itemCount = 0;
    const resolveParticipant = (field, owner, range) => {
        if (field === undefined) {
        ...
        if (participant === undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#unresolved-reference`, "error", `"${value}" is not a declared participant.`, {
    ...
    const walk = (entries, fragmentDepth) => {
        const items = [];
        for (const entry of entries) {
            ...
            if (kindField === undefined || !oneOf(TIMELINE_KINDS, kind)) {
                diagnostics.push(kindField === undefined
                    ? diag(namespace, "unknown-item", "A timeline item requires `- kind:`.", entry.range, sourceName, {
                        value: kind,
                    })
                    : diag(namespace, "unknown-item", `"${kind}" is not a registered timeline item kind.`, kindField.range, sourceName, { value: kind }, vocabularySuggestion(kind, TIMELINE_KINDS)));
                continue;
            }
            if (kind === "message") {
                allowedFields(namespace, entry.fields, MESSAGE_FIELDS, "Message", sourceName, diagnostics);
                const form = readDefaultEnumField(namespace, fieldOf(entry, "form"), MESSAGE_FORMS, "sync", sourceName, diagnostics);
            ...
            if (kind === "note") {
                ...
                if (overField === undefined) {
                    diagnostics.push(diag(namespace, "missing-field", "Note `over:` names one or two participants.", entry.range, sourceName));
                }
                    ...
                    for (const spanEntry of overField.collection.entries) {
                        if (spanEntry.value === undefined) {
                            diagnostics.push(diag(namespace, "invalid-note-span", "Note `over:` entries are bare participant names.", spanEntry.range, sourceName, { count: over.length, limit: MAX_SEQUENCE_NOTE_SPAN }));
                            continue;
                        }
                        const span = resolveParticipant({ key: "over", value: spanEntry.value, range: spanEntry.range }, "Note `over:`", spanEntry.range);
                        if (span !== undefined)
                            over.push(span);
                    }
                }
                if (over.length > MAX_SEQUENCE_NOTE_SPAN) {
                    diagnostics.push(diag(namespace, "invalid-note-span", `A note spans ${over.length} participants; the limit is ${MAX_SEQUENCE_NOTE_SPAN}.`, overField?.range ?? entry.range, sourceName, { count: over.length, limit: MAX_SEQUENCE_NOTE_SPAN }));
                }
                ...
                if (textField === undefined) {
                    ...
                    diagnostics.push(diag(namespace, "invalid-text", "Note `text:` is a `|` multiline field.", textField.range, sourceName, {
                        field: "text",
                    }));
                }
                else {
                    ...
                    if (content === "") {
                        ...
                        diagnostics.push(diag(namespace, "invalid-text", `Note text is ${codePoints(content)} characters; the ceiling is ${MAX_SEQUENCE_NOTE_TEXT_CHARS}.`, textField.range, sourceName, { field: "text", count: codePoints(content), limit: MAX_SEQUENCE_NOTE_TEXT_CHARS }));
                    }
                    else if (lines.length > MAX_SEQUENCE_NOTE_TEXT_LINES) {
                        diagnostics.push(diag(namespace, "invalid-text", `Note text is ${lines.length} lines; the ceiling is ${MAX_SEQUENCE_NOTE_TEXT_LINES}.`, textField.range, sourceName, { field: "text", count: lines.length, limit: MAX_SEQUENCE_NOTE_TEXT_LINES }));
                    }
                    else {
                        noteText = `${content}\n`;
                    }
                }
                if (over.length === 0 || noteText === undefined)
                    continue;
                const note = { kind: "note", over, text: noteText, range: entry.range };
                items.push(note);
                continue;
            }
            const bodyOf = (owner, range) => {
                const bodyField = fieldOf(entry, "body");
                if (bodyField === undefined || bodyField.collection === undefined) {
                    diagnostics.push(diag(namespace, "missing-field", `${owner} requires a \`body:\` collection.`, range, sourceName));
                    return undefined;
                }
                ...
                if (fragmentDepth > MAX_SEQUENCE_FRAGMENT_DEPTH) {
                    diagnostics.push(limitExceeded(namespace, "fragment-depth", fragmentDepth, MAX_SEQUENCE_FRAGMENT_DEPTH, range, sourceName));
                }
            ...
            if (divisionsField === undefined || divisionsField.collection === undefined) {
                diagnostics.push(diag(namespace, "missing-field", "An `alt` fragment requires a `divisions:` collection.", entry.range, sourceName));
                continue;
            }
            const divisionEntries = divisionsField.collection.entries;
            if (divisionEntries.length > MAX_SEQUENCE_ALT_DIVISIONS) {
                diagnostics.push(limitExceeded(namespace, "alt-divisions", divisionEntries.length, MAX_SEQUENCE_ALT_DIVISIONS, divisionsField.range, sourceName));
            }
            const divisions = [];
            let anyItem = false;
            for (const divisionEntry of divisionEntries) {
                ...
                const condition = text(fieldOf(divisionEntry, "condition"), "Division `condition:`", false, divisionEntry.range);
                const bodyField = fieldOf(divisionEntry, "body");
                if (bodyField === undefined || bodyField.collection === undefined) {
                    diagnostics.push(diag(namespace, "missing-field", "An alt division requires a `body:` collection.", divisionEntry.range, sourceName));
                }
                if (fragmentDepth > MAX_SEQUENCE_FRAGMENT_DEPTH) {
                    diagnostics.push(limitExceeded(namespace, "fragment-depth", fragmentDepth, MAX_SEQUENCE_FRAGMENT_DEPTH, divisionEntry.range, sourceName));
                }
                const body = bodyField?.collection === undefined ? [] : walk(bodyField.collection.entries, fragmentDepth + 1);
                if (body.length > 0)
                    anyItem = true;
                divisions.push({
                    ...(condition === undefined ? {} : { condition }),
            ...
            if (!anyItem) {
                diagnostics.push(diag(namespace, "empty-fragment", "Every division of this `alt` fragment is empty.", entry.range, sourceName));
            }
            const alt = { kind: "alt", divisions, range: entry.range };
            items.push(alt);
        }
        return items;
    };
    ...
    if (itemCount > MAX_SEQUENCE_TIMELINE_ITEMS) {
        diagnostics.push(limitExceeded(namespace, "timeline-items", itemCount, MAX_SEQUENCE_TIMELINE_ITEMS, timelineField.range, sourceName));
    }
    ...
    if (!hasErrors(diagnostics)) {
        for (const participant of participants) {
            if (!used.has(participant.name)) {
                diagnostics.push(warn(namespace, "unused-participant", `Participant "${participant.name}" carries no message or note.`, participant.range, sourceName, { name: participant.name }));
            }
        }
    }
    if (hasErrors(diagnostics))
 ...
 */
function checkActivationBalance(timeline, participants, timelineRange, sourceName, diagnostics) {
    const walk = (items, entry) => {
        const stacks = cloneStacks(entry);
        for (const item of items) {
            if (item.kind === "message") {
                if (item.activate)
                    stacks.set(item.to, (stacks.get(item.to) ?? 0) + 1);
                if (item.deactivate) {
                    const current = stacks.get(item.from) ?? 0;
                    if (current === 0) {
                        diagnostics.push(diag(SEQUENCE_NAMESPACE, "unbalanced-activation", `Deactivation on "${item.from}" has no open activation to close.`, item.range, sourceName, { participant: item.from, reason: "no open activation" }));
                    }
                    else {
            ...
            if (item.kind === "loop") {
                ...
                if (stacksDiffer(end, stacks)) {
                    const offender = participants.find((participant) => (end.get(participant.name) ?? 0) !== (stacks.get(participant.name) ?? 0));
                    diagnostics.push(diag(SEQUENCE_NAMESPACE, "unbalanced-activation", "A loop body must be activation-neutral: every bar it opens closes inside the same iteration.", item.range, sourceName, { reason: "loop body is not activation-neutral", participant: offender?.name ?? "" }));
                }
                continue;
            }
            ...
            if (disagree.length > 0) {
                diagnostics.push(diag(SEQUENCE_NAMESPACE, "unbalanced-activation", "Every `alt` division must end with the same set of open activations.", item.range, sourceName, { reason: "divisions disagree", divisions: [1, ...disagree] }));
            }
            ...
            for (const [name, count] of first)
                stacks.set(name, count);
        }
        return stacks;
    };
    const end = walk(timeline, new Map());
    for (const participant of participants) {
        if ((end.get(participant.name) ?? 0) > 0) {
            diagnostics.push(diag(SEQUENCE_NAMESPACE, "unbalanced-activation", `Activation on "${participant.name}" is still open at the end of the timeline.`, timelineRange, sourceName, { participant: participant.name, reason: "still open at end of timeline" }));
        }
    }
}
export function validateStateBlock(options) {
    const diagnostics = [];
    const sourceName = options.sourceName;
    ...
    let maxDepth = 0;
    const declareName = (name, kind, range, owner) => {
        const first = declarations.get(name);
        if (first !== undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `${owner} "${name}" is declared more than once.`, {
                location: sourceName === undefined ? { range } : { source: sourceName, range },
    ...
    const scopes = [];
    const topLevel = [];
    const readScope = (entries, depth, allowedKinds) => {
        const items = [];
        for (const entry of entries) {
            const kindField = fieldOf(entry, "kind");
            const kind = kindField?.value ?? "";
            if (kindField === undefined || !oneOf(allowedKinds, kind)) {
                diagnostics.push(diag(namespace, "unknown-item", `"${kind}" is not a registered item kind here.`, kindField?.range ?? entry.range, sourceName, { value: kind }, vocabularySuggestion(kind, allowedKinds)));
                continue;
            }
            if (kind === "transition") {
                transitionCount += 1;
            ...
            allowedFields(namespace, entry.fields, pseudo ? MODELS_PSEUDO_STATE_FIELDS : STATE_ITEM_FIELDS, pseudo ? `The \`${kind}\` pseudo-state` : "State", sourceName, diagnostics);
            const name = nameOf(fieldOf(entry, "name"), pseudo ? `An \`${kind}\` pseudo-state` : "A state", entry.range);
            if (name === undefined)
                continue;
            ...
            if (statesField !== undefined && statesField.collection === undefined) {
                diagnostics.push(diag(namespace, "missing-field", `State "${name}" \`states:\` opens a collection of nested states.`, statesField.range, sourceName));
            }
            else if (statesField?.collection !== undefined) {
                const inner = readScope(statesField.collection.entries, depth + 1, NESTED_STATE_KINDS);
                nested = inner.filter((item) => item.kind !== "transition");
    ...
    if (stateCount > MAX_STATE_STATES) {
        diagnostics.push(limitExceeded(namespace, "states", stateCount, MAX_STATE_STATES, options.blockRange, sourceName));
    }
    if (transitionCount > MAX_STATE_TRANSITIONS) {
        diagnostics.push(limitExceeded(namespace, "transitions", transitionCount, MAX_STATE_TRANSITIONS, options.blockRange, sourceName));
    }
    if (maxDepth > MAX_STATE_DEPTH) {
        diagnostics.push(limitExceeded(namespace, "state-depth", maxDepth, MAX_STATE_DEPTH, options.blockRange, sourceName));
    }
    ...
    for (const scope of scopes) {
        const pseudos = scope.items.filter((item) => item.kind === "initial" || item.kind === "final");
        const initials = pseudos.filter((item) => item.kind === "initial");
        const first = initials[0];
        if (first === undefined) {
            diagnostics.push(diag(namespace, "missing-initial", `Scope ${describeScope(scope.scope)} declares no \`initial\` pseudo-state.`, options.blockRange, sourceName, { scope: scope.scope }));
        }
        else if (initials.length > 1) {
            diagnostics.push(createDiagnostic(`${namespace}#multiple-initials`, "error", `Scope ${describeScope(scope.scope)} declares ${initials.length} \`initial\` pseudo-states.`, {
    ...
    for (const { transition, from, to } of resolutions) {
        if (from === undefined) {
            unresolved = true;
            diagnostics.push(diag(namespace, "unresolved-reference", `"${transition.from}" is not a declared state.`, transition.range, sourceName, {
                namespace: "state",
        ...
        if (to === undefined) {
            unresolved = true;
            diagnostics.push(diag(namespace, "unresolved-reference", `"${transition.to}" is not a declared state.`, transition.range, sourceName, {
                namespace: "state",
                name: transition.to,
            }));
        }
    ...
    if (!duplicated) {
        for (const { transition, to } of resolutions) {
            if (to?.kind !== "initial")
                continue;
            diagnostics.push(createDiagnostic(`${namespace}#initial-has-incoming`, "error", `A transition into the \`initial\` pseudo-state "${transition.to}" is not permitted.`, {
        ...
        for (const { transition, from } of resolutions) {
            if (from?.kind !== "final")
                continue;
            diagnostics.push(createDiagnostic(`${namespace}#final-has-outgoing`, "error", `A transition out of the \`final\` pseudo-state "${transition.from}" is not permitted.`, {
                location: sourceName === undefined ? { range: transition.range } : { source: sourceName, range: transition.range },
        ...
        const initialOf = new Map();
        for (const scope of scopes) {
            const initial = scope.items.find((item) => item.kind === "initial");
            if (initial !== undefined && "name" in initial)
                initialOf.set(scope.scope, initial.name);
        }
        ...
        while (queue.length > 0) {
            ...
            if (compositeScopes.has(current))
                enter(current);
        ...
        for (const item of allStates(topLevel)) {
            if (!reachable.has(item.name)) {
                diagnostics.push(warn(namespace, "unreachable-state", `State "${item.name}" is unreachable from its scope's \`initial\`.`, item.range, sourceName, { name: item.name }));
            }
            if (!hasOutgoing.has(item.name)) {
                diagnostics.push(warn(namespace, "dead-end-state", `State "${item.name}" has no outgoing transition.`, item.range, sourceName, { name: item.name }));
            }
        }
    }
    if (unresolved)
...
function collectTransitions(items) {
    return items.filter((item) => item.kind === "transition");
}
/* ------------------------------------------------------------------ *
 * `entity`
 * ------------------------------------------------------------------ */
export function validateEntityBlock(options) {
    const diagnostics = [];
    const sourceName = options.sourceName;
    ...
    for (const entry of entries) {
        ...
        if (kindField === undefined || (kind !== "entity" && kind !== "relationship")) {
            diagnostics.push(diag(namespace, "unknown-item", `"${kind}" is not a registered entity item kind.`, kindField?.range ?? entry.range, sourceName, { value: kind }, vocabularySuggestion(kind, ["entity", "relationship"])));
            continue;
        }
        if (kind === "relationship") {
            ...
            for (const end of [first, second]) {
                ...
                if (entity === undefined || cardinality === undefined)
                    break;
                ends.push({
                    entity,
                    cardinality,
        ...
        if (first !== undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `Entity "${name}" is declared more than once.`, {
                location: sourceName === undefined ? { range: entry.range } : { source: sourceName, range: entry.range },
        ...
        if (attributesField !== undefined && attributesField.collection === undefined) {
            diagnostics.push(diag(namespace, "missing-field", `Entity "${name}" \`attributes:\` opens a collection of attribute records.`, attributesField.range, sourceName));
        }
        const attributeEntries = attributesField?.collection?.entries ?? [];
        if (attributeEntries.length > MAX_ENTITY_ATTRIBUTES) {
            diagnostics.push(limitExceeded(namespace, "attributes", attributeEntries.length, MAX_ENTITY_ATTRIBUTES, attributesField?.range ?? entry.range, sourceName));
        }
    ...
    if (entities.size > MAX_ENTITY_ENTITIES) {
        diagnostics.push(limitExceeded(namespace, "entities", entities.size, MAX_ENTITY_ENTITIES, options.blockRange, sourceName));
    }
    if (relationshipCount > MAX_ENTITY_RELATIONSHIPS) {
        diagnostics.push(limitExceeded(namespace, "relationships", relationshipCount, MAX_ENTITY_RELATIONSHIPS, options.blockRange, sourceName));
    }
    ...
    for (const end of rawEnds) {
        ...
        if (end.entityField === undefined)
            continue;
        if (!entities.has(value)) {
            unresolved = true;
            diagnostics.push(createDiagnostic(`${namespace}#unresolved-reference`, "error", `"${value}" is not a declared entity.`, {
    ...
    for (const entity of entities.values()) {
        for (const attribute of entity.attributes ?? []) {
            ...
            if (targetAttribute === undefined) {
                unresolved = true;
                diagnostics.push(createDiagnostic(`${namespace}#unresolved-reference`, "error", `"${reference.entity}.${reference.attribute}" is not a declared attribute.`, {
                    location: sourceName === undefined ? { range: attribute.range } : { source: sourceName, range: attribute.range },
    ...
    if (!unresolved && !hasErrors(diagnostics)) {
        for (const entity of entities.values()) {
            if (!(entity.attributes ?? []).some((attribute) => attribute.keys?.includes("primary") === true)) {
                diagnostics.push(warn(namespace, "no-primary-key", `Entity "${entity.name}" declares no \`primary\` key.`, entity.range, sourceName, {
                    name: entity.name,
                }));
            }
            if (!linked.has(entity.name)) {
                diagnostics.push(warn(namespace, "unrelated-entity", `Entity "${entity.name}" appears in no relationship.`, entity.range, sourceName, { name: entity.name }));
            }
        }
    }
    if (hasErrors(diagnostics))
...
function readEnd(entry, key, diagnostics, namespace, sourceName) {
    const field = fieldOf(entry, key);
    if (field === undefined || field.record === undefined) {
        diagnostics.push(diag(namespace, "missing-field", `A relationship requires a \`${key}:\` end record with \`entity:\` and \`cardinality:\`.`, field?.range ?? entry.range, sourceName));
        return undefined;
    }
    ...
    if (entityField === undefined || entity === undefined || entity === "") {
        diagnostics.push(diag(namespace, "missing-field", `The \`${key}:\` end requires an \`entity:\` reference.`, field.range, sourceName));
    }
    let cardinality;
    if (cardinalityField === undefined) {
        diagnostics.push(diag(namespace, "missing-field", `The \`${key}:\` end requires a \`cardinality:\` value.`, field.range, sourceName));
    }
    else if (!oneOf(CARDINALITIES, cardinalityField.value)) {
        diagnostics.push(unknownKind(namespace, "cardinality", cardinalityField.value, CARDINALITIES, cardinalityField.range, sourceName));
    }
    else {
        cardinality = cardinalityField.value;
    }
    const role = readTextField(namespace, fieldByName(field.record, "role"), `The \`${key}:\` end \`role:\``, false, field.range, sourceName, diagnostics);
    return {
        range: field.range,
...
function readAttributes(entries, entityName, namespace, sourceName, diagnostics) {
    const attributes = [];
    for (const entry of entries) {
        allowedFields(namespace, entry.fields, ATTRIBUTE_FIELDS, "Attribute", sourceName, diagnostics);
        const name = readNameField(namespace, fieldOf(entry, "name"), `An attribute of "${entityName}"`, entry.range, sourceName, diagnostics);
        if (name === undefined)
            continue;
        const previous = attributes.find((attribute) => attribute.name === name);
        if (previous !== undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `Attribute "${name}" is declared more than once on "${entityName}".`, {
                location: sourceName === undefined ? { range: entry.range } : { source: sourceName, range: entry.range },
        ...
        const type = readTextField(namespace, fieldOf(entry, "type"), `Attribute "${name}" \`type:\``, false, entry.range, sourceName, diagnostics);
        const keysField = fieldOf(entry, "keys");
        const keys = [];
        if (keysField !== undefined && keysField.collection === undefined) {
            diagnostics.push(diag(namespace, "missing-field", `Attribute "${name}" \`keys:\` opens a collection of registered key kinds.`, keysField.range, sourceName));
        }
        for (const keyEntry of keysField?.collection?.entries ?? []) {
            if (keyEntry.value === undefined) {
                diagnostics.push(diag(namespace, "unknown-kind", "`keys:` entries are bare registered key kinds.", keyEntry.range, sourceName, { field: "keys", value: "" }, vocabularySuggestion("", ENTITY_KEYS)));
                continue;
            }
            ...
            if (keys.includes(value)) {
                diagnostics.push(diag(namespace, "duplicate-key", `Attribute "${name}" lists the key "${value}" twice.`, keyEntry.range, sourceName, {
                    key: value,
                    name,
                }));
                continue;
        ...
        if (referencesField !== undefined) {
            if (referencesField.record === undefined) {
                diagnostics.push(diag(namespace, "missing-field", `Attribute "${name}" \`references:\` is a record of \`entity:\` and \`attribute:\`.`, referencesField.range, sourceName));
            }
            else {
                ...
                if (targetEntity === undefined || targetEntity === "" || targetAttribute === undefined || targetAttribute === "") {
                    diagnostics.push(diag(namespace, "missing-field", `Attribute "${name}" \`references:\` requires an \`entity:\` and an \`attribute:\`.`, referencesField.range, sourceName));
                }
                else if (!keys.includes("foreign")) {
                    diagnostics.push(diag(namespace, "invalid-reference", `Attribute "${name}" declares \`references:\` without a \`foreign\` key.`, referencesField.range, sourceName, { reason: "requires key foreign" }));
                }
                else {
 ...
 * `class`
 * ------------------------------------------------------------------ */
export function validateClassBlock(options) {
    const diagnostics = [];
    const sourceName = options.sourceName;
    ...
    for (const entry of entries) {
        ...
        if (kindField === undefined || !oneOf(CLASS_ITEM_KINDS, kind)) {
            diagnostics.push(diag(namespace, "unknown-item", `"${kind}" is not a registered class item kind.`, kindField?.range ?? entry.range, sourceName, { value: kind }, vocabularySuggestion(kind, CLASS_ITEM_KINDS)));
            continue;
        }
        if (kind === "relationship") {
            allowedFields(namespace, entry.fields, CLASS_RELATIONSHIP_FIELDS, "Relationship", sourceName, diagnostics);
            const form = readEnumField(namespace, fieldOf(entry, "form"), "A class relationship `form:`", CLASS_RELATIONSHIP_FORMS, entry.range, sourceName, diagnostics);
            ...
            if (form === undefined || from === undefined || to === undefined)
                continue;
            if ((form === "inheritance" || form === "implementation") &&
                ...
                diagnostics.push(diag(namespace, "multiplicity-on-ranked-relationship", `A \`${form}\` relationship carries no multiplicity.`, field?.range ?? entry.range, sourceName, { field: field?.key ?? "from-multiplicity" }));
                continue;
            }
            const relationship = {
                kind: "relationship",
        ...
        allowedFields(namespace, entry.fields, isInterface ? MODELS_INTERFACE_FIELDS : CLASS_ITEM_FIELDS, isInterface ? "Interface" : "Class", sourceName, diagnostics);
        const name = nameOf(fieldOf(entry, "name"), isInterface ? "An interface" : "A class", entry.range);
        if (name === undefined)
            continue;
        const first = classifiers.get(name);
        if (first !== undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `"${name}" is declared more than once.`, {
                location: sourceName === undefined ? { range: entry.range } : { source: sourceName, range: entry.range },
        ...
        if (!isInterface && attributesField !== undefined && attributesField.collection === undefined) {
            diagnostics.push(diag(namespace, "missing-field", `"${name}" \`attributes:\` opens a collection of member records.`, attributesField.range, sourceName));
        }
        const attributeEntries = attributesField?.collection?.entries ?? [];
        if (attributeEntries.length > MAX_CLASS_ATTRIBUTES) {
            diagnostics.push(limitExceeded(namespace, "attributes", attributeEntries.length, MAX_CLASS_ATTRIBUTES, attributesField?.range ?? entry.range, sourceName));
        }
        ...
        if (operationsField !== undefined && operationsField.collection === undefined) {
            diagnostics.push(diag(namespace, "missing-field", `"${name}" \`operations:\` opens a collection of operation records.`, operationsField.range, sourceName));
        }
        const operationEntries = operationsField?.collection?.entries ?? [];
        if (operationEntries.length > MAX_CLASS_OPERATIONS) {
            diagnostics.push(limitExceeded(namespace, "operations", operationEntries.length, MAX_CLASS_OPERATIONS, operationsField?.range ?? entry.range, sourceName));
        }
        const operations = readClassOperations(operationEntries, name, attributes.map((attribute) => attribute.name), namespace, sourceName, diagnostics);
        const classifier = {
    ...
    if (classifierCount > MAX_CLASS_CLASSIFIERS) {
        diagnostics.push(limitExceeded(namespace, "classes", classifierCount, MAX_CLASS_CLASSIFIERS, options.blockRange, sourceName));
    }
    if (relationships.length > MAX_CLASS_RELATIONSHIPS) {
        diagnostics.push(limitExceeded(namespace, "relationships", relationships.length, MAX_CLASS_RELATIONSHIPS, options.blockRange, sourceName));
    }
    let unresolved = false;
    const linked = new Set();
    for (const { relationship, entry } of relationships) {
        for (const [key, value] of [
            ...
            if (source === undefined) {
                unresolved = true;
                diagnostics.push(createDiagnostic(`${namespace}#unresolved-reference`, "error", `"${value}" is not a declared class or interface.`, {
                    location: sourceName === undefined || field === undefined
    ...
    if (!unresolved) {
        for (const { relationship, entry } of relationships) {
            ...
            if (relationship.form === "inheritance") {
                ...
                if (!sameKind) {
                    diagnostics.push(diag(namespace, "invalid-relationship-target", `Inheritance between "${from.name}" (${from.kind}) and "${to.name}" (${to.kind}) mixes classifiers.`, entry.range, sourceName, { reason: "inheritance ends must both be classes or both be interfaces" }));
                }
            }
            if (relationship.form === "implementation" && to.kind !== "interface") {
                diagnostics.push(diag(namespace, "invalid-relationship-target", `Implementation target "${to.name}" is not an interface.`, entry.range, sourceName, { reason: "implementation target must be an interface" }));
            }
        }
    }
    if (!unresolved && !hasErrors(diagnostics)) {
        for (const classifier of classifiers.values()) {
            if (!linked.has(classifier.name)) {
                diagnostics.push(warn(namespace, "unrelated-class", `"${classifier.name}" appears in no relationship.`, classifier.range, sourceName, { name: classifier.name }));
            }
        }
        for (const cycle of inheritanceCycles(relationships)) {
            ...
            if (first === undefined)
                continue;
            diagnostics.push(createDiagnostic(`${namespace}#inheritance-cycle`, "error", `Inheritance is cyclic: ${cycle.path.join(" → ")}.`, {
...
function readClassAttributes(entries, className, namespace, sourceName, diagnostics) {
    const attributes = [];
    for (const entry of entries) {
        allowedFields(namespace, entry.fields, CLASS_ATTRIBUTE_FIELDS, "Attribute", sourceName, diagnostics);
        const name = readNameField(namespace, fieldOf(entry, "name"), `An attribute of "${className}"`, entry.range, sourceName, diagnostics);
        if (name === undefined)
            continue;
        const previous = attributes.find((attribute) => attribute.name === name);
        if (previous !== undefined) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `Attribute "${name}" is declared more than once on "${className}".`, {
                location: sourceName === undefined ? { range: entry.range } : { source: sourceName, range: entry.range },
                data: { namespace: "attribute", name },
                relatedLocations: [related(previous.range, `Attribute "${name}" was first declared here.`, sourceName)],
            }));
...
function readClassOperations(entries, className, attributeNames, namespace, sourceName, diagnostics) {
    const operations = [];
    for (const entry of entries) {
        allowedFields(namespace, entry.fields, OPERATION_FIELDS, "Operation", sourceName, diagnostics);
        const name = readNameField(namespace, fieldOf(entry, "name"), `An operation of "${className}"`, entry.range, sourceName, diagnostics);
        if (name === undefined)
            continue;
        ...
        if (clash !== undefined || attributeClash) {
            diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `Member "${name}" is declared more than once on "${className}".`, {
                location: sourceName === undefined ? { range: entry.range } : { source: sourceName, range: entry.range },
        ...
        const visibility = readVisibility(fieldOf(entry, "visibility"), namespace, sourceName, diagnostics);
        const returnType = readTextField(namespace, fieldOf(entry, "return-type"), `Operation "${name}" \`return-type:\``, false, entry.range, sourceName, diagnostics);
        const parametersField = fieldOf(entry, "parameters");
        if (parametersField !== undefined && parametersField.collection === undefined) {
            diagnostics.push(diag(namespace, "missing-field", `Operation "${name}" \`parameters:\` opens a collection of parameter records.`, parametersField.range, sourceName));
        }
        const parameterEntries = parametersField?.collection?.entries ?? [];
        if (parameterEntries.length > MAX_CLASS_PARAMETERS) {
            diagnostics.push(limitExceeded(namespace, "parameters", parameterEntries.length, MAX_CLASS_PARAMETERS, parametersField?.range ?? entry.range, sourceName));
        }
        const parameters = [];
        for (const parameterEntry of parameterEntries) {
            ...
            const parameterName = readNameField(namespace, fieldOf(parameterEntry, "name"), `A parameter of "${name}"`, parameterEntry.range, sourceName, diagnostics);
            if (parameterName === undefined)
                continue;
            const previous = parameters.find((parameter) => parameter.name === parameterName);
            if (previous !== undefined) {
                diagnostics.push(createDiagnostic(`${namespace}#duplicate-name`, "error", `Parameter "${parameterName}" is declared more than once on "${name}".`, {
            ...
            const type = readTextField(namespace, fieldOf(parameterEntry, "type"), `Parameter "${parameterName}" \`type:\``, false, parameterEntry.range, sourceName, diagnostics);
            parameters.push({
                name: parameterName,
                ...(type === undefined ? {} : { type }),
                range: parameterEntry.range,
...
function readVisibility(field, namespace, sourceName, diagnostics) {
    if (field === undefined)
        return undefined;
 ...
 */
function inheritanceCycles(relationships) {
...
export const sequencePlugin = Object.freeze({ descriptor: sequenceDescriptor });
...
export const statePlugin = Object.freeze({ descriptor: stateDescriptor });
...
export const entityPlugin = Object.freeze({ descriptor: entityDescriptor });
...
export const classPlugin = Object.freeze({ descriptor: classDescriptor });
