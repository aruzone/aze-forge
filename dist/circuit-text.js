...
function scriptPayload(raw, index, invalid) {
    if (raw[index] !== "{") {
        let end = index;
 ...
 */
export function parseCircuitText(options) {
...
export function circuitTextValue(text) {
    return text
        .map((run) => run.kind === "quantity" ? `${run.coefficient}${run.prefix}${run.unit}` : run.value)
        .join("");
}
//# sourceMappingURL=circuit-text.js.map