...
export function artifactBytesHash(bytes) {
    return sha256(bytes);
}
function projectInlineNode(node) {
    switch (node.kind) {
        case "text":
        case "code":
...
function projectCellValue(cell) {
    switch (cell.kind) {
        case "prose":
...
function projectTableData(data) {
    return {
        kind: "typed",
        ...
        rows: data.rows.map((row) => Object.fromEntries(data.columns.map((column) => [
            column.key,
            row[column.key] === undefined
                ? null
                : projectCellValue(row[column.key]),
        ]))),
    };
}
/** Pseudocode statements keep authored order and branch attachment. */
function projectAlgorithmStatement(statement) {
 ...
 */
function timingIntervals(intervals) {
    const merged = [];
    for (const interval of intervals) {
        const previous = merged[merged.length - 1];
        const sameText = previous !== undefined &&
            (previous.value === undefined || interval.value === undefined
                ? previous.value === interval.value
                : canonicalJson(previous.value) ===
                    canonicalJson(interval.value));
...
export function documentContentHash(document) {
    ...
    if (document.metadata.title !== undefined)
        metadata.title = document.metadata.title;
    if (document.metadata.theme !== undefined)
        metadata.theme = document.metadata.theme;
    if (document.metadata.outputs !== undefined)
        metadata.outputs = document.metadata.outputs;
    ...
    function projectBlock(block) {
        ...
        if (block.kind === "diagram") {
            ...
            const label = (value) => value === undefined ? undefined : value;
        ...
        if (block.kind === "sequence") {
            ...
            const timeline = (items) => items.map((item) => {
                if (item.kind === "message")
                    return message(item);
                if (item.kind === "note")
                    return { kind: item.kind, over: [...item.over], text: item.text };
                if (item.kind === "loop") {
                    return {
                        kind: item.kind,
        ...
        if (block.kind === "state") {
            const items = (entries) => entries.map((item) => {
        ...
        if (block.kind === "circuit") {
            const projected = {
                ...
                nodes: block.nodes.map(({ range: _range, ...node }) => node),
                components: block.components.map(({ range: _range, ...component }) => component),
                relations: block.relations.map(({ range: _range, ...relation }) => relation),
                annotations: block.annotations.map(({ range: _range, ...annotation }) => annotation),
            };
            if (block.id !== undefined)
                projected.id = block.id;
            if (block.number !== undefined)
        ...
        if (block.kind === "equation") {
            ...
            if (block.notation === "latex") {
                if (block.tex !== undefined)
                    projected.tex = block.tex;
            }
            else {
                if (block.tree !== undefined)
                    projected.tree = block.tree;
            }
            if (block.id !== undefined)
                projected.id = block.id;
            if (block.number !== undefined)
        ...
        if (block.kind === "plot") {
            const projected = {
                ...
                xAxis: block.xAxis,
                yAxis: block.yAxis,
                series: block.series.map((entry) => entry.kind === "function"
                    ? {
                        kind: entry.kind,
        ...
        if (block.kind === "chart") {
            const projected = {
                ...
                legend: block.legend,
                grid: block.grid,
                series: block.series.map((entry) => entry.kind === "bars"
                    ? {
                        kind: entry.kind,
                        ...(entry.label === undefined ? {} : { label: entry.label }),
            ...
            if (block.xLabel !== undefined)
                projected.xLabel = block.xLabel;
            if (block.yLabel !== undefined)
                projected.yLabel = block.yLabel;
            if (block.yMin !== undefined)
                projected.yMin = block.yMin;
            if (block.yMax !== undefined)
                projected.yMax = block.yMax;
            return projected;
        }
        if (block.kind === "thematicBreak") {
            const projected = { kind: block.kind };
            if (block.id !== undefined)
                projected.id = block.id;
            return projected;
        }
        ...
        if (block.kind === "code") {
            ...
            if (block.language !== undefined)
                projected.language = block.language;
            return projected;
        }
        if (block.kind === "table") {
            const projected = {
            ...
            if (block.pluginVersion !== undefined)
                projected.pluginVersion = block.pluginVersion;
            return projected;
        }
        if (block.kind === "callout") {
            ...
            if (block.id !== undefined)
                projected.id = block.id;
            if (block.title !== undefined)
                projected.title = block.title.map(projectInlineNode);
            return projected;
        }
        if (block.kind === "geometry") {
            const projected = {
                ...
                width: block.width,
                height: block.height,
                declarations: block.declarations.map((entry) => ({ ...entry })),
            };
            if (block.id !== undefined)
                projected.id = block.id;
            if (block.number !== undefined)
        ...
        if (block.kind === "reaction") {
            ...
            if (block.above !== undefined)
                projected.above = block.above;
            if (block.below !== undefined)
                projected.below = block.below;
            return projected;
        }
        ...
        if (block.kind === "statement") {
            ...
            if (block.proof !== undefined)
                projected.proof = block.proof.map(projectBlock);
            if (block.id !== undefined)
                projected.id = block.id;
            if (block.number !== undefined)
                projected.number = block.number;
        ...
        if (block.kind === "example") {
            ...
            if (block.result !== undefined)
                projected.result = block.result.map(projectBlock);
            if (block.id !== undefined)
                projected.id = block.id;
            if (block.number !== undefined)
                projected.number = block.number;
        ...
        if (block.kind === "heading")
            projected.level = block.level;
        return projected;
    }
    const blocks = document.blocks.map((block) => projectBlock(block));
