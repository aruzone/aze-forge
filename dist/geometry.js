...
const NAMESPACE = "azeforge.geometry";
...
const REGISTERED_KINDS = Object.freeze([
    ...PRIMITIVE_KINDS,
    ...CONSTRUCTION_KINDS,
    ...MARK_KINDS,
]);
...
function parseHeader(headerLines, blockRange, sourceName) {
    const diagnostics = [];
    let id;
    let number;
    ...
    for (const line of headerLines) {
        ...
        if (boundsFields !== undefined && GEOMETRY_BOUNDS_KEYS.includes(key)) {
        ...
        seen.add(key);
        if (!GEOMETRY_HEADER_FIELDS.includes(key)) {
...
function finishBounds(fields, sourceName) {
    const diagnostics = [];
    const values = {};
    ...
    if (missing.length > 0) {
        ...
        diagnostics.push(diag("invalid-bounds", `Geometry bounds require ${GEOMETRY_BOUNDS_KEYS.join(", ")} (missing: ${missing.join(", ")}).`, first ?? { start: { line: 0, column: 0 }, end: { line: 0, column: 0 } }, sourceName));
        return { diagnostics };
    }
    ...
    if (!(minX < maxX && minY < maxY)) {
        const first = fields[0]?.line.range;
        diagnostics.push(diag("invalid-bounds", "Geometry bounds must be finite and nonempty (min < max).", first ?? { start: { line: 0, column: 0 }, end: { line: 0, column: 0 } }, sourceName));
        return { diagnostics };
    }
    const bounds = {
        minX: values["min-x"],
...
function branchKey(p) {
    return `${quantize(p.x)}:${quantize(p.y)}`;
}
function orderBranches(points) {
...
function duplicateFields(raw, sourceName, diagnostics) {
    const seen = new Set();
    for (const field of raw.fields) {
...
export function validateGeometryBlock(options) {
    ...
    const flush = () => {
        if (current === undefined)
            return;
        const collections = new Map([...current.collections.entries()].map(([key, entries]) => [key, [...entries]]));
    ...
    for (const raw of raws) {
        ...
        const readPick = (branchCount, kindRange) => {
        ...
        const registerName = (entry, extra) => {
            if (name === undefined)
                return;
        ...
        switch (raw.kind) {
            ...
            case "midpoint": {
                ...
                if (raw.kind === "segment") {
                    ...
                    registerName({ line: { px: from.point.x, py: from.point.y, dx, dy }, point: undefined }, { from: from.name, to: to.name });
                }
                else {
            ...
            case "line": {
                ...
                registerName({ line: { px: first.point.x, py: first.point.y, dx: second.point.x - first.point.x, dy: second.point.y - first.point.y } }, { throughFirst: first.name, throughSecond: second.name });
                break;
            }
            case "ray": {
                const origin = readPointRef("origin");
                const through = readPointRef("through");
                if (origin === undefined || through === undefined)
                    break;
                ...
                registerName({ line: { px: origin.point.x, py: origin.point.y, dx: through.point.x - origin.point.x, dy: through.point.y - origin.point.y } }, { origin: origin.name, through: through.name });
                break;
            }
            case "circle": {
                ...
                if (radiusField !== undefined) {
                    ...
                    registerName({ circle: { cx: center.point.x, cy: center.point.y, r: radius.num } }, { center: center.name, radius: radius.value });
                }
                    ...
                    registerName({ circle: { cx: center.point.x, cy: center.point.y, r } }, { center: center.name, point: onField.value });
                }
                break;
            }
            case "arc": {
                ...
                if (center === undefined || radius === undefined || startField === undefined || endField === undefined || directionField === undefined)
                    break;
                if (!(radius.num > 0)) {
                ...
                registerName({ circle: { cx: center.point.x, cy: center.point.y, r: radius.num } }, { center: center.name, radius: radius.value, startAngle: start, endAngle: end, direction: directionField.value });
                break;
            }
            case "polygon": {
                ...
                const distinct = new Set(vertices.map((v) => branchKey(points.get(v))));
            ...
            case "tangent-line": {
                ...
                if (atField !== undefined && atField.value !== "") {
                    ...
                    registerName({ line: { px: at.x, py: at.y, dx: -dy, dy: dx } }, { circle: circleRef.name, at: atField.value });
                }
                    ...
                    const touch = ordered[pick - 1];
                    registerName({ line: { px: from.x, py: from.y, dx: touch.x - from.x, dy: touch.y - from.y } }, { circle: circleRef.name, from: fromField.value, pick });
                }
                break;
            }
            ...
            case "parallel-line": {
                const through = readPointRef("through");
                const to = readLineish("to");
                if (through === undefined || to === undefined)
                    break;
                const direction = raw.kind === "parallel-line"
                    ? { dx: to.line.dx, dy: to.line.dy }
                    : { dx: -to.line.dy, dy: to.line.dx };
                registerName({ line: { px: through.point.x, py: through.point.y, dx: direction.dx, dy: direction.dy } }, { through: through.name, to: to.name });
                break;
            }
            case "angle-mark":
    ...
    if (header.bounds !== undefined) {
        ...
        for (const declaration of declarations) {
            ...
            if (point === undefined)
                continue;
            if (point.x < Number(header.bounds.minX) || point.x > Number(header.bounds.maxX) ||
                point.y < Number(header.bounds.minY) || point.y > Number(header.bounds.maxY)) {
                outside.push(declaration.name);
            }
        }
        if (outside.length > 0) {
            warnings.push(diag("geometry-out-of-bounds", `Geometry declarations outside explicit bounds are clipped: ${outside.join(", ")}.`, blockRange, sourceName, { severity: "warning", data: { names: outside } }));
        }
    }
 ...
 * ------------------------------------------------------------------ */
function lineParams(entry) {
    return entry.line;
}
function intersectShapes(a, b, aName, bName, resolved, points) {
    void aName;
    void bName;
    void resolved;
    void points;
...
function intersectLines(a, b) {
    ...
    if (Math.abs(denom) <= GEOMETRY_EPSILON * scale * scale)
        return undefined;
...
function tangentPointsFromExterior(circle, from) {
...
export function resolveGeometry(block) {
    const points = new Map();
    const lines = new Map();
    ...
    for (const declaration of block.declarations) {
        ...
        const at = (name) => points.get(name);
        const lineAt = (name) => resolved.get(name)?.line;
        const circleAt = (name) => resolved.get(name)?.circle;
        if (kind === "point" && declaration.name !== undefined) {
            ...
            const from = at(declaration.from);
            const to = at(declaration.to);
            ...
            const first = at(declaration.throughFirst);
            const second = at(declaration.throughSecond);
            const entry = { px: first.x, py: first.y, dx: second.x - first.x, dy: second.y - first.y };
            lines.set(declaration.name, entry);
            resolved.set(declaration.name, { line: entry });
        }
        else if (kind === "ray" && declaration.name !== undefined) {
            const origin = at(declaration.origin);
            const through = at(declaration.through);
            ...
            const center = at(declaration.center);
            const r = declaration.radius !== undefined
                ? Number(declaration.radius)
                : Math.hypot(at(declaration.point).x - center.x, at(declaration.point).y - center.y);
            ...
            const a = resolved.get(declaration.first);
            const b = resolved.get(declaration.second);
            const candidates = intersectShapes(a, b, "", "", resolved, points) ?? [];
            const ordered = orderBranches(candidates);
            const pick = declaration.pick ?? 1;
            ...
            const circle = circleAt(declaration.circle);
            if (declaration.at !== undefined) {
                const touch = at(declaration.at);
                ...
                const ordered = orderBranches(tangency);
                const touch = ordered[(declaration.pick ?? 1) - 1] ?? ordered[0];
                if (touch !== undefined) {
            ...
            const from = at(declaration.from);
            const to = lineAt(declaration.to);
...
export class GeometrySanitizerError extends Error {
    code = "azeforge.geometry#sanitizer-compromise";
    constructor(message) {
        super(message);
...
export function assertGeometryFragmentSafe(svg) {
...
function geometryContentSeed(block) {
...
function renderMeasuredLength(block, resolved, declaration) {
    ...
    if (declaration.segment !== undefined) {
        ...
        if (target === undefined)
            return undefined;
        const from = target.from === undefined ? undefined : at(target.from);
        const to = target.to === undefined ? undefined : at(target.to);
        if (from === undefined || to === undefined)
            return undefined;
...
function renderMeasuredAngle(resolved, declaration) {
    const first = declaration.first === undefined ? undefined : resolved.points.get(declaration.first);
    const second = declaration.second === undefined ? undefined : resolved.points.get(declaration.second);
    const third = declaration.third === undefined ? undefined : resolved.points.get(declaration.third);
    if (first === undefined || second === undefined || third === undefined)
        return undefined;
...
export function renderGeometryFragment(block, _context) {
    ...
    for (const point of resolved.points.values())
    ...
    for (const declaration of block.declarations) {
        if (declaration.kind === "segment" && declaration.name !== undefined) {
            const from = declaration.from === undefined ? undefined : resolved.points.get(declaration.from);
            const to = declaration.to === undefined ? undefined : resolved.points.get(declaration.to);
            if (from !== undefined)
        ...
        if (declaration.kind === "polygon") {
            for (const vertex of declaration.vertices ?? []) {
    ...
    for (const declaration of block.declarations) {
        ...
        switch (declaration.kind) {
            ...
            case "angle-mark": {
                const vertex = declaration.second === undefined ? undefined : resolved.points.get(declaration.second);
                if (vertex === undefined)
                    break;
            ...
            case "length-mark": {
                ...
                if (declaration.segment !== undefined) {
                    const target = block.declarations.find((entry) => entry.name === declaration.segment);
                    const from = target?.from === undefined ? undefined : resolved.points.get(target.from);
                    const to = target?.to === undefined ? undefined : resolved.points.get(target.to);
                    if (from !== undefined && to !== undefined)
                        anchor = { x: (from.x + to.x) / 2, y: (from.y + to.y) / 2 };
                }
                else if (declaration.from !== undefined && declaration.to !== undefined) {
                    const from = resolved.points.get(declaration.from);
                    const to = resolved.points.get(declaration.to);
                    if (from !== undefined && to !== undefined)
                        anchor = { x: (from.x + to.x) / 2, y: (from.y + to.y) / 2 };
                }
                if (anchor === undefined)
            ...
            case "equal-marks": {
                const segments = declaration.segments ?? [];
                segments.forEach((segmentName) => {
                    const target = block.declarations.find((entry) => entry.name === segmentName);
                    const from = target?.from === undefined ? undefined : resolved.points.get(target.from);
                    const to = target?.to === undefined ? undefined : resolved.points.get(target.to);
                    if (from === undefined || to === undefined)
                        return;
    ...
    const constructions = block.declarations
        .filter((declaration) => declaration.name !== undefined && CONSTRUCTION_KINDS.includes(declaration.kind))
        .map((declaration) => `${declaration.kind} ${declaration.name}`);
    const marks = block.declarations.filter((declaration) => MARK_KINDS.includes(declaration.kind)).length;
...
export function geometryDependencyClosure() {
    return {
...
export const geometryPlugin = Object.freeze({
    descriptor: pluginDescriptor,
});
export const GEOMETRY_HTML_BLOCK_RENDERER_ID = "azeforge.geometry.html/v1";
export const GEOMETRY_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
...
export const geometryHtmlBlockRenderer = Object.freeze({
