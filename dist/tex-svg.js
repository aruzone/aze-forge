...
function findAttribute(element, local) {
    for (const [name, value] of element.attributes) {
        if (localName(name).toLowerCase() === local)
            return value;
    }
    return undefined;
}
 ...
 */
function scanElement(element, scan) {
    ...
    if (FORBIDDEN_ELEMENTS[name] === true)
        throw new TexSvgError("unsafe");
    for (const [attribute, value] of element.attributes) {
        if (NAMESPACE_ATTRIBUTE.test(attribute)) {
            if (attribute.startsWith("xmlns:"))
                scan.prefixes.add(attribute.slice("xmlns:".length));
            continue;
        }
        ...
        if (/^on[a-z]/.test(local))
            throw new TexSvgError("unsafe");
        if (local === "id")
            scan.identifiers.add(value);
        if (local === "style" && EXTERNAL_STYLE.test(value))
            throw new TexSvgError("external");
        if (local === "href") {
            if (value.startsWith("#"))
                scan.references.push(value.slice(1));
        ...
        for (const reference of value.matchAll(URL_REFERENCE)) {
            ...
            if (target.startsWith("#"))
                scan.references.push(target.slice(1));
...
function normalizeElement(element, prefix) {
    ...
    for (const [attribute, raw] of element.attributes) {
        ...
        if (local === "id") {
            ...
            value = value.replace(URL_REFERENCE, (match, single, double, bare) => {
    ...
    for (const child of element.children) {
        ...
        if (child.text.trim() === "" && TEXT_CONTENT_ELEMENTS[name] !== true)
            continue;
        children.push(child);
    }
...
function serializeNode(node) {
    ...
    const attributes = names
        .map((name) => {
        const value = (node.attributes.get(name) ?? "").replace(/[&<>"]/g, (character) => XML_ESCAPES[character] ?? character);
        return ` ${name}="${value}"`;
    })
        .join("");
...
function assertIntrinsicExtent(root) {
 ...
 */
export function projectTexSvg(options) {
    const root = parseDocument(options.svg);
    if (localName(root.name).toLowerCase() !== "svg")
        throw new TexSvgError("malformed");
    if (root.attributes.get("xmlns") !== SVG_NAMESPACE)
        throw new TexSvgError("malformed");
    const scan = {
    ...
    for (const reference of scan.references) {
        if (!scan.identifiers.has(reference))
            throw new TexSvgError("reference");
    }
    const prefix = `tex-${options.requestIndex}-`;
    const normalized = normalizeElement(root, prefix);
    if (normalized === undefined)
        throw new TexSvgError("malformed");
    if (normalized.attributes.get("role") === undefined)
        normalized.attributes.set("role", "img");
    ...
    normalized.children.unshift({ name: "title", attributes: new Map(), children: [{ text: options.title }] }, { name: "desc", attributes: new Map(), children: [{ text: options.description }] });
