...
import { buildGrammarDocument, grammarDirectiveTypes, serializeGrammar, } from "./grammar.js";
import { createDiagnostic } from "./diagnostics.js";
import { createDiagnosticsReport } from "./diagnostics-json.js";
import { HELP_COMMANDS, commandHelp, globalHelp, humanCapabilitiesReport, humanGrammarReport, humanVersionReport, versionLine, } from "./help.js";
...
function parseGrammarArguments(rest, diagnosticsMode) {
    let json = false;
    let directive;
...
function parseArguments(rawArguments, diagnosticsMode) {
    ...
    if ((command !== "validate" && command !== "render") ||
        ...
        throw new CliUsageError("Usage: azeforge render <source> --output <artifact> | azeforge validate <source> | azeforge format <source> [--write | --check] | azeforge watch <source> --output <artifact> | azeforge serve <source> [--port <port>] | azeforge capabilities [--probe] [--json] | azeforge grammar [--json] [--directive <type>] | azeforge version [--json]");
    }
    if (command === "validate") {
...
async function main() {
    ...
    try {
        ...
        if (arguments_.command === "grammar") {
            if (arguments_.machine) {
                process.stdout.write(serializeGrammar(buildGrammarDocument(arguments_.directive === undefined
                    ? {}
                    : { directive: arguments_.directive })));
                return;
            }
            process.stderr.write(humanGrammarReport(buildGrammarDocument(arguments_.directive === undefined
                ? {}
                : { directive: arguments_.directive })));
            return;
        }
