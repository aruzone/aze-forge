...
function collectInlineImages(nodes, range, out) {
...
export function isAbsoluteSource(src) {
    return (isAbsolute(src) ||
        src.startsWith("/") ||
        src.startsWith("\\") ||
        /^[A-Za-z]:[\\/]/.test(src));
}
...
function jpegDimensions(bytes) {
    if (bytes.length < 4 || bytes[0] !== 0xff || bytes[1] !== 0xd8)
        return undefined;
    let offset = 2;
    const markerAt = (at) => at + 1 < bytes.length && bytes[at] === 0xff ? bytes[at + 1] : undefined;
    while (offset < bytes.length) {
...
function hasExternalSvgReference(body) {
    const references = body.matchAll(/(?<![-\w])(?:xlink:href|href|src)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/gi);
...
export function checkSvg(text) {
    ...
    if (/<script[\s>]/i.test(body))
        return "active";
    if (/<foreignObject[\s>]/i.test(body))
        return "active";
...
function failure(code, message, target, sourceName, suggestion) {
...
function embedInlineImages(nodes, embedded) {
...
function embedBlockImages(blocks, embedded) {
    return blocks.map((block) => {
        switch (block.kind) {
            ...
            case "blockquote":
                return Object.freeze({
                    ...block,
                    children: embedBlockImages(block.children, embedded),
                });
            case "callout":
                return Object.freeze({
                    ...block,
            ...
            case "list":
                return Object.freeze({
                    ...block,
                    items: block.items.map((item) => Object.freeze({
                        ...item,
                        blocks: embedBlockImages(item.blocks, embedded),
                    })),
                });
            case "table":
                {
            ...
            case "derivation":
                return Object.freeze({
                    ...block,
                    steps: block.steps.map((step) => Object.freeze({
                        ...step,
            ...
            case "figure": {
                ...
                return Object.freeze({
                    ...
                    ...(block.caption === undefined
                        ? {}
                        : { caption: embedInlineImages(block.caption, embedded) }),
                    children: embedBlockImages(groups[0], embedded),
                });
            }
            case "statement": {
                const groups = blockGroups(block);
                return Object.freeze({
                    ...
                    ...(block.caption === undefined
                        ? {}
                        : { caption: embedInlineImages(block.caption, embedded) }),
                    text: embedBlockImages(groups[0], embedded),
                    ...(groups[1] === undefined
                        ? {}
                        : { proof: embedBlockImages(groups[1], embedded) }),
                });
            }
            case "example": {
                const groups = blockGroups(block);
                ...
                return Object.freeze({
                    ...
                    ...(block.caption === undefined
                        ? {}
                        : { caption: embedInlineImages(block.caption, embedded) }),
                    problem: embedBlockImages(groups[0], embedded),
                    steps: block.steps.map((step, index) => {
                        ...
                        return Object.freeze({
                            ...
                            text: group === undefined
                                ? step.text
                                : embedBlockImages(group, embedded),
                        });
                    }),
                    ...(hasResult
                        ? { result: embedBlockImages(result, embedded) }
                        : {}),
                });
            }
            case "bibliography":
...
export function assetManifestHash(manifest) {
...
function checkRasterDimensions(dimensions) {
...
export async function resolveProjectImages(document, options = {}) {
    const targets = [];
    ...
    if (options.projectRoot === undefined) {
        return {
            diagnostics: [
                createDiagnostic("azeforge.asset#root-required", "error", "Project images need a project root before Artifact publication.", {
                    ...(options.sourceName === undefined
                        ? {}
                        : { location: { source: options.sourceName } }),
                    data: { images: targets.length },
                    suggestion: "Render through the CLI so images resolve inside the Source directory.",
                }),
            ],
        };
    }
    ...
    try {
        ...
        return {
            diagnostics: [
                createDiagnostic("azeforge.asset#root-required", "error", "The project root could not be resolved.", {
                    ...(options.sourceName === undefined
                        ? {}
                        : { location: { source: options.sourceName } }),
                    data: {},
    ...
    for (const target of targets) {
        if (embedded.has(target.node))
            continue;
        const src = target.node.src;
        const fail = (code, message, suggestion) => {
            diagnostics.push(failure(`azeforge.asset#${code}`, message, target, options.sourceName, suggestion));
        };
        if (src.trim() === "" || src.includes(String.fromCharCode(0))) {
            fail("unsupported-type", "Image source is empty.", "Reference a project PNG, JPEG, or SVG image with a relative path.");
            continue;
        }
        if (isAbsoluteSource(src)) {
            fail("path-escape", `Image "${src}" escapes the project root; absolute image paths are rejected.`, "Reference the image with a relative path inside the project directory.");
            continue;
        }
        if (isRemoteSource(src)) {
            fail("unsupported-type", `Image "${src}" is remote; project images must be local files.`, "Add the image to the project directory and reference it with a relative path.");
            continue;
        }
        const expected = EXTENSION_MEDIA_TYPES[extname(src).toLowerCase()];
        if (expected === undefined) {
            fail("unsupported-type", `Image "${src}" has an unsupported type; P0 images are PNG, JPEG, or SVG.`, "Convert the image to PNG, JPEG, or SVG inside the project directory.");
            continue;
        }
        const candidate = resolve(root, src);
        if (!isContained(root, candidate)) {
            fail("path-escape", `Image "${src}" escapes the project root.`, "Reference the image with a relative path inside the project directory.");
            continue;
        }
        let candidateReal;
        try {
            ...
            fail("unreadable", `Image "${src}" could not be read.`, "Check that the image exists inside the project directory.");
            continue;
        }
        if (!isContained(root, candidateReal)) {
            fail("path-escape", `Image "${src}" escapes the project root through a symlink.`, "Keep project images as real files inside the project directory.");
            continue;
        }
        let bytes;
        try {
        ...
        if (bytes.length > MAX_IMAGE_BYTES) {
            fail("decompression-bomb", `Image "${src}" exceeds the ${MAX_IMAGE_BYTES} byte image limit.`, "Shrink the image before referencing it from project Source.");
            continue;
        }
        const sniffed = sniffMediaType(bytes);
        if (sniffed === undefined) {
            fail("unsupported-type", `Image "${src}" is not a readable PNG, JPEG, or SVG image.`, "Replace it with a valid static PNG, JPEG, or SVG image.");
            continue;
        }
        if (sniffed !== expected) {
            fail("mime-mismatch", `Image "${src}" ends in ${extname(src).toLowerCase()} but its bytes are ${sniffed}.`, "Rename the image so its extension matches its bytes.");
            continue;
        }
        if (sniffed === "image/png" || sniffed === "image/jpeg") {
            const dimensions = sniffed === "image/png" ? pngDimensions(bytes) : jpegDimensions(bytes);
            if (sniffed === "image/png" && dimensions !== undefined && "animated" in dimensions && dimensions.animated) {
                fail("animated", `Image "${src}" is animated; P0 images are static.`, "Replace it with a static PNG, JPEG, or SVG image.");
                continue;
            }
            const verdict = checkRasterDimensions(dimensions);
            if (verdict === "malformed") {
                fail("malformed", `Image "${src}" is a malformed ${sniffed === "image/png" ? "PNG" : "JPEG"} image.`, "Replace it with a valid static PNG, JPEG, or SVG image.");
                continue;
            }
            if (verdict === "bomb") {
                fail("decompression-bomb", `Image "${src}" exceeds the P0 image dimension limits.`, "Shrink the image before referencing it from project Source.");
                continue;
            }
        }
        else {
            ...
            if (verdict === "active") {
                fail("active-content", `Image "${src}" contains scripted SVG content.`, "Remove scripts, event handlers, and foreign objects from the SVG.");
                continue;
            }
            if (verdict === "external") {
                fail("external-resource", `Image "${src}" references resources outside the SVG.`, "Inline every resource the SVG needs so the Artifact stays self-contained.");
                continue;
            }
            if (verdict === "malformed" || verdict === "dimensions") {
                fail("malformed", verdict === "dimensions"
                    ? `Image "${src}" has invalid SVG dimensions.`
                    : `Image "${src}" is a malformed SVG image.`, verdict === "dimensions"
    ...
    const manifest = [...manifestByPath.values()].sort((left, right) => left.path < right.path ? -1 : left.path > right.path ? 1 : 0);
    return {
        diagnostics: [],
