...
export const PDF_RENDERER_ID = "pdf";
export const PDF_RENDERER_VERSION = "1.0.0";
export const PDF_MIME_TYPE = "application/pdf";
export const PDF_PROFILE = "azeforge.pdf.paged/v1";
export const PDF_SERIALIZER = "azeforge-pdf/v1";
const PDF_PRODUCER = `AzeForge pdf/1.0.0 (HeadlessChrome/${CHROME_HEADLESS_SHELL_VERSION})`;
const PDF_CREATOR = "AzeForge";
export const PDF_REQUIRED_CAPABILITIES = Object.freeze(["pdf-paged"]);
...
const ATOMIC_SELECTOR = ".aze-equation,.aze-mermaid,.aze-diagram,.aze-plot,.aze-chart,.aze-geometry,.aze-circuit,.aze-timing,.aze-formula,.aze-reaction,.aze-structure,.aze-sequence,.aze-state,.aze-entity,.aze-class,.aze-control,.aze-free-body,figure,img";
/**
...
function pdfBlockRenderer(renderer) {
...
export const pdfBlockRenderers = Object.freeze([
    ...
    pdfBlockRenderer(diagramHtmlBlockRenderer),
    pdfBlockRenderer(sequenceHtmlBlockRenderer),
    pdfBlockRenderer(stateHtmlBlockRenderer),
    pdfBlockRenderer(entityHtmlBlockRenderer),
...
export const pinnedPdfBrowserCapability = Object.freeze({
    launch: launchPinnedBrowser,
});
export const pdfRendererDescriptor = Object.freeze({
    id: PDF_RENDERER_ID,
...
export function pdfPageGeometryForTheme(theme) {
...
function pdfContentHeightCssPx(geometry) {
 ...
 */
export function pdfPagedCss(theme) {
    const geometry = pdfPageGeometryForTheme(theme);
    const sectionBreak = theme.id === "dark-presentation"
        ? "article>h1:not(:first-child){break-before:page}"
        : "";
    return (`@page{${geometry.pageCss}}` +
...
function pdfHexText(value) {
    ...
    for (let index = 0; index < little.length; index += 2) {
        ...
        big[index + 3] = little[index];
    }
    return big.toString("hex").toUpperCase();
}
const PDF_WHITESPACE = new Set(["\0", "\t", "\n", "\f", "\r", " "]);
function skipPdfWhitespace(text, position) {
    while (position < text.length && PDF_WHITESPACE.has(text[position])) {
        position += 1;
    }
    return position;
}
function splitPdfObjects(text, xrefOffset) {
    const objects = [];
 ...
 * hash identically.
 */
function isPdfDocumentId(value) {
    return /^[0-9a-f]{32}$/.test(value);
}
export function canonicalizePdf(raw, identity) {
    ...
    if (pageCount > PDF_MAX_PAGES) {
        throw new PdfArtifactLimitError(raw.byteLength, `PDF Artifact has ${pageCount} pages; the limit is ${PDF_MAX_PAGES} pages.`);
    }
    ...
    // deterministic ID.
    const structIdByNode = new Map();
    for (const object of objects) {
        for (const match of object.raw.matchAll(/\/ID \(node(\d+)\)/g)) {
            structIdByNode.set(match[1], object.num);
        }
    }
    const rewriteNodeTokens = (raw) => {
        if (!raw.includes("(node"))
            return raw;
        return raw.replace(/\(node(\d+)\)/g, (_, digits) => {
    ...
    const sortStructNameTree = (raw) => {
        if (!raw.includes("azeforge-struct-"))
            return raw;
        let next = raw;
        ...
        if (names?.[1] !== undefined) {
            ...
            if (pairs.length > 0) {
                ...
                if (limits !== null) {
                    next = next.replace(limits[0], `/Limits [(azeforge-struct-${first}) (azeforge-struct-${last})]`);
                }
            }
            return next;
        }
...
function settlePagedHtml(layout, theme) {
    return (`<!doctype html><html lang="und"><head><meta charset="utf-8"><title>${layout.title}</title>` +
...
async function printPagedPdf(browser, html, geometry, contentHeightPx) {
    ...
    try {
        ...
        const overflow = await page.evaluate(async (input) => {
        ...
        if (overflow.length > 0) {
            throw new PdfArtifactLimitError(Buffer.byteLength(html, "utf8"), `Atomic PDF content is ${Math.max(...overflow)}px tall at the minimum readable scale; the page content box is ${Math.floor(contentHeightPx)}px tall. Split or shrink the atomic content.`);
        }
...
export async function renderPdf(layout, contentHash, theme, assetManifest, browserCapability, info) {
    const geometry = pdfPageGeometryForTheme(theme);
