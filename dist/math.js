 ...
 * ------------------------------------------------------------------ */
const GREEK_LOWER = Object.freeze({
    alpha: "\\alpha",
    beta: "\\beta",
    gamma: "\\gamma",
    delta: "\\delta",
...
const GREEK_VARIANTS = Object.freeze({
...
const GREEK_UPPER = Object.freeze({
    Gamma: "\\Gamma",
    Delta: "\\Delta",
    Theta: "\\Theta",
    Lambda: "\\Lambda",
...
const RELATION_TEX = Object.freeze({
    "=": "=",
    "!=": "\\neq",
    "<": "<",
    ">": ">",
...
const RELATION_SPELLING = Object.freeze({
    "=": "=",
    "!=": "!=",
    "<": "<",
    ">": ">",
...
const SET_OP_TEX = Object.freeze({
    union: "\\cup",
    intersect: "\\cap",
});
...
const MATRIX_NAMES = Object.freeze(["matrix", "pmatrix", "vmatrix"]);
const MATRIX_TEX = Object.freeze({
    matrix: "bmatrix",
    pmatrix: "pmatrix",
    vmatrix: "vmatrix",
});
const MATRIX_DELIMITER = Object.freeze({
    matrix: "bracket",
    pmatrix: "paren",
    vmatrix: "bar",
});
const CLAUSE_WORDS = Object.freeze(["when", "otherwise", "of"]);
function vocabulary() {
    const words = {
        ...GREEK_LOWER,
        ...GREEK_VARIANTS,
    ...
    for (const op of Object.keys(RELATION_TEX))
        words[op] = op;
    for (const op of Object.keys(SET_OP_TEX))
        words[op] = op;
    for (const op of BINDER_OPS)
        words[op] = op;
...
function problem(code, message, offset, length, extra = {}) {
    return {
        code,
...
const PUNCT_TOKENS = Object.freeze({
    "+": "+",
    "-": "-",
    "*": "*",
    "/": "/",
...
function lex(source) {
    ...
    while (index < source.length) {
        ...
        if (WORD_RE.test(char)) {
            let end = index + 1;
            while (end < source.length && WORD_RE.test(source[end] ?? ""))
                end += 1;
        ...
        if (char === "{" || char === "\\") {
            problems.push(problem("unsupported-notation", `"${char}" is TeX spelling; native notation is a closed vocabulary.`, index, 1, {
                suggestion: char === "{"
            ...
            if (char === "\\") {
                index += 1;
                while (index < source.length && WORD_RE.test(source[index] ?? ""))
                    index += 1;
            }
            else {
        ...
        problems.push(problem("unsupported-notation", `"${char}" has no native alpha form; use registered constructs.`, index, 1, {
...
class Parser {
    ...
    error(code, message, token, extra = {}) {
        throw new MathSyntaxError(problem(code, message, token.offset, token.length, extra));
    }
    errorAtEnd(code, message, extra = {}) {
        const token = this.peek();
        throw new MathSyntaxError(problem(code, message, token.offset, 0, extra));
    }
    withDepth(token, fn) {
        this.depth += 1;
        if (this.depth > MAX_MATH_NESTING_DEPTH) {
            this.depth -= 1;
            this.error("limit-exceeded", `Nesting exceeds the maximum supported depth of ${MAX_MATH_NESTING_DEPTH}.`, token, { data: { limit: MAX_MATH_NESTING_DEPTH } });
        }
        try {
            return fn();
        }
    ...
    isAtomStart(token) {
        ...
        if (token.kind === "unicode") {
            return (SYMBOL_NAMES.has(token.text) ||
                token.text === "sqrt" ||
        ...
        if (token.text.length === 1)
            return true;
        if (token.text.length === 2 && VOCABULARY[token.text] === undefined)
            return true;
    ...
    parseRelation() {
        ...
        for (;;) {
            const token = this.peek();
            if (token.kind !== "word" && token.kind !== "punct" && token.kind !== "unicode")
                break;
    ...
    parseSet() {
        ...
        for (;;) {
            const token = this.peek();
            if (token.kind !== "word" && token.kind !== "unicode")
                break;
            ...
            left = { kind: "setop", op: token.text, left, right };
        }
        return left;
    }
    /** Level 3 — additive. */
    ...
    parseMultiplicative() {
        ...
        for (;;) {
            ...
            if (this.isAtomStart(token)) {
                const factors = left.kind === "juxt" ? [...left.factors] : [left];
        ...
        for (;;) {
            ...
            if (token.text === "^") {
                if (sup !== undefined || primes !== undefined) {
                    this.error("chained-power", "Chained powers are ambiguous; parenthesize the exponent.", token, { suggestion: "Parenthesize, for example x^(2^3)." });
                }
                this.advance();
            ...
            if (token.text === "'") {
                if (sub !== undefined || sup !== undefined || primes !== undefined) {
                    this.error("chained-power", "Primes cannot combine with powers or subscripts on one base.", token);
                }
                let count = 0;
                const first = token;
                ...
                if (count > 2) {
                    this.error("chained-power", "At most two primes are supported per identifier.", first);
                }
                primes = count;
                continue;
            }
    ...
    parseSubscript() {
        ...
        this.error("missing-operand", "A subscript must be digits or a lowercase word.", token);
    }
    parseIdentifier() {
        const token = this.peek();
        if (token.kind !== "word" || token.text.length !== 1) {
            this.error("unexpected-token", "An identifier (one ASCII letter) is expected here.", token);
        }
        this.advance();
        let digitSuffix;
    ...
    parseBoundName() {
        ...
        if (token.kind !== "word" || token.text.length !== 1) {
            if (token.kind === "word" && VOCABULARY[token.text] !== undefined) {
                this.error("reserved-identifier", `"${token.text}" is a reserved Greek or keyword alias; choose another bound name.`, token, { suggestion: "Registered aliases can never be identifiers." });
            }
            this.error("unexpected-token", "A bound name (one ASCII letter) is expected here.", token);
        }
    ...
    parseAtom() {
        ...
        this.error("missing-operand", "An operand is expected here.", token);
    }
    parseUnicodeToken(token) {
        this.advance();
        const key = token.text;
        if (key === "sqrt")
            return this.parseSqrt();
        if (SYMBOL_NAMES.has(key))
            return { kind: "symbol", name: key };
        if (RELATION_TEX[key] !== undefined || SET_OP_TEX[key] !== undefined) {
            this.error("unexpected-token", `"${token.text}" needs an operand before it.`, this.peek());
        }
        if (BINDER_OPS.includes(key)) {
            return this.parseBinder(key);
        }
        this.error("unexpected-token", `Unexpected token "${token.text}".`, token);
    }
    parseWordToken(token) {
        ...
        if (!known) {
            ...
            this.error("unknown-word", `"${text}" is not in the native mathematics vocabulary.`, token, {
        ...
        if (MATRIX_NAMES.includes(text)) {
            return this.parseMatrix(text);
        }
        if (text === "cases")
            return this.parseCases();
        if (BINDER_OPS.includes(text)) {
            return this.parseBinder(text);
        }
        if (text === "in" || text === "when" || text === "otherwise" || text === "of") {
            this.error("unexpected-token", `"${text}" cannot begin an expression.`, token);
        }
        this.error("unexpected-token", `Unexpected token "${text}".`, token);
    }
    expectPunct(text, message) {
        const token = this.peek();
    ...
    parseVector() {
        ...
        return this.withDepth(open, () => {
            ...
            for (;;) {
                ...
                if (items.length >= 16) {
                    this.error("limit-exceeded", "A vector may hold at most 16 items.", token, { data: { limit: 16 } });
                }
                if (items.length > 0) {
                    this.expectPunct(",", "A comma separates vector items.");
                }
                if (this.peek().kind === "punct" && this.peek().text === "]") {
                    this.error("missing-operand", "A vector item is expected here.", this.peek());
                }
        ...
        return this.withDepth(open, () => {
            ...
            while (this.peek().kind === "punct" && this.peek().text === "[") {
                if (rows.length >= MAX_MATRIX_DIMENSION) {
                    this.error("limit-exceeded", `A matrix may have at most ${MAX_MATRIX_DIMENSION} rows.`, this.peek(), { data: { limit: MAX_MATRIX_DIMENSION } });
                }
                ...
                for (;;) {
                    ...
                    if (cells.length >= MAX_MATRIX_DIMENSION) {
                        this.error("limit-exceeded", `A matrix may have at most ${MAX_MATRIX_DIMENSION} cells per row.`, token, { data: { limit: MAX_MATRIX_DIMENSION } });
                    }
                    if (cells.length > 0) {
                ...
                if (cellCount === undefined) {
                    ...
                    this.error("ragged-matrix", `Every row needs ${cellCount} cells; this row has ${cells.length}.`, at, { data: { row: rows.length + 1, expected: cellCount, found: cells.length } });
                }
                rows.push(cells);
                if (this.peek().kind === "punct" && this.peek().text === ",") {
                    this.advance();
                    if (!(this.peek().kind === "punct" && this.peek().text === "[")) {
                        this.error("unexpected-token", "A matrix row must open with `[`.", this.peek());
                    }
                }
            }
            ...
            return { kind: "matrix", delimiter: MATRIX_DELIMITER[name], rows };
        });
    }
    parseCases() {
        const open = this.peek();
        return this.withDepth(open, () => {
            ...
            for (;;) {
                ...
                if (branches.length >= MAX_CASES_BRANCHES) {
                    this.error("limit-exceeded", `cases may have at most ${MAX_CASES_BRANCHES} branches.`, token, { data: { limit: MAX_CASES_BRANCHES } });
                }
                ...
                this.error("unexpected-token", "A case branch needs `when <condition>` or `otherwise`.", next);
            }
            if (branches.length === 0) {
    ...
    parseFunction(name) {
        ...
        if (token.kind === "punct" && token.text === "(") {
            ...
            const arg = this.withDepth(open, () => {
                if (this.peek().kind === "punct" && this.peek().text === ")") {
                    this.error("missing-operand", `\`${name}\` needs an argument inside the parentheses.`, this.peek());
                }
    ...
    parseBinder(op) {
        ...
        return this.withDepth(open, () => {
            ...
            if (name !== undefined && this.boundNames.some((bound) => bound.name === name.name)) {
                ...
                this.warn("shadowed-bound-variable", `Bound variable "${name.name}" shadows an outer binder's ${name.name}; parenthesize to keep scopes explicit.`, token.offset, token.length);
            }
            let from;
            let to;
            let target;
            ...
            if (op === "sum" || op === "product" || op === "integral") {
                if (this.peek().kind === "punct" && this.peek().text === "=") {
                    ...
                    if (!(dots.kind === "punct" && dots.text === "..")) {
                        this.errorAtEnd("unexpected-token", "A bound range needs `..` between the bounds.");
                    }
                    this.advance();
                ...
                if (!(arrow.kind === "punct" && arrow.text === "->")) {
                    this.errorAtEnd("unexpected-token", "`limit` requires a target: limit x->a of BODY.");
                }
                this.advance();
                ...
                if (!((inToken.kind === "word" || inToken.kind === "unicode") &&
                    inToken.text === "in")) {
                    this.errorAtEnd("unexpected-token", `\`${op}\` requires a set: ${op} x in S of BODY.`);
                }
                this.advance();
            ...
            if (!(ofToken.kind === "word" && ofToken.text === "of")) {
                this.error("missing-of-clause", `\`${op}\` requires an \`of\` clause before its body.`, ofToken, {
            ...
            this.advance();
            if (name !== undefined)
                this.boundNames.push({ name: name.name });
    ...
    checkAdditiveAfterOf() {
        const token = this.peek();
        if (token.kind === "punct" && (token.text === "+" || token.text === "-")) {
            this.warn("additive-after-of", `"${token.text}" binds outside the binder scope; parenthesize the operand.`, token.offset, token.length);
        }
    }
    finishIntegral(name, body, from, to) {
        ...
        if (name !== undefined && name.name !== variable) {
            ...
            const token = this.tokens.find((candidate) => candidate.offset === differentialOffset) ??
                this.tokens[this.position - 1] ??
                this.peek();
            this.error("differential-mismatch", `The integral binds "${name.name}" but the differential is d${variable}; use d${name.name} or rebind.`, token);
        }
        if (rest.length === 0) {
            this.errorAtEnd("missing-operand", "An integral needs an operand before the differential.");
        }
        const bodyOnly = rest.length === 1
            ? rest[0]
    ...
    missingDifferential() {
        this.errorAtEnd("missing-integration-variable", "The integral is missing a terminal differential such as dx.", { suggestion: "Add an integration variable such as dx to the integral." });
    }
    warn(code, message, offset, length) {
        this.warnings.push(problem(code, message, offset, length, { severity: "warning" }));
    }
}
function isPlainD(node) {
 ...
 */
export function parseNativeMath(source) {
    ...
    try {
        ...
        if (remaining.kind !== "eof") {
            return {
                problems: [
                    problem("unexpected-token", `Unexpected token "${remaining.text}".`, remaining.offset, remaining.length),
                ],
            };
        }
...
function nearestVocabulary(word) {
 ...
 * ------------------------------------------------------------------ */
export function canonicalSpelling(node) {
    switch (node.kind) {
        case "ident": {
            let out = node.name;
            if (node.digitSuffix !== undefined)
                out += node.digitSuffix;
            if (node.subscript !== undefined)
                out += `_${node.subscript}`;
            if (node.primes !== undefined)
                out += "'".repeat(node.primes);
            return out;
        }
        case "number":
        ...
        case "chain":
            return [
                ...
                ...node.links.map((link) => `${RELATION_SPELLING[link.op] ?? link.op} ${canonicalSpelling(link.node)}`),
            ].join(" ");
        case "setop":
        ...
        case "cases":
            return `cases(${node.branches
                .map((branch) => branch.cond.kind === "otherwise"
        ...
        case "binder": {
            ...
            const differential = node.differential === undefined ? "" : ` d${node.differential}`;
            return `${node.op} ${clause} of ${canonicalSpelling(node.body)}${differential}`;
        }
    }
}
function matrixName(delimiter) {
    if (delimiter === "paren")
        return "pmatrix";
    if (delimiter === "bar")
        return "vmatrix";
 ...
 * ------------------------------------------------------------------ */
export function projectMathNode(node) {
    switch (node.kind) {
        case "ident":
            return {
                kind: "ident",
        ...
        case "cases":
            return {
                ...
                branches: node.branches.map((branch) => ({
                    ...
                    cond: branch.cond.kind === "otherwise"
                        ? { kind: "otherwise" }
 ...
 * ------------------------------------------------------------------ */
export function treeToTex(node) {
    switch (node.kind) {
        case "ident": {
            let out = node.name;
            if (node.digitSuffix !== undefined)
                out += node.digitSuffix;
            if (node.subscript !== undefined)
                out += subscriptTex(node.subscript);
            if (node.primes !== undefined)
                out += "'".repeat(node.primes);
            return out;
        }
        ...
        case "chain": {
            const parts = [treeToTex(node.head)];
        ...
        case "matrix": {
            const env = MATRIX_TEX[matrixName(node.delimiter)] ?? "bmatrix";
 ...
 */
export function projectionToNode(value) {
    ...
    switch (kind) {
        case "ident":
            return {
                ...
                ...(obj.primes === undefined ? {} : { primes: obj.primes }),
            };
        case "number":
        ...
        case "add": {
            const left = projectionToNode(obj.left);
            const right = projectionToNode(obj.right);
        ...
        case "juxt": {
            ...
            for (const factor of obj.factors) {
                const node = projectionToNode(factor);
                if (node === undefined)
                    return undefined;
                factors.push(node);
            }
        ...
        case "script": {
            const base = projectionToNode(obj.base);
            if (base === undefined)
                return undefined;
            const sup = obj.sup === undefined ? undefined : projectionToNode(obj.sup);
        ...
        case "chain": {
            const head = projectionToNode(obj.head);
            if (head === undefined || !Array.isArray(obj.links))
                return undefined;
            const links = [];
            for (const link of obj.links) {
                const entry = link;
                const node = projectionToNode(entry.node);
                if (node === undefined)
                    return undefined;
                links.push({ op: String(entry.op ?? ""), node });
            }
            return { kind: "chain", head, links };
        }
        case "setop": {
            ...
            if (left === undefined || right === undefined)
                return undefined;
            return { kind: "setop", op: String(obj.op ?? ""), left, right };
        }
        case "func": {
            const arg = projectionToNode(obj.arg);
            if (arg === undefined)
                return undefined;
            return {
                kind: "func",
        ...
        case "root": {
            const index = projectionToNode(obj.index);
            const arg = projectionToNode(obj.arg);
            if (index === undefined || arg === undefined)
                return undefined;
            return { kind: "root", index, arg };
        }
        case "frac": {
            const num = projectionToNode(obj.num);
            const den = projectionToNode(obj.den);
        ...
        case "vector": {
            ...
            for (const item of obj.items) {
                const node = projectionToNode(item);
                if (node === undefined)
                    return undefined;
                items.push(node);
            }
        ...
        case "matrix": {
            ...
            for (const row of obj.rows) {
                ...
                for (const cell of row) {
                    const node = projectionToNode(cell);
                    if (node === undefined)
                        return undefined;
                    cells.push(node);
                }
            ...
            return {
                kind: "matrix",
                delimiter: String(obj.delimiter ?? "bracket"),
                rows,
            };
        }
        case "cases": {
            ...
            for (const branch of obj.branches) {
                const entry = branch;
                const expr = projectionToNode(entry.expr);
                if (expr === undefined)
                    return undefined;
                const condValue = entry.cond;
                const cond = condValue?.kind === "otherwise"
                    ? { kind: "otherwise" }
                    : projectionToNode(condValue);
                if (cond === undefined)
                    return undefined;
                branches.push({ expr, cond });
            }
        ...
        case "binder": {
            ...
            const body = projectionToNode(obj.body);
            if (body === undefined)
                return undefined;
            ...
            if (name !== undefined && name.kind !== "ident")
                return undefined;
            const from = obj.from === undefined ? undefined : projectionToNode(obj.from);
            if (obj.from !== undefined && from === undefined)
                return undefined;
            const to = obj.to === undefined ? undefined : projectionToNode(obj.to);
            if (obj.to !== undefined && to === undefined)
                return undefined;
            const target = obj.target === undefined ? undefined : projectionToNode(obj.target);
            if (obj.target !== undefined && target === undefined)
                return undefined;
            const set = obj.set === undefined ? undefined : projectionToNode(obj.set);
            if (obj.set !== undefined && set === undefined)
                return undefined;
            const differential = obj.differential === undefined ? undefined : String(obj.differential);
            return {
                kind: "binder",
                op: String(obj.op ?? ""),
                ...(name === undefined ? {} : { name }),
 ...
 */
export function treeToTexFromProjection(projection) {
 ...
 * absolute space.
 */
export function rangeForOffset(bodyRanges, offset, length) {
    ...
    for (let index = 0; index <= lastIndex; index += 1) {
        ...
        const relEnd = next === undefined ? Number.MAX_SAFE_INTEGER : (next.start.offset ?? base) - 1 - base;
