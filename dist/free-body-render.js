 ...
 * contentHash.
 */
import { FREE_BODY_EMITTER_VERSION, FREE_BODY_EVALUATOR_VERSION, FREE_BODY_PLUGIN_TYPE, FREE_BODY_PLUGIN_VERSION, FREE_BODY_SCALE_POLICY_VERSION, } from "./control-schemas.js";
import { circuitTextValue } from "./circuit-text.js";
import { escapeXml, quantize } from "./plot.js";
export const FREE_BODY_HTML_BLOCK_RENDERER_ID = "azeforge.free-body.html/v1";
export const FREE_BODY_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
...
export class FreeBodyRenderError extends Error {
    code = "azeforge.renderer#free-body-render";
    remedy = "Re-check the free-body declaration list and the Theme handed to the renderer; a renderer failure publishes no Artifact.";
    constructor(message) {
        super(message);
...
export function assertFreeBodyFragmentSafe(svg) {
...
function requireUnit(from, to, what) {
...
function lineMarkup(a, b, stroke, widthPx, dash) {
    return (`<line x1="${quantize(a.x)}" y1="${quantize(a.y)}" x2="${quantize(b.x)}" y2="${quantize(b.y)}"` +
...
function circleMarkup(center, radiusPx, fill, stroke, widthPx) {
    return (`<circle cx="${quantize(center.x)}" cy="${quantize(center.y)}" r="${quantize(radiusPx)}"` +
...
function dotMarkup(center, radiusPx, fill, stroke) {
    return (`<circle cx="${quantize(center.x)}" cy="${quantize(center.y)}" r="${quantize(radiusPx)}"` +
        ` fill="${escapeXml(fill)}" stroke="${escapeXml(stroke)}"/>`);
}
function polygonMarkup(points, fill, stroke, widthPx) {
    const coordinates = points.map((point) => `${quantize(point.x)},${quantize(point.y)}`).join(" ");
    return (`<polygon points="${coordinates}" fill="${escapeXml(fill)}"` +
        ` stroke="${escapeXml(stroke)}" stroke-width="${quantize(widthPx)}"/>`);
}
/**
 ...
 */
function arrowHeadLength(stemPx, floorPx, heads) {
...
function arrowHeadMarkup(tip, unit, lengthPx, fill) {
    if (!(lengthPx > 0))
        return "";
...
function labelMarkup(options) {
    if (options.text === "")
        return "";
    return (`<text id="${options.id}" class="aze-free-body-label" x="${quantize(options.at.x)}"` +
 ...
 */
function arcMarkup(center, radiusFrame, startDegrees, sweepDegrees, project, scale, stroke, widthPx) {
    if (!(radiusFrame > 0) || sweepDegrees === 0)
        return undefined;
    ...
    const path = `<path d="M ${quantize(start.x)} ${quantize(start.y)}` +
    ...
    const tangentFrame = sweepDegrees > 0
...
function requireLine(lines, name) {
...
function resolveFreeBody(block) {
...
function attachmentFrame(attachment, resolved) {
 ...
 */
function directionVector(direction, resolved) {
    switch (direction.kind) {
        ...
        case "perpendicular-to": {
            ...
            const length = Math.hypot(dx, dy);
            if (!(length > 0)) {
                throw new FreeBodyRenderError(`Free-body line "${direction.line}" has no direction: its two points coincide.`);
            }
...
function forceLength(block, declaration) {
    if (block.scale !== undefined) {
        if (declaration.magnitude === undefined) {
            throw new FreeBodyRenderError("Free-body force under an explicit scale: carries no magnitude:.");
        }
 ...
 */
function extendDeclaration(extent, declaration, resolved, block) {
...
function axesArms(origin, angle, half) {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
    return [
...
const NO_MARKS = Object.freeze({ shapes: "", text: "" });
/** One authored label, at its declaration's positional id. */
function textMarkup(env, index, suffix, text, at, anchor, fill, fontSizePx) {
    return labelMarkup({
...
function emitForce(declaration, index, env) {
    ...
    if (stemPx > 0) {
        ...
        if (headPx < stemPx) {
            shapes.push(lineMarkup(tailPx, { x: tipPx.x - unit.x * headPx, y: tipPx.y - unit.y * headPx }, tokens.forceStroke, tokens.forceStrokeWidthPx, ""));
        }
...
function emitMoment(declaration, index, env) {
    ...
    const arc = arcMarkup(center, radiusFrame, MOMENT_ARC_START_DEGREES, sweep, env.project, env.scale, tokens.momentStroke, tokens.momentStrokeWidthPx);
    if (arc === undefined)
        return NO_MARKS;
    ...
    const label = declaration.label === undefined ? "" : circuitTextValue(declaration.label);
    const headPx = arrowHeadLength(MOMENT_ARC_RADIUS_PX * Math.abs(radians(sweep)), tokens.arrowheadMinLengthPx, 1);
    return {
...
function emitAxes(declaration, index, env) {
    const tokens = env.tokens;
    ...
    const shapes = lineMarkup(env.project({ x: origin.x - cos * half, y: origin.y - sin * half }), xEnd, tokens.axisStroke, tokens.axisStrokeWidthPx, tokens.axisDash) +
        lineMarkup(env.project({ x: origin.x + sin * half, y: origin.y - cos * half }), yEnd, tokens.axisStroke, tokens.axisStrokeWidthPx, tokens.axisDash);
    const xUnit = requireUnit(originPx, xEnd, "axis");
    const yUnit = requireUnit(originPx, yEnd, "axis");
    const text = textMarkup(env, index, "-label-x", circuitTextValue(declaration.xLabel), { x: xEnd.x + xUnit.x * AXIS_LABEL_OFFSET_PX, y: xEnd.y + xUnit.y * AXIS_LABEL_OFFSET_PX }, "middle", tokens.axisStroke, tokens.labelFontSizePx) +
        textMarkup(env, index, "-label-y", circuitTextValue(declaration.yLabel), { x: yEnd.x + yUnit.x * AXIS_LABEL_OFFSET_PX, y: yEnd.y + yUnit.y * AXIS_LABEL_OFFSET_PX }, "middle", tokens.axisStroke, tokens.labelFontSizePx);
    return { shapes, text };
}
function emitAngleMark(declaration, index, env) {
    const tokens = env.tokens;
    ...
    const arc = arcMarkup(vertex, radiusFrame, (theta * 180) / Math.PI, (sweep * 180) / Math.PI, env.project, env.scale, tokens.bodyStroke, tokens.bodyStrokeWidthPx);
    if (arc === undefined)
        return NO_MARKS;
    ...
    const label = declaration.label === undefined ? "" : circuitTextValue(declaration.label);
    const headPx = arrowHeadLength(radiusFrame * env.scale * Math.abs(sweep), tokens.arrowheadMinLengthPx, 1);
    return {
...
function emitDimension(declaration, index, env) {
    const tokens = env.tokens;
    ...
    const stub = (at) => lineMarkup({ x: at.x - normal.x * tick, y: at.y - normal.y * tick }, { x: at.x + normal.x * tick, y: at.y + normal.y * tick }, stroke, width, "");
    const shapes = [stub(fromPx), stub(toPx)];
    if (headPx > 0 && headPx * 2 < spanPx) {
        shapes.push(lineMarkup({ x: fromPx.x + unit.x * headPx, y: fromPx.y + unit.y * headPx }, { x: toPx.x - unit.x * headPx, y: toPx.y - unit.y * headPx }, stroke, width, ""));
    }
    ...
    return {
        shapes: shapes.join(""),
        text: textMarkup(env, index, "-label", circuitTextValue(declaration.label), labelAt, "middle", stroke, tokens.labelFontSizePx),
    };
}
/** One declaration's decoration and label, in the emitter's positional vocabulary. */
function emitDeclaration(declaration, index, env) {
    const tokens = env.tokens;
    switch (declaration.kind) {
        case "block": {
            if (!declaration.visible)
        ...
        case "polygon": {
            if (!declaration.visible || declaration.vertices.length < 3)
                return NO_MARKS;
            const vertices = declaration.vertices.map((name) => env.project(requirePoint(env.resolved.points, name)));
            return {
        ...
        case "point": {
            ...
            return {
                ...
                text: textMarkup(env, index, "-label", label, { x: at.x + POINT_LABEL_OFFSET_PX, y: at.y - POINT_LABEL_OFFSET_PX }, "start", tokens.bodyStroke, tokens.labelFontSizePx),
            };
        }
        case "line": {
            if (!declaration.visible)
            ...
            return {
                shapes: lineMarkup(env.project(line.from), env.project(line.to), tokens.bodyStroke, tokens.bodyStrokeWidthPx, declaration.style === "dashed" ? LINE_DASH_ARRAY : ""),
                text: "",
            };
        }
        case "force":
...
function describeFreeBody(block) {
    const parts = [];
    ...
    const counted = KIND_ORDER.filter((kind) => counts[kind] > 0).map((kind) => `${counts[kind]} ${KIND_LABEL[kind]}${counts[kind] === 1 ? "" : "s"}`);
    parts.push(counted.length === 0 ? "No declarations." : `Declarations: ${counted.join(", ")}.`);
    parts.push(block.scale === undefined
    ...
    for (const declaration of block.declarations) {
        switch (declaration.kind) {
            case "force": {
                ...
                forces.push(`force ${name} at ${attachmentText(declaration.at)} ${directionText(declaration.direction)}`);
                break;
            }
            case "moment":
                moments.push(`moment ${moments.length + 1} at ${attachmentText(declaration.at)} ${declaration.direction}`);
                break;
            case "axes":
                axes.push(`axes ${axes.length + 1} at ${attachmentText(declaration.at)} angle ${declaration.angle} degrees`);
                break;
            case "block":
            case "circle":
            case "particle":
            ...
            case "line":
                ...
                break;
            case "angle-mark":
                objects.push(`angle-mark ${declaration.vertex} between ${declaration.first} and ${declaration.third}`);
                break;
            case "dimension":
                objects.push(`dimension ${circuitTextValue(declaration.label)} from ${attachmentText(declaration.from)}` +
                    ` to ${attachmentText(declaration.to)}`);
                break;
        }
    }
    if (forces.length > 0)
        parts.push(`Forces in authored order: ${forces.join("; ")}.`);
    if (moments.length > 0)
        parts.push(`Moments in authored order: ${moments.join("; ")}.`);
    if (axes.length > 0)
        parts.push(`Axes in authored order: ${axes.join("; ")}.`);
    if (objects.length > 0) {
        parts.push("Bodies, points, lines, angle marks and dimensions in authored order: " +
            `${objects.join("; ")}.`);
    }
    return truncateDescription(parts.join(" "));
}
function freeBodyTitle(block, ordinal) {
...
export function renderFreeBodyFragment(block, context) {
    const theme = context.theme;
    if (theme === undefined) {
        throw new FreeBodyRenderError("The free-body renderer needs a resolved Theme; none was supplied.");
    }
    ...
    const bounds = authoredBounds === undefined
        ? undefined
        : {
    ...
    const fitted = Number.isFinite(extent.minX) &&
    ...
    if (!Number.isFinite(scale) || !(scale > 0)) {
        throw new FreeBodyRenderError(`The free-body canvas (${quantize(width)} × ${quantize(height)} px) cannot hold the diagram.`);
    }
    ...
    /* ---- declarations -------------------------------------------- */
    const env = { block, resolved, tokens, project, scale, base, axesHalf };
    const decoration = [];
    const labels = [];
    block.declarations.forEach((declaration, index) => {
        const emitted = emitDeclaration(declaration, index, env);
        if (emitted.shapes !== "") {
            decoration.push(`<g id="${base}-n-${quantize(index)}" class="aze-free-body-${declaration.kind}"` +
                ` aria-hidden="true">${emitted.shapes}</g>`);
        }
        if (emitted.text !== "")
            labels.push(emitted.text);
    });
    // An authored `bounds:` clips: an object outside it is clipped, never dropped.
    const clipId = `${base}-clip`;
    const clipCorner = authoredBounds === undefined ? undefined : project({ x: region.minX, y: region.maxY });
    const defs = clipCorner === undefined
        ? ""
    ...
    const svg = `<svg xmlns="${SVG_NAMESPACE}" viewBox="0 0 ${quantize(width)} ${quantize(height)}"` +
 ...
 */
export function freeBodyDependencyClosure() {
...
export const freeBodyHtmlBlockRenderer = Object.freeze({
...
//# sourceMappingURL=free-body-render.js.map