import { STATEMENT_PLUGIN_TYPE, STATEMENT_PLUGIN_VERSION, STATEMENT_BODY_SYNTAX_ID, STATEMENT_BODY_SYNTAX_VERSION, statementSourceSchema, statementDataSchema, } from "./statement-schemas.js";
import { createDiagnostic } from "./diagnostics.js";
import { parseCompositionHeader } from "./block-header.js";
import { escapeAttribute, numberingLabelHtml, renderInlineHtml, } from "./html-fragment.js";
import { rangeFromLineSlice, rangeFromLines, } from "./source-map.js";
/**
 ...
 */
export const STATEMENT_HTML_BLOCK_RENDERER_ID = "azeforge.statement.html/v1";
export const STATEMENT_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
...
export const STATEMENT_HEADER_FIELDS = Object.freeze(["kind"]);
...
const STATEMENT_KIND_WORDS = Object.freeze({
    theorem: "Theorem",
...
export const statementPlugin = Object.freeze({
...
function statementDiagnostic(code, message, range, sourceName, extra = {}) {
...
function lastEntryFor(entries, key) {
    let found;
...
export function parseStatementHeader(entries, blockRange, sourceName, parseCaption) {
    ...
    if (kindEntry === undefined) {
        diagnostics.push(statementDiagnostic("missing-field", "A statement header requires `kind:`.", blockRange, sourceName, {
            data: { field: "kind" },
            suggestion: `Use one of ${STATEMENT_KINDS.join(", ")}.`,
        }));
    }
    else if (STATEMENT_KINDS.includes(kindEntry.value)) {
        statementKind = kindEntry.value;
    }
    else {
        diagnostics.push(statementDiagnostic("unknown-kind", `Statement kind "${kindEntry.value}" is not supported.`, kindEntry.range, sourceName, {
 ...
 */
function readLiteralContent(bodyLines, start, propertyIndent) {
    ...
    return {
        lines: content,
        contentRange: first === undefined || last === undefined
            ? undefined
 ...
 */
export function parseStatementBody(args) {
    ...
    let textField;
    let proofField;
    const bodyRange = bodyLines.length === 0
        ? blockRange
        : rangeFromLines(bodyLines[0], bodyLines[bodyLines.length - 1]);
    let bodyIndent;
    let cursor = 0;
    // One fault reports once: a rejected declaration still consumes its content.
    const nextAfterField = (index, propertyIndent, multiline) => multiline
    ...
    while (cursor < bodyLines.length) {
        ...
        if (tabbed) {
            diagnostics.push(statementDiagnostic("invalid-field", "Statement body declarations indent with spaces only, two per structural level.", rangeFromLines(line, line), sourceName));
            cursor += 1;
            continue;
        }
        ...
        if (declaration === null || columns !== bodyIndent) {
            diagnostics.push(statementDiagnostic("unknown-field", "Statement body declarations use the shared `key:` form.", rangeFromLineSlice(line, columns, line.text.length), sourceName));
            cursor += 1;
            continue;
        }
        ...
        const valueRange = rangeFromLineSlice(line, valueStart, valueStart + value.length);
        if (key !== "text" && key !== "proof") {
            diagnostics.push(statementDiagnostic("unknown-field", `Statement body field "${key}" is not supported.`, keyRange, sourceName, { data: { field: key }, suggestion: "Use text or proof." }));
        ...
        if (previous !== undefined) {
            diagnostics.push(statementDiagnostic("duplicate-field", `Statement body field "${key}" is declared twice.`, keyRange, sourceName, {
                data: { field: key },
                relatedLocations: [
                    relatedLocation(previous.declarationRange, `"${key}" was first declared here.`, sourceName),
                ],
            }));
            cursor = nextAfterField(cursor, columns, value === MULTILINE_MARKER);
            continue;
        ...
        if (value !== MULTILINE_MARKER) {
            if (value.length === 0) {
                ...
                diagnostics.push(statementDiagnostic("invalid-field", `Statement field "${key}" is a \`|\` multiline Markdown field.`, valueRange, sourceName, { data: { field: key } }));
                cursor = nextAfterField(cursor, columns, value === MULTILINE_MARKER);
                continue;
            }
        }
    ...
    if (textField === undefined) {
        if (!textDeclared) {
            diagnostics.push(statementDiagnostic("missing-text", "A statement requires a `text:` field.", bodyRange, sourceName, {
                data: { field: "text" },
        ...
        diagnostics.push(statementDiagnostic("missing-text", "Statement `text:` must contain Markdown content.", textField.valueRange, sourceName, {
            data: { field: "text" },
            suggestion: "Add the statement as a `text: |` Markdown field.",
        }));
    }
    if (proofField !== undefined && proofField.empty) {
        diagnostics.push(statementDiagnostic("empty-proof", "Statement `proof:` is present but empty.", proofField.contentRange ?? proofField.valueRange, sourceName, {
            data: { field: "proof" },
            suggestion: "Remove the empty `proof:` field, or author the proof.",
        }));
    }
    for (const authored of [textField, proofField]) {
        if (authored === undefined || authored.empty)
            continue;
        const count = markdownChars(authored.lines);
        if (count > MAX_STATEMENT_MARKDOWN_CHARS) {
            diagnostics.push(statementDiagnostic("limit-exceeded", `A statement Block's ${authored.key} Markdown ${count} characters exceeds ${MAX_STATEMENT_MARKDOWN_CHARS}.`, authored.contentRange ?? authored.valueRange, sourceName, {
                data: {
    ...
    if (text === undefined)
        return { diagnostics };
    const proof = proofField === undefined ? undefined : parseBlocks(proofField.lines);
    if (proofField !== undefined && proof === undefined)
        return { diagnostics };
    return {
        diagnostics,
        body: {
 ...
 */
export function renderStatementFragment(block, context) {
    ...
    const figcaption = labelled || hasCaption
        ? `<figcaption>${prefix}${hasCaption ? ` ${caption}` : ""}</figcaption>`
        : "";
    const attributes = (block.id === undefined
        ? ""
    ...
    const text = context.renderBlocks(block.text);
    const proof = block.proof === undefined
        ? ""
        : `<div class="aze-statement-proof">${context.renderBlocks(block.proof)}<span class="aze-statement-qed">&#9633;</span></div>`;
    return `<figure class="aze-statement"${attributes}>${figcaption}${text}${proof}</figure>`;
}
export const statementHtmlBlockRenderer = Object.freeze({
