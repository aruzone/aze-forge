...
function locationOrder(left, right) {
    ...
    if (leftLocation === undefined || rightLocation === undefined)
        return 0;
    const sourceOrder = compareText(leftLocation.source ?? "", rightLocation.source ?? "");
    if (sourceOrder !== 0)
        return sourceOrder;
...
function containingBlock(diagnostic, blockRanges) {
    const range = diagnostic.location?.range;
    if (range === undefined)
        return undefined;
    return blockRanges.find((blockRange) => blockRange.start.offset <= range.start.offset &&
...
export function normalizeAndLimitDiagnostics(phases, limits = DEFAULT_DIAGNOSTIC_LIMITS, blockRanges = []) {
    const seen = new Set();
    const retained = [];
    const omitted = [];
    ...
    for (const phase of phases) {
        const ordered = phase
            ...
            if (leftRange?.start.offset === rightRange?.start.offset &&
                leftRange?.end.offset === rightRange?.end.offset) {
                const severity = SEVERITY_ORDER[left.diagnostic.severity] -
    ...
    const willTruncate = omitted.length > 0 || retained.length > limits.perDocument;
    ...
    for (const diagnostic of omitted) {
        ...
        const block = containingBlock(diagnostic, blockRanges);
        if (block !== undefined)
            omittedBlocks.add(block);
    }
    retained.push(createDiagnostic("azeforge.diagnostics#truncated", highestSeverity(omitted), "Additional diagnostics were omitted because the configured limit was reached.", {
        data: {
            omittedBySeverity,
