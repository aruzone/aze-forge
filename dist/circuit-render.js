...
function textElement(value, x, y, size = 12) {
...
function portOffsets(component) {
    const terminals = component.terminals;
    const inputRows = (names, x) => names.map((terminal, index) => ({
        terminal, x, y: (index - (names.length - 1) / 2) * 16,
    }));
    const side = (name, x, y = 0) => ({ terminal: name, x, y });
    switch (component.kind) {
        ...
        case "not":
        case "buffer": return [side("in", -66), side("out", 66)];
        case "mux-2to1":
        case "mux-4to1":
        ...
        case "voltage-source":
        case "current-source": return [side("positive", 0, -46), side("negative", 0, 46)];
        ...
        case "diode":
        case "led":
        case "switch": return [side(terminals[0], -66), side(terminals[1], 66)];
    }
}
function terminalBoundaries(component, convention, ports) {
    const side = (terminal, x, y = 0) => ({ terminal, x, y });
    const inputBoundary = (port) => {
        if (convention === "iec" || component.kind === "and" || component.kind === "nand")
            return { ...port, x: -48 };
    ...
    switch (component.kind) {
        ...
        case "voltage-source":
        case "current-source": return [side("positive", 0, -25), side("negative", 0, 25)];
        case "digital-input": return [side("out", 28)];
        case "digital-output": return [side("in", -28)];
        case "resistor":
        case "inductor": return [side(component.terminals[0], -32), side(component.terminals[1], 32)];
        case "capacitor": return [side(component.terminals[0], -8), side(component.terminals[1], 8)];
        case "diode":
        case "led": return [side(component.terminals[0], -24), side(component.terminals[1], 28)];
...
function body(component, convention) {
    ...
    switch (kind) {
        case "not":
        case "buffer": return `<path d="M-48 -26L48 0L-48 26Z"/>${bubble}`;
        case "mux-2to1":
        case "mux-4to1": return `<rect x="-48" y="-26" width="96" height="52"/>${textElement(kind === "mux-2to1" ? "2:1 MUX" : "4:1 MUX", 0, 4, 10)}`;
...
export function renderCircuitFragment(block, _context) {
    ...
    const wires = block.relations.map((relation, index) => {
        ...
        if (port === undefined || node === undefined)
            return "";
    ...
    const annotations = block.annotations.map((annotation, index) => {
        if (annotation.kind === "voltage-label") {
            const positive = nodePositions.get(annotation.positive);
            const negative = nodePositions.get(annotation.negative);
...
export const circuitHtmlBlockRenderer = Object.freeze({
...
//# sourceMappingURL=circuit-render.js.map