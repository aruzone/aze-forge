...
import { TEX_RENDERER_PROTOCOL, TEX_RENDER_REQUEST_MAX_BYTES, TEX_RENDER_RESPONSE_MAX_BYTES, } from "./model.js";
...
const ADAPTER_ERROR_CATEGORIES = {
...
export class TexAdapterCancelled extends Error {
    name = "TexAdapterCancelled";
    constructor() {
        super("The TeX renderer batch was cancelled.");
    }
}
export function buildTexRenderRequest(blocks) {
    const figures = blocks.map((block, index) => ({
        index,
...
export async function runTexRendererBatch(renderer, request, options) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        child.stderr.on("data", (chunk) => {
            ...
            if (stderrBytes > STDERR_CAPTURE_MAX_BYTES)
                return;
            stderr.push(chunk);
        });
...
function classifyExit(code, signal, log) {
    if (signal !== null)
        return "timeout";
    if (code === 125)
 ...
 */
function validateTexRenderResponse(value, request, rendererIdentity) {
...
function validateTexRenderResult(value, figureCount) {
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        throw new TexAdapterFailure("protocol-invalid");
    }
    const record = value;
...
function validateTexRenderDiagnostic(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        throw new TexAdapterFailure("protocol-invalid");
    }
    const record = value;
    ...
    const diagnostic = { code: record.code, message: record.message };
 ...
 */
function validateBodyLocationInBody(location, body) {
...
function validateTexBodyLocation(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        throw new TexAdapterFailure("protocol-invalid");
    }
    const record = value;
    ...
    if (record.endLine !== undefined || record.endColumn !== undefined) {
        if (typeof record.endLine !== "number" ||
...
export function texResultCategory(result) {
 ...
 */
export function texBodyRange(block, location, lines) {
    const bodyLineCount = block.body.split("\n").length;
    const firstLine = block.range.end.line - bodyLineCount;
    const lineAt = (offset) => lines[firstLine - 1 + offset];
...
//# sourceMappingURL=tex-adapter.js.map