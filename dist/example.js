import { EXAMPLE_PLUGIN_TYPE, EXAMPLE_PLUGIN_VERSION, EXAMPLE_BODY_SYNTAX_ID, EXAMPLE_BODY_SYNTAX_VERSION, exampleSourceSchema, exampleDataSchema, } from "./example-schemas.js";
 ...
 */
export const EXAMPLE_HTML_BLOCK_RENDERER_ID = "azeforge.example.html/v1";
export const EXAMPLE_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
...
export const examplePlugin = Object.freeze({
...
export function parseExampleHeader(entries, _blockRange, sourceName, parseCaption) {
...
export const REGISTERED_FIELDS = Object.freeze([
    "problem",
    "givens",
    "steps",
    "result",
...
export const STEP_FIELDS = Object.freeze(["text"]);
...
function sliced(line, startIndex, endIndex) {
    const text = line.text.slice(startIndex, endIndex);
    const startOffset = line.startOffset + Buffer.byteLength(line.text.slice(0, startIndex), "utf8");
    return {
        number: line.number,
        text,
...
function error(ctx, code, message, range, details = {}) {
    ctx.diagnostics.push(createDiagnostic(`${NAMESPACE}#${code}`, "error", message, {
        location: ctx.sourceName === undefined
            ? { range }
...
function indentationFault(ctx, line) {
    error(ctx, "unknown-field", "Declarations indent with spaces only, two per structural level.", rangeFromLines(line, line), { data: { field: declarationKey(line.text) } });
}
...
function skipEntry(ctx, start, parentIndent) {
    let index = start;
    while (index < ctx.bodyLines.length) {
        ...
        if (contentIndent(line.text) <= parentIndent)
            break;
        index += 1;
    }
    return index;
 ...
 */
function readLiteral(ctx, start, propertyIndent) {
    const lines = [];
    let index = start;
    let first;
    let last;
    while (index < ctx.bodyLines.length) {
        ...
        if (indent <= propertyIndent)
            break;
        lines.push(sliced(line, Math.min(indent, propertyIndent + 2), line.text.length));
        first ??= line;
        last = line;
        index += 1;
    }
...
function readField(ctx, start, indent) {
    ...
    if (value === "|") {
        ...
        return {
            ...
            line,
            range: lineRange,
            contentRange: literal.first === undefined || literal.last === undefined
                ? lineRange
...
function readGivens(ctx, start, itemIndent) {
    const givens = [];
    let index = start;
    while (index < ctx.bodyLines.length) {
        const line = ctx.bodyLines[index];
        ...
        if (indent < itemIndent)
            break;
        if (indent > itemIndent) {
            error(ctx, "unknown-field", `Expected a \`- \` collection item at indentation ${itemIndent}.`, rangeFromLines(line, line), { data: { field: declarationKey(line.text) } });
            index += 1;
            continue;
        }
        const item = ITEM.exec(line.text.slice(itemIndent));
        if (item === null) {
            error(ctx, "unknown-field", "Expected a `- ` collection item.", rangeFromLines(line, line), { data: { field: declarationKey(line.text) } });
            index += 1;
            continue;
        }
        const rawItem = item[2] ?? "";
        const itemRange = rangeFromLines(line, line);
        const valueFrom = itemIndent + item[0].length - rawItem.length +
            (rawItem.length - rawItem.trimStart().length);
        const raw = rawItem.trim();
        if (raw === "") {
            error(ctx, "empty-value", "A `givens:` item must not be empty.", itemRange, { data: { field: "givens" } });
            index += 1;
            continue;
        }
        const decoded = decodeText(raw);
        if (decoded.invalid) {
            error(ctx, "invalid-text", "A `givens:` item is not a well-formed quoted value.", rangeFromLineSlice(line, valueFrom, valueFrom + raw.length), { data: { field: "givens" } });
            index += 1;
            continue;
        }
        givens.push({
...
function readSteps(ctx, start, itemIndent) {
    const steps = [];
    let index = start;
    while (index < ctx.bodyLines.length) {
        const line = ctx.bodyLines[index];
        ...
        if (indent < itemIndent)
            break;
        if (indent > itemIndent) {
            error(ctx, "unknown-field", `Expected a \`- text:\` step at indentation ${itemIndent}.`, rangeFromLines(line, line), { data: { field: declarationKey(line.text) } });
            index += 1;
            continue;
        }
        const itemRange = rangeFromLines(line, line);
        ...
        if (item === null) {
            error(ctx, "unknown-field", "Each example step must open with `- text:`.", itemRange, { data: { field: declarationKey(line.text) } });
        ...
        const value = rawValue.trim();
        if (key !== "text") {
            error(ctx, "unknown-field", `Step field "${key}" is not a supported example step field.`, itemRange, {
                data: { field: key },
        ...
        const valueFrom = itemIndent + item[0].length - rawValue.length +
 ...
 */
function markdownBlocks(ctx, field, label) {
    const range = field.contentRange ?? field.range;
    if (field.text.trim().length === 0) {
        error(ctx, "empty-value", `The example's \`${label}:\` field must not be empty.`, range, { data: { field: label } });
        return undefined;
    }
    const length = [...field.text].length;
    if (length > MAX_EXAMPLE_MARKDOWN_CHARS) {
        error(ctx, "limit-exceeded", `This Block's ${label} length ${length} exceeds ${MAX_EXAMPLE_MARKDOWN_CHARS}.`, range, {
            data: {
...
export function parseExampleBody(args) {
    const ctx = {
    ...
    while (index < ctx.bodyLines.length) {
        ...
        if (indent > base) {
            error(ctx, "unknown-field", `Expected a \`key: value\` declaration at indentation ${base}.`, rangeFromLines(line, line), { data: { field: declarationKey(line.text) } });
            index += 1;
            continue;
        }
        const read = readField(ctx, index, base);
        if (read === undefined) {
            error(ctx, "unknown-field", "Expected a `key: value` declaration.", rangeFromLines(line, line), { data: { field: declarationKey(line.text) } });
            index += 1;
            continue;
        }
        index = read.next;
        if (!REGISTERED_FIELDS.includes(read.key)) {
            error(ctx, "unknown-field", `Example field "${read.key}" is not supported.`, read.range, {
                data: { field: read.key },
                suggestion: didYouMean(read.key, REGISTERED_FIELDS) ??
        ...
        if (read.key === "givens") {
            if (givensField !== undefined) {
                error(ctx, "duplicate-field", `Field "givens" is declared twice.`, read.range, { data: { field: "givens" } });
                continue;
            }
            givensField = read;
            if (read.value !== "") {
                error(ctx, "unknown-field", "`givens:` requires a `- ` collection of single-line items.", read.range, { data: { field: "givens" } });
                index = skipEntry(ctx, index, base);
                continue;
            }
        ...
        if (read.key === "steps") {
            if (stepsField !== undefined) {
                error(ctx, "duplicate-field", `Field "steps" is declared twice.`, read.range, { data: { field: "steps" } });
                continue;
            }
            stepsField = read;
            if (read.value !== "") {
                error(ctx, "unknown-field", "`steps:` requires a `- text:` collection.", read.range, { data: { field: "steps" } });
                index = skipEntry(ctx, index, base);
                continue;
            }
        ...
        if (existing !== undefined) {
            error(ctx, "duplicate-field", `Field "${read.key}" is declared twice.`, read.range, { data: { field: read.key } });
            continue;
        }
        if (read.key === "problem")
            problem = read;
    ...
    if (problem === undefined) {
        error(ctx, "missing-problem", "The example requires a `problem:` section.", ctx.blockRange, {
            data: { field: "problem" },
            suggestion: "Add a `problem:` field stating what the example works through.",
        });
    }
    else {
    ...
    if (givenItems.length > MAX_EXAMPLE_GIVENS) {
        ...
        error(ctx, "limit-exceeded", `This Block's given count ${givenItems.length} exceeds ${MAX_EXAMPLE_GIVENS}.`, offender?.range ?? ctx.blockRange, {
            data: {
                subject: "given count",
    ...
    const steps = [];
    if (stepsField === undefined) {
        error(ctx, "missing-steps", "The example requires a `steps:` collection with at least one `- text:` step.", ctx.blockRange, {
            data: { field: "steps" },
            suggestion: "Add a `steps:` collection of `- text:` records.",
        });
    }
    else if (stepItems.length === 0) {
        error(ctx, "missing-steps", "The example requires a `steps:` collection with at least one `- text:` step.", stepsField.range, { data: { field: "steps" } });
    }
    else if (stepItems.length > MAX_EXAMPLE_STEPS) {
        const offender = stepItems[MAX_EXAMPLE_STEPS];
        error(ctx, "limit-exceeded", `This Block's step count ${stepItems.length} exceeds ${MAX_EXAMPLE_STEPS}.`, offender?.range ?? stepsField.range, {
            data: {
                subject: "step count",
        ...
        for (const item of stepItems) {
            if (item.text.trim().length === 0) {
                error(ctx, "empty-step", "An example step requires non-empty Markdown content.", item.contentRange, { suggestion: "Give the step content or remove the empty step." });
                continue;
            }
            const length = [...item.text].length;
            if (length > MAX_EXAMPLE_MARKDOWN_CHARS) {
                error(ctx, "limit-exceeded", `This Block's step length ${length} exceeds ${MAX_EXAMPLE_MARKDOWN_CHARS}.`, item.contentRange, {
                    data: {
                        subject: "step length",
                        count: length,
                        limit: MAX_EXAMPLE_MARKDOWN_CHARS,
    ...
    const resultBlocks = result === undefined ? undefined : markdownBlocks(ctx, result, "result");
 ...
 */
export function renderExampleFragment(block, context) {
    const id = block.id === undefined ? "" : ` data-example-id="${escapeHtml(block.id)}"`;
    ...
    const figcaption = caption === "" && numberLabel === ""
        ? ""
        : `<figcaption>${numberLabel}${caption}</figcaption>`;
    const problem = `<section class="aze-example-problem"><p class="aze-example-label">Problem</p>${context.renderBlocks(block.problem)}</section>`;
    const givens = block.givens.length === 0
        ? ""
        : `<ul class="aze-example-givens">${block.givens
            .map((given) => `<li>${escapeHtml(given)}</li>`)
            .join("")}</ul>`;
    const steps = block.steps.length === 0
        ? ""
        : `<ol class="aze-example-steps">${block.steps
            .map((step) => `<li class="aze-example-step">${context.renderBlocks(step.text)}</li>`)
            .join("")}</ol>`;
    const result = block.result === undefined || block.result.length === 0
        ? ""
        : `<section class="aze-example-result"><p class="aze-example-label">Result</p>${context.renderBlocks(block.result)}</section>`;
    return `<figure class="aze-example"${id}${number}>${figcaption}${problem}${givens}${steps}${result}</figure>`;
}
export const exampleHtmlBlockRenderer = Object.freeze({
