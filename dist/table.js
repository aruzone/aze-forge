import { TABLE_PLUGIN_TYPE, TABLE_PLUGIN_VERSION, TABLE_BODY_SYNTAX_ID, TABLE_BODY_SYNTAX_VERSION, tableSourceSchema, tableDataSchema, } from "./table-schemas.js";
...
export const TABLE_HTML_BLOCK_RENDERER_ID = "azeforge.table.html/v1";
export const TABLE_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
...
export const tablePlugin = Object.freeze({
 ...
 */
function renderExponentHtml(text) {
 ...
 */
function renderTypedTableFragment(block, context) {
    const data = block.data;
    const ordinal = context.ordinal ?? 0;
    const base = `aze-table-${ordinal}`;
    const label = block.id === undefined ? "" : ` data-table-id="${escapeHtml(block.id)}"`;
    ...
    const headerId = (column) => ` id="${base}-h-${escapeHtml(column.key)}"`;
    const columnName = (column) => column.name === undefined || column.name === ""
        ? column.key
        : column.name;
    ...
    if (data.groups !== undefined && data.groups.length > 0) {
        ...
        while (index < data.columns.length) {
            ...
            const group = data.groups.find((candidate) => candidate.columns.includes(column.key));
            const members = group?.columns ?? [];
            const colspan = members.length;
            const first = byKey.get(members[0]);
    ...
    const headerCells = data.columns
        ...
        .map((column) => `<th scope="col"${headerId(column)}>${renderExponentHtml(columnName(column))}</th>`)
        .join("");
    ...
    const bodyRows = data.rows
        ...
        const cells = data.columns
            ...
            const unit = column.unit === undefined || column.unit === ""
                ? ""
    ...
    const describedBy = caption === "" ? "" : ` aria-describedby="${base}-caption"`;
...
export function renderTableFragment(block, context) {
