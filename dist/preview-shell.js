 ...
 * update in lockstep.
 */
export const PREVIEW_SSE_SCRIPT = `"use strict";\n` +
    `const source = new EventSource("/events");\n` +
    `source.addEventListener("reload", () => {\n` +
    `  window.location.reload();\n` +
    `});\n`;
export function previewScriptHash() {
...
const ARTIFACT_CSP_META = `<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src data:; font-src data:">`;
function previewCspMeta() {
    return `<meta http-equiv="Content-Security-Policy" content="${previewContentSecurityPolicy()}">`;
}
function previewScriptTag() {
 ...
 */
export function wrapPreviewShell(artifactHtml) {
...
export function renderDiagnosticsPage(diagnostics, sourceLabel) {
    const items = diagnostics
        .map((diagnostic) => `<li><strong>${escapeHtml(diagnostic.severity)} [${escapeHtml(diagnostic.code)}]</strong> ` +
...
export function renderStartingPage(sourceLabel) {
    return (`<!doctype html>\n` +
        `<html lang="und"><head><meta charset="utf-8">` +
        `<meta name="viewport" content="width=device-width,initial-scale=1">` +
        `${previewCspMeta()}` +
 ...
 */
export function extractServedAssets(artifactHtml) {
    ...
    for (let match = pattern.exec(artifactHtml); match !== null; match = pattern.exec(artifactHtml)) {
        ...
        if (!assets.has(token))
            assets.set(token, { token, mediaType, bytes });
    }
    return [...assets.values()];
}
//# sourceMappingURL=preview-shell.js.map