...
function isSourcePosition(value) {
    ...
    return (Number.isInteger(value.line) &&
        value.line >= 1 &&
...
function isSourceRange(value) {
    if (!isObjectRecord(value) || !hasOnlyKeys(value, ["start", "end"]))
        return false;
    if (!isSourcePosition(value.start) || !isSourcePosition(value.end))
        return false;
...
function hasValidCommonBlockFields(value, allowed) {
...
function isInlineNode(value) {
    ...
    if (value.kind === "emphasis" || value.kind === "strong") {
        return (hasOnlyKeys(value, ["kind", "children"]) &&
            ...
            value.children.every((child) => isInlineNode(child)));
    }
    if (value.kind === "break") {
        return hasOnlyKeys(value, ["kind"]);
    }
    if (value.kind === "link") {
        return (hasOnlyKeys(value, ["kind", "href", "children", "title", "range"]) &&
    ...
    if (value.kind === "reference") {
        return (hasOnlyKeys(value, ["kind", "target", "form", "locator", "range", "resolved"]) &&
    ...
    if (value.kind === "referenceGroup") {
        return (hasOnlyKeys(value, ["kind", "targets", "range", "resolved"]) &&
            Array.isArray(value.targets) &&
            value.targets.length > 0 &&
            value.targets.every((target) => isInlineNode(target)) &&
            (value.range === undefined || isSourceRange(value.range)));
    }
    if (value.kind === "footnote") {
        return (hasOnlyKeys(value, ["kind", "label", "range", "resolved"]) &&
            typeof value.label === "string" &&
            value.label.length > 0 &&
            (value.range === undefined || isSourceRange(value.range)));
    }
    if (value.kind === "image") {
        return (hasOnlyKeys(value, ["kind", "src", "alt", "title", "range"]) &&
    ...
    return (Array.isArray(value) &&
        value.every((run) => {
            ...
            if (run.kind === "text" ||
                ...
                return (hasOnlyKeys(run, ["kind", "value"]) && typeof run.value === "string");
            }
            return (run.kind === "quantity" &&
...
const DIAGRAM_PORT_SIDES = {
    left: true,
    right: true,
    top: true,
    bottom: true,
};
function isDiagramLabel(value) {
    return Array.isArray(value) && value.length >= 1 && value.every((line) => isCircuitText(line));
}
function isDiagramPort(value) {
    if (!isObjectRecord(value) || !hasOnlyKeys(value, ["name", "side", "range"])) {
        return false;
    }
    return (typeof value.name === "string" &&
        (value.side === undefined || DIAGRAM_PORT_SIDES[value.side] === true) &&
        isSourceRange(value.range));
}
function isDiagramEndpoint(value) {
...
function isDiagramDeclaration(value) {
    if (!isObjectRecord(value))
        return false;
    if (value.kind === "node") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "shape", "parent", "ports", "range"]) &&
            typeof value.name === "string" &&
            (value.label === undefined || isDiagramLabel(value.label)) &&
            DIAGRAM_SHAPES[value.shape] === true &&
    ...
    if (value.kind === "group") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "parent", "range"]) &&
            typeof value.name === "string" &&
            (value.label === undefined || isDiagramLabel(value.label)) &&
            (value.parent === undefined || typeof value.parent === "string") &&
            isSourceRange(value.range));
    ...
    if (value.kind === "edge") {
        return (hasOnlyKeys(value, ["kind", "from", "to", "label", "direction", "range"]) &&
...
function isDiagramBlock(value) {
    return (hasOnlyKeys(value, [
        ...
        DIAGRAM_MODES[value.mode] === true &&
        DIAGRAM_FLOWS[value.flow] === true &&
        (value.id === undefined || typeof value.id === "string") &&
...
function isControlDeclaration(value) {
    if (!isObjectRecord(value))
        return false;
    if (value.kind === "block") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "tf", "range"]) &&
            typeof value.name === "string" &&
            value.name.length > 0 &&
    ...
    if (value.kind === "sum") {
        return (hasOnlyKeys(value, ["kind", "name", "signs", "range"]) &&
            typeof value.name === "string" &&
            value.name.length > 0 &&
            Array.isArray(value.signs) &&
            value.signs.length >= 1 &&
            value.signs.every((sign) => CONTROL_SIGNS[sign] === true) &&
            isSourceRange(value.range));
    }
    if (value.kind === "input" || value.kind === "output") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "range"]) &&
            typeof value.name === "string" &&
            value.name.length > 0 &&
    ...
    if (value.kind === "edge") {
        return (hasOnlyKeys(value, ["kind", "from", "to", "label", "range"]) &&
...
function isFreeBodyAttachment(value) {
    if (!isObjectRecord(value))
        return false;
    if (value.kind === "point") {
    ...
    return (value.kind === "coordinates" &&
        hasOnlyKeys(value, ["kind", "x", "y"]) &&
        isCanonicalDecimal(value.x) &&
        isCanonicalDecimal(value.y));
}
function isFreeBodyDirection(value) {
    if (!isObjectRecord(value))
        return false;
    if (value.kind === "angle") {
        return hasOnlyKeys(value, ["kind", "degrees"]) && isCanonicalDecimal(value.degrees);
    }
    return ((value.kind === "parallel-to" || value.kind === "perpendicular-to") &&
        hasOnlyKeys(value, ["kind", "line"]) &&
        typeof value.line === "string" &&
        value.line.length > 0);
}
function isFreeBodyDeclaration(value) {
    if (!isObjectRecord(value))
        return false;
    switch (value.kind) {
        case "block":
            return (hasOnlyKeys(value, ["kind", "name", "x", "y", "width", "height", "angle", "visible", "range"]) &&
                typeof value.name === "string" &&
                value.name.length > 0 &&
                isCanonicalDecimal(value.x) &&
                isCanonicalDecimal(value.y) &&
                ...
                isSourceRange(value.range));
        case "circle":
            return (hasOnlyKeys(value, ["kind", "name", "x", "y", "radius", "visible", "range"]) &&
                typeof value.name === "string" &&
                value.name.length > 0 &&
                isCanonicalDecimal(value.x) &&
                isCanonicalDecimal(value.y) &&
                ...
                isSourceRange(value.range));
        case "particle":
            return (hasOnlyKeys(value, ["kind", "name", "x", "y", "visible", "range"]) &&
                typeof value.name === "string" &&
                value.name.length > 0 &&
                isCanonicalDecimal(value.x) &&
                isCanonicalDecimal(value.y) &&
                ...
                isSourceRange(value.range));
        case "polygon":
            return (hasOnlyKeys(value, ["kind", "name", "vertices", "visible", "range"]) &&
                typeof value.name === "string" &&
                value.name.length > 0 &&
                ...
                isSourceRange(value.range));
        case "point":
            return (hasOnlyKeys(value, ["kind", "name", "label", "x", "y", "visible", "range"]) &&
                typeof value.name === "string" &&
                value.name.length > 0 &&
                isCanonicalDecimal(value.x) &&
                isCanonicalDecimal(value.y) &&
                ...
                isSourceRange(value.range));
        case "line":
            return (hasOnlyKeys(value, ["kind", "name", "from", "to", "visible", "style", "range"]) &&
                typeof value.name === "string" &&
                value.name.length > 0 &&
                typeof value.from === "string" &&
                typeof value.to === "string" &&
                ...
                isSourceRange(value.range));
        case "force":
            return (hasOnlyKeys(value, ["kind", "at", "direction", "magnitude", "length", "label", "range"]) &&
                ...
                isSourceRange(value.range));
        case "moment":
            return (hasOnlyKeys(value, ["kind", "at", "direction", "label", "range"]) &&
                isFreeBodyAttachment(value.at) &&
                (value.direction === "cw" || value.direction === "ccw") &&
                (value.label === undefined || isCircuitText(value.label)) &&
                isSourceRange(value.range));
        case "axes":
            return (hasOnlyKeys(value, ["kind", "at", "angle", "xLabel", "yLabel", "range"]) &&
                isFreeBodyAttachment(value.at) &&
                isCanonicalDecimal(value.angle) &&
                ...
                isSourceRange(value.range));
        case "angle-mark":
            return (hasOnlyKeys(value, ["kind", "first", "vertex", "third", "label", "range"]) &&
...
function isFreeBodyBounds(value) {
    return (isObjectRecord(value) &&
...
function isFreeBodyBlock(value) {
    return (hasOnlyKeys(value, [
        "kind",
        "pluginVersion",
        "range",
...
function isCircuitComponent(value) {
    if (!isObjectRecord(value))
        return false;
...
function isCircuitAnnotation(value) {
    if (!isObjectRecord(value))
        return false;
    if (value.kind === "voltage-label") {
        return (hasOnlyKeys(value, ["kind", "positive", "negative", "range"]) &&
...
function isTimingIntervalState(value) {
    return (value === "low" ||
        value === "high" ||
        value === "unknown" ||
        value === "impedance" ||
...
function isTimingInterval(value) {
    return (isObjectRecord(value) &&
...
function isTimingBlock(value) {
    return (hasOnlyKeys(value, [
        ...
        value.groups.every((group) => isObjectRecord(group) &&
        ...
        value.markers.every((marker) => isObjectRecord(marker) &&
        ...
        value.arrows.every((arrow) => isObjectRecord(arrow) &&
...
const SEQUENCE_PARTICIPANT_KINDS = { participant: true, actor: true };
const SEQUENCE_MESSAGE_FORMS = { sync: true, async: true, return: true };
const CARDINALITY_VALUES = {
    "one": true,
    "zero-or-one": true,
    "many": true,
    "one-or-many": true,
};
const ENTITY_KEY_VALUES = { primary: true, foreign: true, unique: true };
const CLASS_VISIBILITY_VALUES = {
    public: true,
    private: true,
    protected: true,
    package: true,
...
function isModelsHeader(value, kind, keys) {
...
function isSequenceBlock(value) {
    return (isModelsHeader(value, "sequence", [
        "kind",
        "range",
        "id",
        "number",
        ...
        value.participants.every((participant) => isObjectRecord(participant) &&
            hasOnlyKeys(participant, ["name", "kind", "label", "range"]) &&
            typeof participant.name === "string" &&
            SEQUENCE_PARTICIPANT_KINDS[participant.kind] === true &&
...
function isSequenceTimelineItem(value) {
    if (!isObjectRecord(value) || !isSourceRange(value.range))
        return false;
    if (value.kind === "message") {
        return (hasOnlyKeys(value, ["kind", "form", "from", "to", "text", "activate", "deactivate", "range"]) &&
            SEQUENCE_MESSAGE_FORMS[value.form] === true &&
            typeof value.from === "string" &&
            typeof value.to === "string" &&
...
function isStateScopedItem(value) {
    if (!isObjectRecord(value) || !isSourceRange(value.range) || typeof value.name !== "string")
        return false;
    if (value.kind === "state") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "states", "range"]) &&
...
function isStateBlock(value) {
    return (isModelsHeader(value, "state", [
        "kind",
        "range",
        "id",
        "number",
        ...
        value.items.every((item) => {
            if (isObjectRecord(item) && item.kind === "transition") {
                return (isSourceRange(item.range) &&
...
function isEntityAttribute(value) {
    return (isObjectRecord(value) &&
        hasOnlyKeys(value, ["name", "type", "keys", "optional", "reference", "range"]) &&
        typeof value.name === "string" &&
        (value.type === undefined || typeof value.type === "string") &&
        (value.keys === undefined ||
            (Array.isArray(value.keys) && value.keys.every((key) => ENTITY_KEY_VALUES[key] === true))) &&
...
function isEntityRelationshipEnd(value) {
    return (isObjectRecord(value) &&
        hasOnlyKeys(value, ["entity", "cardinality", "role", "range"]) &&
        typeof value.entity === "string" &&
        CARDINALITY_VALUES[value.cardinality] === true &&
        (value.role === undefined || typeof value.role === "string") &&
        isSourceRange(value.range));
}
function isEntityBlock(value) {
    return (isModelsHeader(value, "entity", [
        "kind",
        "range",
        "id",
        "number",
        ...
        Array.isArray(value.items) &&
        value.items.every((item) => {
            if (!isObjectRecord(item) || !isSourceRange(item.range))
                return false;
            if (item.kind === "entity") {
                return (hasOnlyKeys(item, ["kind", "name", "label", "attributes", "range"]) &&
            ...
            if (item.kind !== "relationship")
                return false;
            return (hasOnlyKeys(item, ["kind", "label", "first", "second", "range"]) &&
                (item.label === undefined || typeof item.label === "string") &&
                isEntityRelationshipEnd(item.first) &&
                isEntityRelationshipEnd(item.second));
        }));
}
function isClassClassifier(value) {
    if (value.kind === "class") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "abstract", "attributes", "operations", "range"]) &&
            typeof value.abstract === "boolean" &&
            Array.isArray(value.attributes));
    }
    if (value.kind === "interface") {
        return (hasOnlyKeys(value, ["kind", "name", "label", "operations", "range"]) &&
            Array.isArray(value.operations));
    }
    return false;
}
function isClassMember(value) {
    return (isObjectRecord(value) &&
        hasOnlyKeys(value, ["name", "type", "visibility", "static", "range"]) &&
        typeof value.name === "string" &&
        (value.type === undefined || typeof value.type === "string") &&
        (value.visibility === undefined || CLASS_VISIBILITY_VALUES[value.visibility] === true) &&
        typeof value.static === "boolean" &&
        isSourceRange(value.range));
}
function isClassOperation(value) {
    return (isObjectRecord(value) &&
        ...
        (value.parameters === undefined ||
            (Array.isArray(value.parameters) &&
                value.parameters.every((parameter) => isObjectRecord(parameter) &&
...
function isClassBlock(value) {
    return (isModelsHeader(value, "class", [
        "kind",
        "range",
        "id",
        "number",
        ...
        value.items.every((item) => {
            ...
            if (item.kind === "relationship") {
                return (hasOnlyKeys(item, [
                    ...
                    CLASS_RELATIONSHIP_FORMS[item.form] === true &&
                    typeof item.from === "string" &&
                    typeof item.to === "string" &&
                    (item.label === undefined || typeof item.label === "string") &&
                    (item.fromMultiplicity === undefined || CARDINALITY_VALUES[item.fromMultiplicity] === true) &&
                    (item.toMultiplicity === undefined || CARDINALITY_VALUES[item.toMultiplicity] === true));
            }
            if (!isClassClassifier(item))
                return false;
...
function isParsedBlock(value) {
    if (!isObjectRecord(value))
        return false;
    if (value.kind === "circuit") {
        return (hasOnlyKeys(value, ["kind", "range", "id", "number", "pluginVersion", "title", "description", "flow", "symbolConvention", "nodes", "components", "relations", "annotations"]) &&
            isSourceRange(value.range) &&
            (value.id === undefined || typeof value.id === "string") &&
            (value.number === undefined || typeof value.number === "boolean") &&
            value.pluginVersion === "1.0.0" &&
            ...
            value.nodes.every((node) => isObjectRecord(node) &&
    ...
    if (value.kind === "heading") {
        return (hasValidCommonBlockFields(value, ["kind", "level", "children", "range", "id"]) &&
    ...
    if (value.kind === "paragraph") {
        return (hasValidCommonBlockFields(value, ["kind", "children", "range", "id"]) &&
            hasInlineChildren(value.children));
    }
    ...
    if (value.kind === "blockquote") {
        return (hasValidCommonBlockFields(value, ["kind", "children", "range", "id"]) &&
            Array.isArray(value.children) &&
            value.children.every((child) => isParsedBlock(child)));
    }
    if (value.kind === "list") {
        return (hasValidCommonBlockFields(value, ["kind", "ordered", "items", "range", "id", "start"]) &&
            typeof value.ordered === "boolean" &&
            Array.isArray(value.items) &&
            value.items.every((item) => isObjectRecord(item) &&
                hasOnlyKeys(item, ["blocks", "range"]) &&
                Array.isArray(item.blocks) &&
                item.blocks.every((child) => isParsedBlock(child)) &&
                isSourceRange(item.range)) &&
            (value.start === undefined ||
                (Number.isInteger(value.start) && value.start >= 0)));
    }
    if (value.kind === "code") {
        return (hasValidCommonBlockFields(value, ["kind", "value", "range", "id", "language"]) &&
            typeof value.value === "string" &&
            (value.language === undefined || typeof value.language === "string"));
    }
    if (value.kind === "table") {
        return (hasValidCommonBlockFields(value, [
            "kind",
            "data",
            "range",
            "id",
        ...
        return (hasValidCommonBlockFields(value, ["kind", "variant", "children", "range", "id", "title", "pluginVersion"]) &&
            ...
            (value.title === undefined ||
                (Array.isArray(value.title) &&
                    value.title.every((node) => isInlineNode(node)))) &&
            value.pluginVersion === "1.0.0");
    }
    ...
    if (value.kind === "tex") {
        return (hasOnlyKeys(value, ["kind", "range", "id", "pluginVersion", "title", "description", "profile", "body"]) &&
            isSourceRange(value.range) &&
            (value.id === undefined || typeof value.id === "string") &&
            value.pluginVersion === "1.0.0" &&
    ...
    if (value.kind === "derivation") {
        return (hasOnlyKeys(value, [
            ...
            value.steps.every((step) => isObjectRecord(step) &&
                ...
                (step.annotation === undefined ||
                    (Array.isArray(step.annotation) &&
                        step.annotation.every((node) => isInlineNode(node))))) &&
            (value.number === undefined || typeof value.number === "boolean") &&
            (value.align === undefined ||
                value.align === "left" ||
                value.align === "center" ||
    ...
    if (value.kind === "invalid") {
        return (hasOnlyKeys(value, [
            ...
            value.diagnosticIndexes.every((index) => Number.isInteger(index) && index >= 0) &&
    ...
    if (value.kind === "algorithm") {
        return (hasValidCommonBlockFields(value, [
            ...
            Array.isArray(value.parameters) &&
            value.parameters.every((name) => typeof name === "string") &&
            Array.isArray(value.steps) &&
            value.steps.every(isAlgorithmStatement) &&
            (value.number === undefined || typeof value.number === "boolean") &&
            (value.numberLabel === undefined || typeof value.numberLabel === "string") &&
    ...
    if (value.kind === "example") {
        return (hasValidCommonBlockFields(value, [
            ...
            value.givens.every((item) => typeof item === "string") &&
            Array.isArray(value.steps) &&
            value.steps.every((step) => isObjectRecord(step) &&
    ...
    if (value.kind === "bibliography") {
        return (hasValidCommonBlockFields(value, [
            ...
            value.entries.every(isBibliographyEntry) &&
            (value.worksCited === undefined ||
                (Array.isArray(value.worksCited) &&
                    value.worksCited.every(isBibliographyEntry))) &&
            (value.number === undefined || typeof value.number === "boolean") &&
            (value.numberLabel === undefined || typeof value.numberLabel === "string") &&
            (value.caption === undefined || hasInlineChildren(value.caption)) &&
            typeof value.pluginVersion === "string");
    ...
    if (value.kind === "footnoteDefinition") {
        return (hasValidCommonBlockFields(value, ["kind", "label", "children", "range"]) &&
            typeof value.label === "string" &&
            value.label.length > 0 &&
            hasInlineChildren(value.children));
    }
...
function isLocator(value) {
    ...
    return (hasOnlyKeys(value, ["word", "value"]) &&
        typeof value.word === "string" &&
        words.includes(value.word) &&
        typeof value.value === "string" &&
...
function isTypedTableCell(value) {
    if (!isObjectRecord(value))
        return false;
    switch (value.kind) {
        case "prose":
        ...
        case "decimal":
            return hasOnlyKeys(value, ["kind", "value"]) && typeof value.value === "string";
        case "quantity":
            return (hasOnlyKeys(value, ["kind", "coefficient", "unit"]) &&
...
function isAlgorithmStatement(value) {
    if (!isObjectRecord(value) || !isSourceRange(value.range))
        return false;
    switch (value.kind) {
        case "assign":
            return (hasOnlyKeys(value, ["kind", "target", "index", "expression", "range"]) &&
                typeof value.target === "string" &&
                typeof value.expression === "string" &&
        ...
        case "if":
            return (hasOnlyKeys(value, [
                ...
                value.then.every(isAlgorithmStatement) &&
                Array.isArray(value.elseIf) &&
                value.elseIf.every((branch) => isObjectRecord(branch) &&
                    ...
                    branch.statements.every(isAlgorithmStatement)) &&
                (value.else === undefined ||
                    (Array.isArray(value.else) &&
                        value.else.every(isAlgorithmStatement))));
        case "for":
            return (hasOnlyKeys(value, [
                "kind",
                "variable",
                ...
                value.statements.every(isAlgorithmStatement));
        case "while":
            return (hasOnlyKeys(value, ["kind", "condition", "statements", "range"]) &&
                typeof value.condition === "string" &&
                Array.isArray(value.statements) &&
                value.statements.every(isAlgorithmStatement));
        case "return":
            return (hasOnlyKeys(value, ["kind", "expression", "range"]) &&
...
function isBibliographyEntry(value) {
    return (isObjectRecord(value) &&
        hasOnlyKeys(value, [
            "key",
            "entryType",
        ...
        value.authors.every((author) => isObjectRecord(author) &&
            hasOnlyKeys(author, ["name", "family"]) &&
            typeof author.name === "string" &&
            (author.family === undefined || typeof author.family === "string")) &&
        ["year", "venue", "publisher", "edition", "pages", "url", "doi", "note"].every((field) => value[field] === undefined || typeof value[field] === "string"));
}
function isDocumentComposition(value) {
    return (isObjectRecord(value) &&
        ...
        value.endnotes.every((entry) => isObjectRecord(entry) &&
...
function isMetadata(value) {
    if (!isObjectRecord(value) ||
        ...
        !Object.entries(value.extensions).every(([key, extension]) => /^x-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(key) && isJsonValue(extension)) ||
        (value.title !== undefined && typeof value.title !== "string") ||
    ...
    return (Array.isArray(value.outputs) &&
        value.outputs.every((output) => typeof output === "string" && OUTPUT_FORMATS[output] === true) &&
        new Set(value.outputs).size === value.outputs.length);
}
export function validateDocumentSchema(document) {
    const valid = isObjectRecord(document) &&
    ...
    if (valid)
        return [];
    return [
        createDiagnostic("azeforge.document#schema-invalid", "error", "ParsedDocument does not conform to AzeMark Document schema v3."),
    ];
}
