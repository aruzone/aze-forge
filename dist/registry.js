...
import { derivationHtmlBlockRenderer, derivationPlugin, } from "./derivation.js";
import { equationHtmlBlockRenderer, equationPlugin, htmlRendererDescriptor, } from "./equation.js";
import { tableHtmlBlockRenderer, tablePlugin } from "./table.js";
import { mermaidHtmlBlockRenderer, mermaidPlugin, } from "./mermaid.js";
...
import { formulaHtmlBlockRenderer, formulaPlugin, reactionHtmlBlockRenderer, reactionPlugin, structureHtmlBlockRenderer, structurePlugin, } from "./chemistry.js";
...
import { svgBlockRenderers, svgRendererDescriptor, } from "./render-svg.js";
import { pdfBlockRenderers, pdfRendererDescriptor, } from "./render-pdf.js";
import { pngBlockRenderers, pngRendererDescriptor, } from "./render-png.js";
export const REGISTRY_CONFORMANCE_SEAM_VERSION = "2.0.0";
const SEMVER = /^(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)$/;
const PLUGIN_TYPE = /^(?:@[a-z0-9-]+(?:\/[a-z][a-z0-9-]*)?|[a-z][a-z0-9]*(?:-[a-z0-9]+)*)$/;
const ADAPTER_ID = /^(?:@[a-z0-9-]+(?:\/[a-z][a-z0-9-.]*)?|[a-z][a-z0-9]*(?:[.-][a-z0-9]+)*(?:\/[a-z0-9]+(?:[.-][a-z0-9]+)*)?)$/;
const RENDERER_CAPABILITY = {
    browser: true,
    filesystem: true,
    subprocess: true,
};
...
export function satisfiesSemverRange(version, range) {
    ...
    if (!atLeastBase)
        return false;
    if (base[0] !== 0)
        return mine[0] === base[0];
    if (base[1] !== 0)
        return mine[0] === 0 && mine[1] === base[1];
    return mine[0] === 0 && mine[1] === 0 && mine[2] === base[2];
}
export function isWellFormedVersionRange(range) {
    const trimmed = range.trim();
    if (SEMVER.test(trimmed))
        return true;
    return /^\^(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)$/.test(trimmed);
}
function rangeBaseVersion(range) {
    const trimmed = range.trim();
    if (trimmed.startsWith("^"))
        return trimmed.slice(1);
    return trimmed;
}
function versionRangesOverlap(first, second) {
    return (satisfiesSemverRange(rangeBaseVersion(first), second) ||
...
export function validateRegistry(plugins, blockRenderers, renderers) {
    const violations = [];
    const sortedPlugins = [...plugins].sort((a, b) => a.descriptor.type < b.descriptor.type ? -1 : 1);
    ...
    for (const plugin of sortedPlugins) {
        ...
        if (typeof d.bodySyntax?.id !== "string" ||
    ...
    const sortedRenderers = [...renderers].sort((a, b) => a.id === b.id
        ? a.version < b.version
            ? -1
            : 1
        : a.id < b.id
            ? -1
            : 1);
    const seenRendererKeys = new Set();
    const formatOwners = new Map();
    for (const renderer of sortedRenderers) {
        ...
        if (!Array.isArray(renderer.capabilities) ||
            renderer.capabilities.some((capability, index) => RENDERER_CAPABILITY[capability] !== true ||
    ...
    const pluginByType = new Map(plugins.map((plugin) => [plugin.descriptor.type, plugin.descriptor]));
    const rendererById = new Map(renderers.map((r) => [r.id, r]));
    const sortedBlockRenderers = [...blockRenderers].sort((a, b) => a.descriptor.id < b.descriptor.id ? -1 : 1);
    const seenBlockRendererIds = new Set();
    ...
    for (const blockRenderer of sortedBlockRenderers) {
        ...
        if (!isWellFormedVersionRange(d.pluginVersionRange) ||
        ...
        if (previous.some((range) => versionRangesOverlap(range, d.pluginVersionRange))) {
            violations.push({
                key: `${key}:overlap`,
 ...
 */
export function assertRegistryDescriptorsImmutable(registry) {
    const failures = [];
    ...
    if (failures.length > 0) {
        throw new CompilerConfigurationError("azeforge.config#mutable-descriptor", `Registry descriptors failed conformance: ${failures.sort().join(", ")}.`);
    }
}
 ...
 */
export function freezeRegistryForCompiler(registry) {
    return Object.freeze({
        plugins: Object.freeze(registry.plugins.map((plugin) => Object.freeze({
            ...plugin,
            descriptor: (() => {
        ...
        blockRenderers: Object.freeze(registry.blockRenderers.map((blockRenderer) => Object.freeze({
            ...blockRenderer,
            descriptor: (() => {
        ...
        renderers: Object.freeze(registry.renderers.map((renderer) => {
