...
const PLOT_NAMESPACE = "azeforge.plot";
const CHART_NAMESPACE = "azeforge.chart";
...
export function didYouMean(value, candidates) {
    let best;
 ...
 */
export function canonicalDecimal(spelling) {
 ...
 * ------------------------------------------------------------------ */
const EVALUABLE_FUNCS = Object.freeze({
    sin: Math.sin,
    cos: Math.cos,
    tan: Math.tan,
    asin: Math.asin,
 ...
 */
export function normalizeEvaluableSpelling(expression) {
    return expression
        .replace(/\basin\b/g, "arcsin")
        .replace(/\bacos\b/g, "arccos")
        .replace(/\batan\b/g, "arctan");
}
function nonEvaluable(detail) {
    return { failure: { kind: "non-evaluable", detail } };
}
function juxtParameterName(factors) {
    ...
    for (const factor of rest) {
        if (factor.kind !== "number" || !/^[0-9]+$/.test(factor.spelling))
            return undefined;
...
export function evaluateNode(node, scope) {
    switch (node.kind) {
        ...
        case "ident": {
            ...
            if ((fullName === "pi" || node.name === "pi") && node.digitSuffix === undefined)
                return { value: Math.PI };
            if (fullName === "e" && node.digitSuffix === undefined)
                return { value: Math.E };
        ...
        case "func": {
            ...
            if ((node.name === "max" || node.name === "min") && node.arg.kind === "vector") {
                ...
                if (values.length === 0)
                    return nonEvaluable(`function "${node.name}" needs arguments`);
                let acc = values[0] ?? 0;
        ...
        default:
            return nonEvaluable(`${node.kind} constructs are symbolic-only; sample a closed form`);
    }
}
/* ------------------------------------------------------------------ *
...
function limitExceeded(namespace, subject, count, limit, range, sourceName) {
    return diag(namespace, "limit-exceeded", `Too many ${subject}: got ${count}, limit ${limit}.`, range, sourceName, { data: { subject, count, limit } });
}
function parseBooleanField(namespace, key, value, line, sourceName, diagnostics) {
    if (value !== "true" && value !== "false") {
        diagnostics.push(diag(namespace, "invalid-attribute", `${kindWord(namespace)} attribute "${key}" must be true or false.`, valueRange(line, value), sourceName, { suggestion: `Use ${key}: true or ${key}: false.`, data: { attribute: key, value } }));
        return undefined;
    }
    return value === "true";
}
function parseDimensionField(namespace, key, value, line, sourceName, diagnostics) {
    if (!/^[0-9]+$/.test(value)) {
        diagnostics.push(diag(namespace, "invalid-attribute", `${kindWord(namespace)} attribute "${key}" must be a positive integer number of CSS pixels.`, valueRange(line, value), sourceName, { data: { attribute: key, value } }));
        return undefined;
    }
    const parsed = Number.parseInt(value, 10);
    if (parsed < 1) {
        diagnostics.push(diag(namespace, "invalid-attribute", `${kindWord(namespace)} attribute "${key}" must be at least 1.`, valueRange(line, value), sourceName, { data: { attribute: key, value } }));
        return undefined;
    }
    if (parsed > MAX_DIMENSION_PX) {
        diagnostics.push(limitExceeded(namespace, key, parsed, MAX_DIMENSION_PX, valueRange(line, value), sourceName));
        return undefined;
    }
    return parsed;
}
function parseLabelField(namespace, key, value, line, sourceName, diagnostics) {
    if (value.length === 0) {
        diagnostics.push(diag(namespace, "invalid-attribute", `${kindWord(namespace)} attribute "${key}" must not be empty.`, line.range, sourceName, {
            data: { attribute: key },
        }));
        return undefined;
    }
    if (value.length > MAX_LABEL_CHARS) {
        diagnostics.push(limitExceeded(namespace, "label", value.length, MAX_LABEL_CHARS, valueRange(line, value), sourceName));
        return undefined;
    }
    return value;
}
...
export const AXIS_SCALES = Object.freeze(["linear", "log"]);
function isAxisScale(value) {
    return AXIS_SCALES.some((scale) => scale === value);
}
function unknownField(namespace, kind, key, line, sourceName, registered, diagnostics) {
    const suggestion = didYouMean(key, registered);
    diagnostics.push(diag(namespace, "unknown-field", `${kind} field "${key}" is not registered.`, line.range, sourceName, {
        ...(suggestion === undefined ? {} : { suggestion }),
        data: { field: key, registered: registered.join(",") },
    }));
}
function duplicateField(namespace, key, line, sourceName, diagnostics) {
    diagnostics.push(diag(namespace, "duplicate-field", `${kindWord(namespace)} field "${key}" is declared more than once.`, line.range, sourceName, {
        data: { field: key },
    }));
}
function parseHeaderFields(namespace, kind, lines, nestedKeys, registered, sourceName, diagnostics) {
    ...
    while (index < cursor.length) {
        ...
        if (parsed === null || parsed === undefined || current.indent !== 0) {
            unknownField(namespace, kind, parsed?.key ?? current.text.trim(), current.line, sourceName, registered, diagnostics);
            failed = true;
            index += 1;
            continue;
        }
        ...
        if (nestedKeys.has(parsed.key)) {
            if (parsed.value !== "") {
                diagnostics.push(diag(namespace, "invalid-attribute", `${kindWord(namespace)} attribute "${parsed.key}" takes a nested mapping, not an inline value.`, valueRange(current.line, parsed.value), sourceName, { data: { attribute: parsed.key, value: parsed.value } }));
                failed = true;
                index += 1;
                continue;
            }
            ...
            while (index < cursor.length) {
                ...
                if (child === undefined || child.indent === 0)
                    break;
...
function applySharedHeader(namespace, fields, sourceName, diagnostics, defaults = {}) {
    let id;
    let number;
    ...
    for (const field of fields) {
        if (!["id", "number", "width", "height", "legend", "grid"].includes(field.key))
            continue;
        ...
        switch (field.key) {
            case "id": {
                if (!ID_PATTERN.test(field.value)) {
                    diagnostics.push(diag(namespace, "invalid-attribute", `${kindWord(namespace)} attribute "id" must match ${ID_PATTERN.source}.`, valueRange(field.line, field.value), sourceName, {
...
function parseParameters(fields, sourceName, diagnostics) {
    const parameters = {};
    let failed = false;
    const seen = new Set();
    for (const field of fields) {
        ...
        for (const child of field.children) {
            ...
            if (seen.has(child.key)) {
                diagnostics.push(diag(PLOT_NAMESPACE, "duplicate-parameter", `Plot parameter "${child.key}" is declared more than once.`, child.line.range, sourceName, { data: { parameter: child.key } }));
                failed = true;
                continue;
            }
            seen.add(child.key);
            ...
            if (canonical === undefined) {
                diagnostics.push(diag(PLOT_NAMESPACE, "invalid-parameter", `Plot parameter "${child.key}" must be an exact decimal number.`, valueRange(child.line, child.value), sourceName, { data: { parameter: child.key, value: child.value } }));
                failed = true;
                continue;
            }
    ...
    if (count > MAX_PARAMETERS && holder !== undefined) {
        diagnostics.push(limitExceeded(PLOT_NAMESPACE, "parameters", count, MAX_PARAMETERS, holder.line.range, sourceName));
        failed = true;
    }
    return { parameters, failed };
}
function parseAxisSection(name, fields, sourceName, diagnostics) {
    const axis = { scale: "linear" };
    let failed = false;
    const seen = new Set();
    for (const field of fields) {
        if (field.key !== name)
            ...
            switch (child.key) {
                ...
                case "scale": {
                    if (!isAxisScale(child.value)) {
                        diagnostics.push(diag(PLOT_NAMESPACE, "invalid-attribute", `Plot axis "${name}" scale must be linear or log.`, valueRange(child.line, child.value), sourceName, { suggestion: "Use scale: linear or scale: log.", data: { axis: name, value: child.value } }));
                        failed = true;
                    }
                    else {
                ...
                case "max": {
                    const canonical = canonicalDecimal(child.value);
                    if (canonical === undefined) {
                        diagnostics.push(diag(PLOT_NAMESPACE, "invalid-domain", `Plot axis "${name}" ${child.key} must be an exact decimal number.`, valueRange(child.line, child.value), sourceName, { data: { axis: name, bound: child.key, value: child.value } }));
                        failed = true;
                    }
    ...
    if (axis.min !== undefined && axis.max !== undefined && Number(axis.min) >= Number(axis.max)) {
        ...
        if (holder !== undefined) {
            diagnostics.push(diag(PLOT_NAMESPACE, "invalid-domain", `Plot axis "${name}" min must be less than max.`, holder.line.range, sourceName, {
...
function collectSeriesFields(namespace, seriesKind, cursor, start, openerIndent, nestedKeys, includeOpener, sourceName, diagnostics) {
    ...
    let failed = false;
    let index = start + 1;
    const consumeGroup = (key, parentIndent) => {
        const group = [];
        let itemIndent = -1;
        while (index < cursor.length) {
            const member = cursor[index];
            if (member === undefined || member.indent <= parentIndent)
                break;
    ...
    while (index < cursor.length) {
        ...
        if (current === undefined)
            break;
        if (current.indent <= openerIndent)
            break;
...
function parseDecimalValue(namespace, code, value, line, sourceName, diagnostics, extraData = {}) {
    const canonical = canonicalDecimal(value);
    if (canonical === undefined) {
        diagnostics.push(diag(namespace, code, `Value "${value}" is not an exact decimal number.`, valueRange(line, value), sourceName, {
...
function checkErrorFields(namespace, point, line, index, sourceName, diagnostics) {
    const hasLow = point.errorLow !== undefined;
    const hasHigh = point.errorHigh !== undefined;
    if (hasLow !== hasHigh) {
        diagnostics.push(diag(namespace, "mismatched-error-fields", `Point ${index} declares only one of error-low/error-high; use both or symmetric error:.`, line.range, sourceName, { data: { index } }));
        return false;
    }
    for (const [name, raw] of [
        ...
        if (raw === undefined)
            continue;
        if (Number(raw) < 0) {
            diagnostics.push(diag(namespace, "negative-error", `Point ${index} field "${name}" must not be negative.`, line.range, sourceName, {
...
function parsePointsGroup(namespace, group, holder, seriesIndex, xLog, yLog, sourceName, diagnostics) {
    const points = [];
    let failed = false;
    let index = 0;
    let pointIndex = 0;
    ...
    while (index < group.length) {
        ...
        while (index < group.length) {
            ...
            if (member === undefined || member.indent <= opener.indent)
                break;
        ...
        if (rawX === undefined || rawY === undefined) {
            diagnostics.push(diag(namespace, "invalid-datum", `Point ${currentIndex} is missing ${rawX === undefined ? "x" : "y"}.`, opener.line.range, sourceName, {
        ...
        if (xLog && Number(x) <= 0)
            badX.push(currentIndex);
        if (yLog && Number(y) <= 0)
            badY.push(currentIndex);
        points.push({
            x,
            y,
    ...
    if (badX.length > 0) {
        diagnostics.push(diag(namespace, "log-axis-value", `Series ${seriesIndex} has non-positive x values on a log x-axis.`, holder.range, sourceName, {
            data: { series: seriesIndex, axis: "x", indexes: badX.join(",") },
        }));
        failed = true;
    }
    if (badY.length > 0) {
        diagnostics.push(diag(namespace, "log-axis-value", `Series ${seriesIndex} has non-positive y values on a log y-axis.`, holder.range, sourceName, {
            data: { series: seriesIndex, axis: "y", indexes: badY.join(",") },
        }));
        failed = true;
    }
    if (points.length === 0 && !failed) {
        diagnostics.push(diag(namespace, "empty-series", "Point series declares no points.", holder.range, sourceName, {}));
        failed = true;
    }
...
function validateFunctionExpression(expression, variable, parameters, line, sourceName, diagnostics) {
    if (expression.length === 0) {
        diagnostics.push(diag(PLOT_NAMESPACE, "missing-expression", "Function series requires an expression.", line.range, sourceName, {}));
        return undefined;
    }
    if (expression.length > MAX_EXPRESSION_CHARS) {
        diagnostics.push(limitExceeded(PLOT_NAMESPACE, "expression", expression.length, MAX_EXPRESSION_CHARS, valueRange(line, expression), sourceName));
        return undefined;
    }
    ...
    if (!("tree" in parsed)) {
        ...
        diagnostics.push(diag(PLOT_NAMESPACE, "non-evaluable-construct", `Expression does not parse as evaluable mathematics: ${first?.message ?? "invalid expression"}.`, line.range, sourceName, { suggestion: "Write a closed numeric form over the domain variable and declared parameters." }));
        return undefined;
    }
    ...
    for (const name of parameters.keys())
        probe.set(name, 1.5);
    const probeScope = { variable, variableValue: 1.5, parameters: probe };
    const failure = checkEvaluable(parsed.tree, probeScope);
    if (failure !== undefined) {
        if (failure.kind === "unbound") {
            diagnostics.push(diag(PLOT_NAMESPACE, "unbound-variable", `Expression names "${failure.name}": declare it under parameters: or use the domain variable "${variable}".`, line.range, sourceName, { data: { name: failure.name, variable } }));
        }
        else {
            diagnostics.push(diag(PLOT_NAMESPACE, "non-evaluable-construct", `Expression is not evaluable: ${failure.detail}.`, line.range, sourceName, {
...
function checkEvaluable(node, scope) {
...
export function validatePlotBlock(options) {
    ...
    if (shared === undefined || shared.failed)
        return { diagnostics };
    ...
    if (parametersFailed || xFailed || yFailed)
        return { diagnostics };
    const parameterValues = new Map();
    for (const [name, canonical] of Object.entries(parameters))
        parameterValues.set(name, Number(canonical));
    ...
    while (index < cursor.length) {
        ...
        if (kind !== "function" && kind !== "line" && kind !== "scatter") {
            ...
            diagnostics.push(diag(PLOT_NAMESPACE, "unknown-series-kind", `Plot series kind "${kind}" is not registered.`, opener.line.range, sourceName, {
        ...
        if (kind === "function") {
            ...
            if (variableRaw !== undefined && !VARIABLE_PATTERN.test(variable)) {
                diagnostics.push(diag(PLOT_NAMESPACE, "invalid-attribute", `Function series variable "${variable}" must be a single ASCII letter.`, valueRange(variableRaw.line, variableRaw.value), sourceName, { data: { variable } }));
                failed = true;
                continue;
            }
            if (parameterValues.has(variable)) {
                diagnostics.push(diag(PLOT_NAMESPACE, "duplicate-parameter", `Function series variable "${variable}" collides with a parameters: entry; one scope, one declaration per name.`, variableRaw === undefined ? head.opener.range : valueRange(variableRaw.line, variableRaw.value), sourceName, { data: { variable } }));
                failed = true;
                continue;
            }
            const expressionField = scalar.get("expression");
            if (expressionField === undefined) {
                diagnostics.push(diag(PLOT_NAMESPACE, "missing-expression", "Function series requires an expression.", head.opener.range, sourceName, {}));
                failed = true;
                continue;
            }
            ...
            if (domainGroup === undefined) {
                diagnostics.push(diag(PLOT_NAMESPACE, "missing-domain", "Function series require an explicit domain.", head.opener.range, sourceName, {
            ...
            if (domainFailed || domainMin === undefined || domainMax === undefined) {
                if (!domainFailed) {
                    diagnostics.push(diag(PLOT_NAMESPACE, "missing-domain", "Function series domain requires min: and max: bounds.", head.opener.range, sourceName, {}));
                }
                failed = true;
                continue;
            }
            if (Number(domainMin) >= Number(domainMax)) {
                diagnostics.push(diag(PLOT_NAMESPACE, "invalid-domain", "Function series domain min must be less than max.", head.opener.range, sourceName, {
            ...
            if (xAxisFields.scale === "log" && Number(domainMin) <= 0) {
                diagnostics.push(diag(PLOT_NAMESPACE, "log-domain-invalid", "Function domain under a log x-axis must be positive.", head.opener.range, sourceName, {
            ...
            if (samplesRaw !== undefined) {
                if (!/^[0-9]+$/.test(samplesRaw.value)) {
                    diagnostics.push(diag(PLOT_NAMESPACE, "invalid-samples", `Function series samples "${samplesRaw.value}" must be an integer.`, valueRange(samplesRaw.line, samplesRaw.value), sourceName, {
                ...
                if (samples < MIN_SAMPLES) {
                    diagnostics.push(diag(PLOT_NAMESPACE, "invalid-samples", `Function series samples must be at least ${MIN_SAMPLES}.`, valueRange(samplesRaw.line, samplesRaw.value), sourceName, {
                        data: { value: samplesRaw.value },
                    }));
                    failed = true;
                    continue;
                ...
                if (samples > MAX_SAMPLES) {
                    diagnostics.push(limitExceeded(PLOT_NAMESPACE, "samples", samples, MAX_SAMPLES, valueRange(samplesRaw.line, samplesRaw.value), sourceName));
                    failed = true;
                    continue;
                }
            }
            ...
            if (pointsGroup === undefined) {
                diagnostics.push(diag(PLOT_NAMESPACE, "missing-points", `${kind} series require a points: collection.`, head.opener.range, sourceName, {}));
                failed = true;
                continue;
            }
...
function sampleFunctionWarnings(block, sourceName) {
    const warnings = [];
    for (const entry of block.series) {
        ...
        if (samples.values.every((value) => value === undefined)) {
            warnings.push(diag(PLOT_NAMESPACE, "all-undefined-samples", `Function series${entry.label === undefined ? "" : ` "${entry.label}"`} produced no finite samples; the curve renders absent.`, block.range, sourceName, { severity: "warning", data: { expression: entry.expression } }));
        }
    }
    return warnings;
}
export function sampleFunctionSeries(series, parameters) {
    const min = Number(series.domainMin);
    const max = Number(series.domainMax);
    const count = series.samples;
    const scope = new Map();
    for (const [name, canonical] of Object.entries(parameters))
        scope.set(name, Number(canonical));
    const xs = [];
    const values = [];
    for (let i = 0; i < count; i += 1) {
        const x = count === 1 ? min : min + (i * (max - min)) / (count - 1);
        xs.push(x);
        const result = evaluateNode(series.tree, {
...
function parseBarsGroup(group, holder, sourceName, diagnostics) {
    const bars = [];
    let failed = false;
    let index = 0;
    let barIndex = 0;
    while (index < group.length) {
        ...
        if (rawCategory === undefined || rawValue === undefined) {
            diagnostics.push(diag(CHART_NAMESPACE, "invalid-datum", `Bar ${currentIndex} is missing ${rawCategory === undefined ? "category" : "value"}.`, opener.line.range, sourceName, {
                data: { index: currentIndex },
            }));
            failed = true;
            continue;
        ...
        if (rawCategory.value.length === 0) {
            diagnostics.push(diag(CHART_NAMESPACE, "invalid-datum", `Bar ${currentIndex} category must not be empty.`, opener.line.range, sourceName, {
                data: { index: currentIndex },
            }));
            failed = true;
            continue;
        ...
        if (rawCategory.value.length > MAX_LABEL_CHARS) {
            diagnostics.push(limitExceeded(CHART_NAMESPACE, "label", rawCategory.value.length, MAX_LABEL_CHARS, opener.line.range, sourceName));
            failed = true;
            continue;
        }
...
function parseScalarList(group, code, sourceName, diagnostics) {
    const values = [];
    let failed = false;
...
export function validateChartBlock(options) {
    const { headerLines, bodyLines, blockRange, sourceName, defaults } = options;
    const diagnostics = [];
    const fields = parseHeaderFields(CHART_NAMESPACE, "Chart", headerLines, new Set(), CHART_TOP_LEVEL_FIELDS, sourceName, diagnostics);
    if (fields === undefined)
        return { diagnostics };
    const seenTop = new Set();
    for (const field of fields) {
    ...
    if (typeField === undefined) {
        diagnostics.push(diag(CHART_NAMESPACE, "unknown-chart-type", "Chart Block requires a type: field.", blockRange, sourceName, {
    ...
    if (!CHART_TYPES.includes(typeField.value)) {
        diagnostics.push(diag(CHART_NAMESPACE, "unknown-chart-type", `Chart type "${typeField.value}" is not registered.`, valueRange(typeField.line, typeField.value), sourceName, {
            suggestion: "Use type: bar, grouped-bar, stacked-bar, or histogram.",
            data: { type: typeField.value, registered: CHART_TYPES.join(",") },
        }));
        return { diagnostics };
    }
    const chartType = typeField.value;
    let xLabel;
    let yLabel;
    let yMin;
    let yMax;
    for (const key of ["x-label", "y-label"]) {
    ...
    if (yMin !== undefined && yMax !== undefined && Number(yMin) >= Number(yMax)) {
        diagnostics.push(diag(CHART_NAMESPACE, "invalid-domain", "Chart y-min must be less than y-max.", blockRange, sourceName, {
    ...
    while (index < cursor.length) {
        ...
        if (chartType === "histogram") {
            if (scalar.has("bars")) {
                diagnostics.push(diag(CHART_NAMESPACE, "invalid-series", "Histogram series require values:, not bars:.", head.opener.range, sourceName, {}));
                failed = true;
                continue;
            }
            const valuesGroup = nested.get("values");
            if (valuesGroup === undefined) {
                diagnostics.push(diag(CHART_NAMESPACE, "missing-values", "Histogram series require a values: collection.", head.opener.range, sourceName, {}));
                failed = true;
                continue;
            }
            ...
            if (edgesGroup !== undefined) {
                ...
                if (binCountRaw === undefined || minRaw === undefined || maxRaw === undefined) {
                    diagnostics.push(diag(CHART_NAMESPACE, "missing-binning", "Histogram series require edges: or bin-count: with min: and max:.", head.opener.range, sourceName, {}));
                    failed = true;
                    continue;
                }
                if (!/^[0-9]+$/.test(binCountRaw.value) || Number.parseInt(binCountRaw.value, 10) < 1) {
                    diagnostics.push(diag(CHART_NAMESPACE, "invalid-edges", `Histogram bin-count "${binCountRaw.value}" must be a positive integer.`, valueRange(binCountRaw.line, binCountRaw.value), sourceName, {
                ...
                if (Number(min) >= Number(max)) {
                    diagnostics.push(diag(CHART_NAMESPACE, "invalid-edges", "Histogram min must be less than max.", head.opener.range, sourceName, {
                        data: { min, max },
                    }));
                    failed = true;
                    continue;
            ...
            for (let e = 1; e < edges.length; e += 1) {
                if (Number(edges[e] ?? 0) <= Number(edges[e - 1] ?? 0)) {
                    diagnostics.push(diag(CHART_NAMESPACE, "invalid-edges", "Histogram edges must be strictly increasing.", head.opener.range, sourceName, {
            ...
            values.forEach((entry, valueIndex) => {
                ...
                if (numeric < lo || numeric > hi)
                    outOfRange.push(valueIndex);
            });
            if (outOfRange.length > 0) {
                diagnostics.push(diag(CHART_NAMESPACE, "value-out-of-bin-range", `Histogram values fall outside the edges: indexes ${outOfRange.join(", ")}.`, head.opener.range, sourceName, {
            ...
            if (scalar.has("values") || scalar.has("edges") || scalar.has("bin-count")) {
                diagnostics.push(diag(CHART_NAMESPACE, "invalid-series", `Bar-family series require bars:, not histogram data.`, head.opener.range, sourceName, {}));
                failed = true;
                continue;
            }
            const barsGroup = nested.get("bars");
            if (barsGroup === undefined) {
                diagnostics.push(diag(CHART_NAMESPACE, "missing-bars", "Bar series require a bars: collection.", head.opener.range, sourceName, {}));
                failed = true;
                continue;
            }
            ...
            for (const bar of bars) {
                if (seenCategories.has(bar.category)) {
                    diagnostics.push(diag(CHART_NAMESPACE, "duplicate-category", `Bar category "${bar.category}" repeats within one series.`, head.opener.range, sourceName, {
                        severity: "warning",
 ...
 * ------------------------------------------------------------------ */
function niceLinearDomain(lo, hi) {
    if (!(lo < hi)) {
        lo -= 1;
        hi += 1;
    }
...
function resolveAxisDomain(axis, fitted, logFallback) {
    ...
    if (authoredMin !== undefined && authoredMax !== undefined)
        return [authoredMin, authoredMax];
    const finite = fitted.filter((value) => Number.isFinite(value) && (axis.scale === "linear" || value > 0));
    let lo = authoredMin;
    let hi = authoredMax;
    if (lo === undefined || hi === undefined) {
        if (finite.length === 0)
            return axis.scale === "log" ? logFallback : [0, 1];
    ...
    const finalHi = hi ?? 1;
    if (!(finalLo < finalHi))
        return axis.scale === "log" ? niceLogDomain(finalLo, finalHi) : niceLinearDomain(finalLo, finalHi);
...
export function assertPlotFragmentSafe(svg) {
...
function renderErrorBar(px, loValue, hiValue, yScale, color) {
    const lo = quantize(yScale(loValue));
    const hi = quantize(yScale(hiValue));
    const x = quantize(px);
    const cap = 4;
    return (`<line x1="${x}" y1="${lo}" x2="${x}" y2="${hi}" stroke="${color}" stroke-width="1.5"/>` +
...
function plotContentSeed(block) {
    const parameters = {};
...
function chartContentSeed(block) {
    return JSON.stringify({
        id: block.id ?? "",
...
function makeScale(scale, domain, range) {
...
function functionSegments(samples, xScale, yScale, yLog, yRange) {
    const segments = [];
    let current = [];
    let previous;
    const transform = (value) => (yLog ? Math.log10(value) : value);
...
export function renderPlotFragment(block, _context) {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${block.width}" height="${block.height}" viewBox="0 0 ${block.width} ${block.height}" role="img">` +
...
function histogramCounts(values, edges) {
    ...
    const counts = new Array(numericEdges.length - 1).fill(0);
...
export function renderChartFragment(block, _context) {
    ...
    if (block.chartType === "histogram") {
        ...
        const edgeValues = histograms.length === 0
            ? [0, 1]
            : [
        ...
        histograms.forEach((entry, seriesIndex) => {
            ...
            seriesCounts.forEach((count, bin) => {
                ...
                parts.push(`<rect x="${quantize(bx)}" y="${quantize(by)}" width="${quantize(width)}" height="${quantize(Math.max(0, yScale(0) - by))}" fill="${color}"/>`);
                void binCount;
            });
        });
    }
        ...
        barSeries.forEach((entry, seriesIndex) => {
            ...
            categories.forEach((category) => {
                ...
                if (bar === undefined && !stacked)
                    return;
                parts.push(`<rect x="${quantize(bx)}" y="${quantize(yTop)}" width="${quantize(width)}" height="${quantize(Math.max(0, yBase - yTop))}" fill="${color}"/>`);
                if (bar !== undefined && (bar.error !== undefined || (bar.errorLow !== undefined && bar.errorHigh !== undefined))) {
                    const span = bar.error !== undefined
...
export function plotDependencyClosure() {
...
export const plotPlugin = Object.freeze({
...
export const chartPlugin = Object.freeze({
    descriptor: chartPluginDescriptor,
});
export const PLOT_HTML_BLOCK_RENDERER_ID = "azeforge.plot.html/v1";
export const PLOT_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
export const CHART_HTML_BLOCK_RENDERER_ID = "azeforge.chart.html/v1";
export const CHART_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
export const PLOT_HTML_RENDERER_ID = "html";
export const PLOT_HTML_RENDERER_VERSION = "1.0.0";
...
export const plotHtmlBlockRenderer = Object.freeze({
    descriptor: plotBlockRendererDescriptor,
    render: renderPlotFragment,
});
export const chartHtmlBlockRenderer = Object.freeze({
    descriptor: chartBlockRendererDescriptor,
    render: renderChartFragment,
});
export const EMPTY_DOCUMENT_DEFAULTS = Object.freeze({});
...
function parseDefaultsSection(section, value, line, sourceName, diagnostics) {
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        diagnostics.push(diag("azeforge.metadata", "invalid-defaults", `Front matter defaults.${section} must be a mapping.`, line.range, sourceName, {
            data: { section },
        }));
        return undefined;
    }
    ...
    for (const [key, entry] of Object.entries(value)) {
        if (!DEFAULT_SETTINGS_FIELDS.includes(key)) {
            ...
            diagnostics.push(diag("azeforge.metadata", "unknown-key", `Unknown front matter defaults key "${key}".`, line.range, sourceName, {
                ...(suggestion === undefined ? {} : { suggestion }),
                data: { key },
            }));
            failed = true;
        ...
        if (key === "legend" || key === "grid") {
            if (entry !== true && entry !== false) {
                diagnostics.push(diag("azeforge.metadata", "invalid-defaults", `Front matter defaults.${section}.${key} must be true or false.`, line.range, sourceName, {
                    data: { section, key },
                }));
                failed = true;
                continue;
        ...
        if (!Number.isInteger(entry) || entry < 1) {
            diagnostics.push(diag("azeforge.metadata", "invalid-defaults", `Front matter defaults.${section}.${key} must be a positive integer number of CSS pixels.`, line.range, sourceName, {
                data: { section, key },
            }));
            failed = true;
            continue;
        }
        if (entry > MAX_DIMENSION_PX) {
            diagnostics.push(limitExceeded("azeforge.metadata", key, entry, MAX_DIMENSION_PX, line.range, sourceName));
            failed = true;
            continue;
        }
        settings[key] = entry;
 ...
 * silently dropped.
 */
export function parseDocumentDefaults(value, line, sourceName) {
    const diagnostics = [];
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        diagnostics.push(diag("azeforge.metadata", "invalid-defaults", "Front matter defaults: must be a mapping.", line.range, sourceName, {}));
    ...
    for (const [section, entry] of Object.entries(value)) {
        if (!DEFAULT_SECTIONS.includes(section)) {
            ...
            diagnostics.push(diag("azeforge.metadata", "unknown-key", `Unknown front matter defaults section "${section}".`, line.range, sourceName, {
                ...(suggestion === undefined ? {} : { suggestion }),
                data: { section },
            }));
            failed = true;
