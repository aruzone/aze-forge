...
import { ELKJS_VERSION, STUB_GLYPH_SIZE, STUB_LABEL_GAP, controlLabelTypography, layoutControl, } from "./control-layout.js";
import { CONTROL_EMITTER_VERSION, CONTROL_LAYOUT_VERSION, CONTROL_OPTIONS_VERSION, CONTROL_PLUGIN_TYPE, CONTROL_PLUGIN_VERSION, } from "./control-schemas.js";
import { escapeXml, quantize } from "./plot.js";
export const CONTROL_HTML_BLOCK_RENDERER_ID = "azeforge.control.html/v1";
export const CONTROL_HTML_BLOCK_RENDERER_VERSION = "1.0.0";
...
export class ControlRenderError extends Error {
    code = "azeforge.renderer#control-render";
    remedy = "Re-check the control declaration list; a renderer failure publishes no Artifact.";
    constructor(message) {
        super(message);
...
function assertExtent(layout) {
    ...
    if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
        throw new ControlRenderError(`The control figure has no positive finite extent (${quantize(width)} × ${quantize(height)} px).`);
    }
}
/** Fail-closed norm extended to control fragments: generated markup must never carry executable content. */
export function assertControlFragmentSafe(svg) {
...
function assertProjection(condition, detail) {
    if (!condition) {
        throw new ControlRenderError(`The control layout does not match the authored Block: ${detail}.`);
    }
}
/**
 ...
 */
function stackedText(lines, centreX, centreY, className, anchor) {
    ...
    for (let index = 0; index < shown.length; index += 1) {
        ...
        if (line === undefined)
            continue;
        if (index > 0)
            offset += (previousLineHeight + line.lineHeightPx) / 2;
        offsets.push(offset);
    ...
    const spans = shown
        ...
        return (`<tspan x="${quantize(centreX)}" y="${quantize(y)}"` +
            ` font-size="${quantize(line.fontSizePx)}">${escapeXml(line.value)}</tspan>`);
    })
        .join("");
    return (`<text class="${className}" text-anchor="${anchor}" dominant-baseline="middle">` +
        `${spans}</text>`);
}
function oneLine(value, fontSizePx, lineHeightPx) {
 ...
 */
function blockTextLines(item, typography) {
    const lines = [
        {
...
function arrowMarkup(edge, theme) {
    ...
    return (`<polygon class="aze-control-arrow" points="${points}"` +
 ...
 */
function sumSignAnchors(node, flow, count) {
 ...
 */
function takeoffPoint(outEdges) {
 ...
 */
function describeControl(block) {
    ...
    for (const declaration of block.declarations) {
        switch (declaration.kind) {
            ...
            case "edge": {
                ...
                edges.push(`${declaration.from} to ${declaration.to}${label === "" ? "" : ` (${label})`}`);
                break;
            }
        }
    }
    ...
    if (authored !== "")
        parts.push(`${authored}.`);
    if (items.length > 0)
        parts.push(`Items in authored order: ${items.join("; ")}.`);
    if (edges.length > 0)
        parts.push(`Edges in authored order: ${edges.join("; ")}.`);
    return truncateDescription(parts.join(" "));
}
function controlTitle(block, ordinal) {
 ...
 * Emitter
 * ------------------------------------------------------------------ */
function emitControlFragment(block, layout, ordinal, theme) {
    assertExtent(layout);
    ...
    const decoration = [];
    const texts = [];
    const push = (bucket, order, markup) => {
        if (markup !== "")
            bucket.push({ order, markup });
    };
    for (const node of layout.nodes) {
        const item = items.get(node.name);
        assertProjection(item !== undefined && item.declaration.kind === node.kind, `node "${node.name}" (${node.kind}) has no matching authored item`);
        if (item === undefined)
            continue;
        ...
        switch (declaration.kind) {
            case "block":
                push(decoration, item.order, `<rect id="${id}" class="${NODE_CLASS} aze-control-block"` +
                ...
                push(texts, item.order, stackedText(blockTextLines(declaration, typography), centreX, centreY, BLOCK_LABEL_CLASS, "middle"));
                break;
            case "sum": {
                push(decoration, item.order, `<circle id="${id}" class="${NODE_CLASS} aze-control-sum"` +
                ...
                declaration.signs.forEach((sign, index) => {
                    ...
                    if (at === undefined)
                        return;
                    push(texts, item.order, stackedText(oneLine(sign, typography.sumSignFontSizePx, typography.sumSignFontSizePx), at.x, at.y, SUM_SIGN_CLASS, "middle"));
                });
                break;
            }
            case "input":
            case "output": {
                ...
                push(decoration, item.order, `<rect id="${id}" class="${NODE_CLASS} aze-control-${declaration.kind}"` +
                ...
                push(texts, item.order, stackedText(oneLine(circuitTextValue(declaration.label), typography.stubFontSizePx, typography.stubLineHeightPx), before ? glyphX - STUB_LABEL_GAP : glyphX + STUB_GLYPH_SIZE + STUB_LABEL_GAP, centreY, STUB_LABEL_CLASS, before ? "end" : "start"));
                break;
            }
        }
    }
    for (const edge of layout.edges) {
        ...
        assertProjection(authored !== undefined &&
            authored.declaration.from === edge.from &&
            authored.declaration.to === edge.to, `edge ${edge.declarationIndex} is "${edge.from} to ${edge.to}", not the authored ` +
        ...
        if (data === "")
            continue;
        let markup = `<path id="${base}-e-${quantize(edge.declarationIndex)}" class="aze-control-edge"` +
        ...
        if (label !== undefined && at !== undefined) {
            ...
            push(texts, authored.order, stackedText(oneLine(circuitTextValue(label), typography.signalFontSizePx, typography.signalLineHeightPx), labelX, labelY, EDGE_LABEL_CLASS, "middle"));
        }
    ...
    for (const [name, outgoing] of outEdges) {
        ...
        if (node === undefined || item === undefined || at === undefined)
            continue;
        push(decoration, item.order, `<circle id="${base}-t-${quantize(node.declarationIndex)}" class="aze-control-takeoff"` +
            ` cx="${quantize(at.x)}" cy="${quantize(at.y)}"` +
            ` r="${quantize(control.takeoffRadiusPx)}" fill="${escapeXml(control.takeoffFill)}"/>`);
    }
    const ordered = (bucket) => bucket
        .slice()
    ...
    const svg = `<svg xmlns="${SVG_NAMESPACE}" viewBox="0 0 ${quantize(layout.width)} ${quantize(layout.height)}"` +
 ...
 */
export async function renderControlFragment(block, context) {
 ...
 */
export function controlDependencyClosure() {
...
export const controlHtmlBlockRenderer = Object.freeze({
...
//# sourceMappingURL=control-render.js.map