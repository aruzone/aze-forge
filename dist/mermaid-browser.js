...
import { Browser as BrowserKind, computeExecutablePath, } from "@puppeteer/browsers";
import puppeteer from "puppeteer-core";
import { loadInterFontFaces } from "./font.js";
export const CHROME_HEADLESS_SHELL_VERSION = "152.0.7977.75";
...
export function throwIfDeniedBrowserRequest(deniedRequest) {
    if (deniedRequest !== undefined) {
        throw new BrowserCapabilityError(`Layout requested an external resource: ${deniedRequest}`);
    }
}
let mermaidScriptPromise;
let fontCssPromise;
function loadMermaidScript() {
    mermaidScriptPromise ??= readFile(new URL(import.meta.resolve("mermaid/dist/mermaid.min.js")), "utf8");
    return mermaidScriptPromise;
}
function loadFontCss() {
    fontCssPromise ??= loadInterFontFaces().then((faces) => faces
        .map((face) => `@font-face{font-family:Inter;font-style:normal;font-weight:${face.weight};font-display:block;src:url(data:font/woff2;base64,${face.data}) format("woff2");unicode-range:${face.unicodeRange}}`)
        .join(""));
...
async function renderAndSettle(page, input) {
    return page.evaluate(async (browserInput) => {
        // Well-known browser global: Mermaid UMD script installs window.mermaid.
        const browserGlobal = globalThis;
        const mermaid = browserGlobal["mermaid"];
        if (mermaid === undefined)
            throw new Error("Pinned Mermaid script did not load.");
        ...
        if (root === null)
            throw new Error("Mermaid render root is missing.");
        ...
        const rendered = await mermaid.render(browserInput.elementId, browserInput.source, root);
        ...
        if ([...first.viewBox, ...first.bounds].some((value) => !Number.isFinite(value)) ||
            viewBoxWidth <= 0 ||
            viewBoxHeight <= 0 ||
            boundsWidth <= 0 ||
            boundsHeight <= 0) {
...
export function resolvePinnedBrowserExecutable() {
...
const pinnedCapabilities = Object.freeze({
    resolveExecutable() {
...
async function acquirePinnedBrowser(capabilities) {
    let browser;
    try {
        ...
        throw new MermaidBrowserUnavailableError(`Pinned Chrome Headless Shell ${CHROME_HEADLESS_SHELL_VERSION} is unavailable. Reinstall AzeForge browser dependencies. ${detail}`);
    }
    try {
        const browserVersion = await browser.version();
        if (browserVersion !== `HeadlessChrome/${CHROME_HEADLESS_SHELL_VERSION}`) {
            throw new MermaidBrowserUnavailableError(`Expected HeadlessChrome/${CHROME_HEADLESS_SHELL_VERSION}, received ${browserVersion}.`);
        }
        return { browser, browserVersion };
    }
    catch (error) {
        await browser.close();
        if (error instanceof MermaidBrowserUnavailableError)
            throw error;
        const detail = error instanceof Error ? error.message : String(error);
        throw new MermaidBrowserUnavailableError(`Pinned Chrome Headless Shell ${CHROME_HEADLESS_SHELL_VERSION} could not be verified. Reinstall AzeForge browser dependencies. ${detail}`);
    }
}
export async function launchPinnedBrowser() {
    return (await acquirePinnedBrowser(pinnedCapabilities)).browser;
}
export async function renderMermaidInBrowser(input, capabilities = pinnedCapabilities) {
    ...
    try {
        ...
        try {
            ...
            await isolated.page.setContent(`<!doctype html><meta charset="utf-8"><style>${fontCss}*{animation:none!important;transition:none!important;caret-color:transparent!important}</style><main id="azeforge-mermaid-root"></main>`, { waitUntil: "domcontentloaded" });
            ...
            if (svg.startsWith("AZE_MERMAID_PARSE_ERROR:")) {
                throw new MermaidBrowserParseError(svg.slice("AZE_MERMAID_PARSE_ERROR:".length));
            }
            if (deniedRequest !== undefined) {
                throw new MermaidCapabilityError(`Browser request was denied: ${deniedRequest}`);
            }
